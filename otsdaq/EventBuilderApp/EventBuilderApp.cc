...
EventBuilderApp::EventBuilderApp(xdaq::ApplicationStub* stub)
    ...
    : xdaq::Application(stub),
      ...
      theConfigurationManager_(new ConfigurationManager)  //(Singleton<ConfigurationManager>::getInstance()) //I always
                                                          //load the full config but if I want to load a partial
                                                          //configuration (new ConfigurationManager)
      ,
      XDAQContextConfigurationName_(
          theConfigurationManager_->__GET_CONFIG__(XDAQContextConfiguration)->getConfigurationName()),
...
//        theFEWInterfacesManager_    (theConfigurationManager_,this->getApplicationDescriptor()->getInstance())
{
  ...
  try {
    supervisorContextUID_ = theConfigurationManager_->__GET_CONFIG__(XDAQContextConfiguration)
  ...
  try {
    supervisorApplicationUID_ = theConfigurationManager_->__GET_CONFIG__(XDAQContextConfiguration)
                                    ->getApplicationUID(getApplicationContext()->getContextDescriptor()->getURL(),
...
//========================================================================================================================
EventBuilderApp::~EventBuilderApp(void) { destroy(); }
//========================================================================================================================
void EventBuilderApp::init(void) {
  ...
  std::cout << __COUT_HDR_FL__ << "ARTDAQBUILDER SUPERVISOR NO ERRORS MAKING MSG FACILITY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
            << std::endl;
  ...
  std::cout << __COUT_HDR_FL__ << "ARTDAQBUILDER SUPERVISOR MSG FACILITY DONE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
            << std::endl;
...
bool EventBuilderApp::stateMachineThread(toolbox::task::WorkLoop* workLoop) {
  ...
  std::cout << __COUT_HDR_FL__ << "Re-sending message..."
            << SOAPUtilities::translate(stateMachineWorkLoopManager_.getMessage(workLoop)).getCommand() << std::endl;
...

{
  ...
  std::pair<std::string /*group name*/, ConfigurationGroupKey> theGroup(
      ...
      ConfigurationGroupKey(SOAPUtilities::translate(theStateMachine_.getCurrentMessage())
                                .getParameters()
                                .getValue("ConfigurationGroupKey")));

  __COUT__ << "Configuration group name: " << theGroup.first << " key: " << theGroup.second << std::endl;
  ...
  // Now that the configuration manager has all the necessary configurations I can create all objects dependent of the
  // configuration  std::string configString = "daq:{event_builder:{expected_fragments_per_event:2
  // first_fragment_receiver_rank:0 fragment_receiver_count:2 mpi_buffer_count:16 print_event_store_stats:true
  // send_triggers:true trigger_port:5001 use_art:true verbose:false} max_fragment_size_words:2.097152e6}
  // outputs:{netMonOutput:{module_type:\"NetMonOutput\"}} physics:{my_output_modules:[\"netMonOutput\"]}
  // process_name:\"DAQ\" services:{Timing:{summaryOnly:true} scheduler:{fileMode:\"NOMERGE\"}
  // user:{NetMonTransportServiceInterface:{data_receiver_count:1 first_data_receiver_rank:4
  // max_fragment_size_words:2.097152e6 mpi_buffer_count:8 service_provider:\"NetMonTransportService\"}}}
  // source:{fragment_type_map:[[1,\"UDP\"]] module_type:\"RawInput\" resume_after_timeout:true waiting_time:900}";

  // ONLY 1 BOARD READER IN THE SYSTEM
  // std::string configString = "daq:{event_builder:{expected_fragments_per_event:1 first_fragment_receiver_rank:0
  // fragment_receiver_count:1 mpi_buffer_count:16 print_event_store_stats:true send_triggers:true trigger_port:5001
  // use_art:true verbose:false} max_fragment_size_words:2.097152e6}
  // outputs:{netMonOutput:{module_type:\"RootMPIOutput\"}} physics:{analyzers:{wf: {module_type:\"WFViewer\"
  // fragment_ids:[0] fragment_type_labels:[ \"DataGen\" ] prescale:60 write_to_file:true
  // fileName:\"/tmp/otsdaqdemo_onmon_evb.root\"}}  a1:[\"wf\"] my_output_modules:[\"netMonOutput\"]}
  // process_name:\"DAQ\" services:{Timing:{summaryOnly:true} scheduler:{fileMode:\"NOMERGE\" errorOnFailureToPut: false}
  // user:{NetMonTransportServiceInterface:{data_receiver_count:1 first_data_receiver_rank:2
  // max_fragment_size_words:2.097152e6 mpi_buffer_count:8 service_provider:\"NetMonTransportService\"}}}
  // source:{fragment_type_map:[[1,\"UDP\"], [4,\"DataGen\"]] module_type:\"RawInput\" resume_after_timeout:true
  // waiting_time:900}";
  ...
  __COUT__ << std::endl;
  //__COUT__ <<
  //theConfigurationManager_->getNode(XDAQContextConfigurationName_).getNode(supervisorConfigurationPath_).getNode("ConfigurationString").getValue<std::string>()
  //<< std::endl;
  ...
  std::string uid =
      theConfigurationManager_->getNode(XDAQContextConfigurationName_).getNode(supervisorConfigurationPath_).getValue();
  ...
  try {
    ...
    throw toolbox::fsm::exception::Exception("Transition Error" /*name*/, ss.str() /* message*/,
                                             "EventBuilderApp::transitionConfiguring" /*module*/, __LINE__ /*line*/,
                                             __FUNCTION__ /*function*/
    );
  }

  std::cout << __COUT_HDR_FL__ << "ARTDAQBUILDER SUPERVISOR DONE CONFIGURING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
            << std::endl;
}
...

{
  art::RunID runId((art::RunNumber_t)boost::lexical_cast<art::RunNumber_t>(
      SOAPUtilities::translate(theStateMachine_.getCurrentMessage()).getParameters().getValue("RunNumber")));
