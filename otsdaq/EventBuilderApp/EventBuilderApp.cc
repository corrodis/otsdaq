...
EventBuilderApp::EventBuilderApp(xdaq::ApplicationStub* stub)
    ...
    , stateMachineWorkLoopManager_(
          toolbox::task::bind(this, &EventBuilderApp::stateMachineThread, "StateMachine"))
...
void EventBuilderApp::init(void)
{
	std::cout << __COUT_HDR_FL__
	          << "ARTDAQBUILDER SUPERVISOR INIT START!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
	          << std::endl;
	...
	std::cout << __COUT_HDR_FL__
	          << "ARTDAQBUILDER SUPERVISOR NO ERRORS MAKING MSG "
	             "FACILITY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
	          << std::endl;
	...
	std::cout
	    << __COUT_HDR_FL__
	    << "ARTDAQBUILDER SUPERVISOR MSG FACILITY DONE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
	    << std::endl;
	...
	std::cout << __COUT_HDR_FL__
	          << "ARTDAQBUILDER SUPERVISOR INIT DONE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
	          << std::endl;
}
...
{
	std::cout << __COUT_HDR_FL__
	          << "ARTDAQBUILDER SUPERVISOR CONFIGURING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
	          << std::endl;
	std::cout << __COUT_HDR_FL__
	...
	// "daq:{event_builder:{expected_fragments_per_event:2 first_fragment_receiver_rank:0
	// fragment_receiver_count:2 mpi_buffer_count:16 print_event_store_stats:true
	// send_triggers:true trigger_port:5001 use_art:true verbose:false}
	// max_fragment_size_words:2.097152e6}
	// outputs:{netMonOutput:{module_type:\"NetMonOutput\"}}
	// physics:{my_output_modules:[\"netMonOutput\"]} process_name:\"DAQ\"
	// services:{Timing:{summaryOnly:true} scheduler:{fileMode:\"NOMERGE\"}
	// user:{NetMonTransportServiceInterface:{data_receiver_count:1
	// first_data_receiver_rank:4 max_fragment_size_words:2.097152e6 mpi_buffer_count:8
	// service_provider:\"NetMonTransportService\"}}}
	// source:{fragment_type_map:[[1,\"UDP\"]] module_type:\"RawInput\"
	// resume_after_timeout:true waiting_time:900}";

	// ONLY 1 BOARD READER IN THE SYSTEM
	// std::string configString = "daq:{event_builder:{expected_fragments_per_event:1
	// first_fragment_receiver_rank:0 fragment_receiver_count:1 mpi_buffer_count:16
	// print_event_store_stats:true send_triggers:true trigger_port:5001 use_art:true
	// verbose:false} max_fragment_size_words:2.097152e6}
	// outputs:{netMonOutput:{module_type:\"RootMPIOutput\"}} physics:{analyzers:{wf:
	// {module_type:\"WFViewer\" fragment_ids:[0] fragment_type_labels:[ \"DataGen\" ]
	// prescale:60 write_to_file:true fileName:\"/tmp/otsdaqdemo_onmon_evb.root\"}}
	// a1:[\"wf\"] my_output_modules:[\"netMonOutput\"]} process_name:\"DAQ\"
	// services:{Timing:{summaryOnly:true} scheduler:{fileMode:\"NOMERGE\"
	// errorOnFailureToPut: false}
	// user:{NetMonTransportServiceInterface:{data_receiver_count:1
	// first_data_receiver_rank:2 max_fragment_size_words:2.097152e6 mpi_buffer_count:8
	// service_provider:\"NetMonTransportService\"}}}
	// source:{fragment_type_map:[[1,\"UDP\"], [4,\"DataGen\"]] module_type:\"RawInput\"
	// resume_after_timeout:true waiting_time:900}";
	...
	__COUT__ << std::endl;
	//__COUT__ <<
	// theConfigurationManager_->getNode(XDAQContextTableName_).getNode(supervisorConfigurationPath_).getNode("ConfigurationString").getValue<std::string>()
	//<< std::endl;
	...
	std::cout << __COUT_HDR_FL__
	          << "ARTDAQBUILDER SUPERVISOR DONE CONFIGURING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
	          << std::endl;
}
...
//========================================================================================================================
void EventBuilderApp::transitionInitializing(toolbox::Event::Reference e) { init(); }
