...
TableVersion::TableVersion(const std::string& versionStr)
...
std::string TableVersion::toString(void) const
{
	...
	return (isTemporaryVersion() || isInvalid()) ? std::to_string((int)version_)
...
//==============================================================================
// operator==
bool TableVersion::operator==(unsigned int version) const
{
	return (version_ == version);
}
bool TableVersion::operator==(const TableVersion& version) const
{
...
bool TableVersion::isTemporaryVersion() const
{
...
bool TableVersion::isScratchVersion() const
{
...
TableVersion TableVersion::getNextVersion(const TableVersion& version)
{
	...
	return (!retVersion.isInvalid() && !retVersion.isTemporaryVersion()) ? retVersion
	                                                                     : TableVersion();
}
...
{
	TableVersion retVersion(version.isTemporaryVersion() ? (version.version_ - 1)
	                                                     : (INVALID - 1));
