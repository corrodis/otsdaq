...
class TableBase
{
  public:
	...
	TableVersion createTemporaryView(
	    ...
	    TableVersion destTemporaryViewVersion =
	        TableVersion::getNextTemporaryVersion());  // source of -1, from MockUp, else
	                                                   // from valid view version
  ...
  protected:
	...
	// NOTE: must be very careful to setVersion of view after manipulating (e.g. copy from
	// different version view)
