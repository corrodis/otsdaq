...
std::string TCPPacket::encode(const std::string& message)
{
	...
	std::string buffer = std::string(TCPPacket::headerLength, ' ') +
	                     message;  // THE HEADER LENGTH IS SET TO 4 = sizeof(uint32_t)
...
bool TCPPacket::decode(std::string& message)
{
	if(fBuffer.length() < headerLength)
		return false;
	uint32_t length = ntohl(reinterpret_cast<uint32_t&>(
	    fBuffer.at(0)));  // THE HEADER IS FIXED TO SIZE 4 = SIZEOF(uint32_t)
