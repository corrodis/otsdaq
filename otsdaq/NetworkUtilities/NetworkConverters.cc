...
std::string NetworkConverters::nameToStringIP(const std::string& value)
{
	...
	if(inet_aton(value.c_str(), &socketAddress.sin_addr) == 0)
		std::cout << __COUT_HDR_FL__ << "ERROR: Invalid IP address " << value
		          << std::endl;
...
{
	return ((((uint32_t)value[0]) & 0xff) << 24) + ((((uint32_t)value[1]) & 0xff) << 16) +
	       ((((uint32_t)value[2]) & 0xff) << 8) + (((uint32_t)value[3]) & 0xff);
}
