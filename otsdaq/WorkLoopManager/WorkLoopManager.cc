...
{
	__COUT__ << "Request name: " << requestName_ << " jobName: " << job_->name()
	         << std::endl;
}
...
//========================================================================================================================
HttpXmlDocument WorkLoopManager::processRequest(std::string workLoopName,
                                                const xoap::MessageReference* message)
{
	...
	__COUT__ << "Processing request! WorkLoops: " << workLoops_.size()
	         << " req: " << requestNumber << std::endl;
	if(workLoops_.size() >= maxWorkLoops)
	{
		if(!removeTimedOutRequests())
		{
			reportStream << "Too many running requests (" << workLoops_.size()
			             << "), try later!" << std::endl;
			__COUT__ << "ERROR: " << reportStream.str() << std::endl;

			xmlDocument.addTextElementToData("RequestStatus", "ERROR");
			xmlDocument.addTextElementToData("RequestName",
			                                 getWorkLoopRequest(workLoopName));
	...
	workLoops_[requestNumber].requestLastTimeChecked =
	    workLoops_[requestNumber].requestStartTime;
	...
	try
	{
		workLoops_[requestNumber].workLoop =
		    toolbox::task::getWorkLoopFactory()->getWorkLoop(workLoopName,
		                                                     cWorkLoopType_);
	}
...
//========================================================================================================================
bool WorkLoopManager::report(toolbox::task::WorkLoop* workLoop,
                             std::string              result,
                             float                    progress,
                             bool                     status)
{
	...
	/*
	    __COUT__ << "Reporting result for request " << getWorkLoopRequest(workLoop) << "
	   with req#: " << requestNumber << std::endl; if(workLoops_.find(requestNumber) ==
	   workLoops_.end())
	    {
	        __COUT__ << "This MUST NEVER happen. You must find out what is wrong!" <<
	   std::endl; assert(0);
	    }
	*/
...
bool WorkLoopManager::getRequestResult(cgicc::Cgicc& cgi, HttpXmlDocument& xmlDocument)
{
	...
	{
		reportStream << "Can't find request " << requestNumber
	...
	if(!workLoops_[requestNumber].done)
	{
		...
		xmlDocument.addTextElementToData("RequestName",
		                                 workLoops_[requestNumber].request);
		xmlDocument.addTextElementToData("RequestNumber", requestNumberString);
		xmlDocument.addTextElementToData("ErrorReport", reportStream.str());
...
bool WorkLoopManager::removeWorkLoop(RequestNumber requestNumber)
{
	if(workLoops_.find(requestNumber) == workLoops_.end())
	{
		__COUT__ << "WorkLoop " << requestNumber << " is not in the WorkLoops list!"
		         << std::endl;
		return false;
	}
	...
	{
		__COUT__ << "WorkLoop " << requestNumber << " was not created at all!"
		         << std::endl;
	...
	catch(xcept::Exception& e)
	{
		...
		// diagService_->reportError("Failed to start Job Control monitoring. Exception:
		// "+string(e.what()),DIAGWARN);
		return false;
	}
	try
	{
	...
	catch(xcept::Exception& e)
	{
		...
		// __COUT__ << "WARNING: Can't remove request WorkLoop: " << requestNumber <<
		// std::endl;
	}
	__COUT__ << "Deleting WorkLoop " << requestNumber << std::endl;
	toolbox::task::getWorkLoopFactory()->removeWorkLoop(
	    workLoops_[requestNumber].workLoopName,
	    cWorkLoopType_);  // delete workLoops_[requestNumber].workLoop; is done by the
	                      // factory
	workLoops_.erase(requestNumber);
	return true;
}
...
bool WorkLoopManager::removeTimedOutRequests(void)
{
	...
	for(; it != workLoops_.end(); it++)
		if(it->second.done &&
		   difftime(now, it->second.requestLastTimeChecked) > timeOutInSeconds)
		{
			__COUT__ << "Removing timed out request #" << it->first << " after "
			         << difftime(now, it->second.requestLastTimeChecked) << " seconds."
			         << std::endl;
			removeWorkLoop(it->first);
			return true;  // since I return I don't care if the iterator pointer is
			              // screwed after I erase the element
		}
                                                 ...
                                                 const xoap::MessageReference& message)
{
	...
	__COUT__ << "Request: " << requestName_ << " Value=" << soapCommand.getCommand()
	         << " WLName: " << name.str() << std::endl;
	if(soapCommand.hasParameters())
	{
		name << '<';
		char separator = ',';
		for(SOAPParameters::const_iterator it = soapCommand.getParameters().begin();
		    it != soapCommand.getParameters().end();
		    it++)
		{
...
//========================================================================================================================
WorkLoopManager::RequestNumber WorkLoopManager::getWorkLoopRequestNumber(
...
{
	return workLoopName.substr(workLoopName.find('-') + 1,
	                           workLoopName.find(std::string("/") + cWorkLoopType_) -
	                               workLoopName.find('-') - 1);
}

//========================================================================================================================
void WorkLoopManager::translateWorkLoopName(toolbox::task::WorkLoop* workLoop,
                                            SOAPCommand&             soapCommand)
{
	...
	else
	{
		...
		while((commaPosition = request.find(',', begin)) != std::string::npos)
		{
			...
			soapCommand.setParameter(
			    request.substr(begin, orPosition - begin),
			    request.substr(orPosition + 1, commaPosition - orPosition - 1));
		...
		soapCommand.setParameter(
		    request.substr(begin, orPosition - begin),
		    request.substr(orPosition + 1, gtPosition - orPosition - 1));
	}
	__COUT__ << soapCommand << std::endl;
	//__COUT__ << name.substr(name.find(',')+1,name.find('/')-name.find(',')-1) <<
	// std::endl;
}
