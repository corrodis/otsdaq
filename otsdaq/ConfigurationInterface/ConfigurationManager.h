...
class ConfigurationManager
{
  ...
  public:
	...
	template<class T>
	const T* 							getTable					(std::string name) const { const T* retPtr = dynamic_cast<const T*>(getTableByName(name)); if(retPtr == nullptr) { __SS__ << "Illegal cast of '" << name << "' to type " << StringMacros::getTypeName<T>() << __E__; __SS_THROW__ } return retPtr;}
