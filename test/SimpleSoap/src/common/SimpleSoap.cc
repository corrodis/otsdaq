                                     ...
                                     xgi::Output           * out ) throw (xgi::exception::Exception)
{
    ...
    *out<<"    <p align=\"left\"><input type=\"submit\" name=\"StateInput\" value=\"PushStart\"/></p>"                              <<std::endl;
    *out<<"  </form>"  <<std::endl;
    *out<<"  <form name=\"input\" method=\"get\" action=\""<<url<<"/Start"<<"\" enctype=\"multipart/form-data\">"  <<std::endl;
    *out<<"    <p align=\"left\"><input type=\"submit\" name=\"StateInput\" value=\"Start\"/></p>"                                  <<std::endl;
    *out<<"  </form>"  <<std::endl;
    *out<<"</body>"<<std::endl;
}
...
void SimpleSoap::StateMachineXgiHandler(xgi::Input * in, xgi::Output * out ) throw (xgi::exception::Exception)
{
    ...
    {
        //std::set<xdaq::ApplicationDescriptor*> set_SimpleSoap = getApplicationContext()->getDefaultZone()->getApplicationGroup("rivera")->getApplicationDescriptors("SimpleSoap::SimpleSoap");
        std::set<xdaq::ApplicationDescriptor*> set_SimpleSoap = getApplicationContext()->getDefaultZone()->getApplicationGroup("daq")->getApplicationDescriptors("SimpleSoap::SimpleSoap");

        for (std::set<xdaq::ApplicationDescriptor*>::iterator i_set_SimpleSoap=set_SimpleSoap.begin(); i_set_SimpleSoap!=set_SimpleSoap.end(); ++i_set_SimpleSoap)
        {
            try
            {
