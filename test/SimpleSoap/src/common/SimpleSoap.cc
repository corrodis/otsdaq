...
void SimpleSoap::StateMachineXgiHandler(xgi::Input* in, xgi::Output* out) {
  ...
  if (Command == "PushStart") {
    ...
    std::set<xdaq::ApplicationDescriptor*> set_SimpleSoap =
        getApplicationContext()->getDefaultZone()->getApplicationGroup("daq")->getApplicationDescriptors(
            "SimpleSoap::SimpleSoap");

    for (std::set<xdaq::ApplicationDescriptor*>::iterator i_set_SimpleSoap = set_SimpleSoap.begin();
         i_set_SimpleSoap != set_SimpleSoap.end(); ++i_set_SimpleSoap) {
      try {
        ...
        if (sReply == "StartDone")
          ...
          std::cout << __COUT_HDR_FL__ << "All underlying Supervisors could not be started by browser button!"
                    << std::endl
                    << std::endl;
...
void SimpleSoap::stateInitial(toolbox::fsm::FiniteStateMachine& fsm) {
  ...
  // diagService_->reportError("PixelSupervisor::stateInitial: workloop active: "+stringF(calibWorkloop_->isActive())+",
