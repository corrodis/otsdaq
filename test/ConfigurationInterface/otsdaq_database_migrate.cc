...
void readxml_writedb_configurations() {
  ...
  if (getenv("USER_DATA") == NULL)
    std::cout << __COUT_HDR_FL__ << "Missing env variable: USER_DATA. It must be set!" << std::endl;
  ...
  for (unsigned int i = 0; i < configTables.size(); ++i) {
    ...
    try {
      ...
      std::cout << __COUT_HDR_FL__ << "Caught exception, so skip. (likely not a defined configuration class) "
                << std::endl;
    ...
    for (auto currVersion : version) {
      ...
      try {
        ...
        std::cout << __COUT_HDR_FL__ << std::endl << e.what() << std::endl;
        std::cout << __COUT_HDR_FL__ << "Caught exception for version, so skip. (likely invalid column names) "
                  << std::endl;
  ...
  std::cout << __COUT_HDR_FL__ << "\n\nList of failed configs:versions (size=" << failedConfigVersions.size()
            << std::endl;
  for (auto &f : failedConfigVersions) std::cout << __COUT_HDR_FL__ << f << std::endl;
