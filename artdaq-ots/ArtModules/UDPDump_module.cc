...
void ots::UDPDump::analyze(art::Event const& evt) {
  ...
  if (raw.isValid()) {
    ...
    std::cout << __COUT_HDR_FL__ << std::dec << "Run " << evt.run() << ", subrun " << evt.subRun() << ", event "
              << eventNumber << " has " << raw->size() << " fragment(s) of type " << frag_type_ << std::endl;
    ...
    for (size_t idx = 0; idx < raw->size(); ++idx) {
      ...
      if (frag.hasMetadata()) {
        ...
        std::cout << __COUT_HDR_FL__ << "Board port number = " << ((int)md->port) << ", address = " << std::string(buf)
                  << std::endl;
      }
