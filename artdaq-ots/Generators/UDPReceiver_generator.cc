...
ots::UDPReceiver::UDPReceiver(fhicl::ParameterSet const &ps)
  ...
  /*if(fcntl(datasocket_, F_SETFL, O_NONBLOCK) == -1) {
          ...
          throw art::Exception(art::errors::Configuration) << "UDPReceiver: Cannot set socket to nonblocking!" <<
    TLOG_ENDL;
    }*/
  ...
  while (!should_stop()) {
    ...
    if (rv > 0) {
      ...
      if (ufds[0].revents == POLLIN || ufds[0].revents == POLLPRI) {
        ...
        TLOG_TRACE("UDPReceiver") << "Received UDP Datagram with sequence number " << std::hex << "0x"
                                  << static_cast<int>(peekBuffer[1]) << "!" << std::dec << TLOG_ENDL;
        TLOG_TRACE("UDPReceiver") << "peekBuffer[1] == expectedPacketNumber_: " << std::hex
                                  << static_cast<int>(peekBuffer[1]) << " =?= " << (int)expectedPacketNumber_
                                  << TLOG_ENDL;
        TLOG_TRACE("UDPReceiver") << "peekBuffer: 0: " << std::hex << static_cast<int>(peekBuffer[0])
                                  << ", 1: " << std::hex << static_cast<int>(peekBuffer[1]) << ", 2: " << std::hex
                                  << static_cast<int>(peekBuffer[2]) << ", 3: " << std::hex
                                  << static_cast<int>(peekBuffer[3]) << TLOG_ENDL;
        ...
        if (seqNum >= expectedPacketNumber_ || (seqNum < 64 && expectedPacketNumber_ > 192)) {
          if (seqNum != expectedPacketNumber_) {
            ...
            TLOG_WARNING("UDPReceiver") << std::dec << "Sequence Number different than expected! (delta: " << delta
                                        << ")" << TLOG_ENDL;
          ...
          int sts =
              recvfrom(datasocket_, &receiveBuffer[0], receiveBuffer.size(), 0, (struct sockaddr *)&si_data_, &dataSz);
          if (sts == -1) {
          ...
          TLOG_TRACE("UDPReceiver") << "Now placing UDP datagram with sequence number " << std::hex << (int)seqNum
          ...
          TLOG_WARNING("UDPReceiver") << "Received out-of-order datagram: " << seqNum << " != " << expectedPacketNumber_
                                      << " (expected)" << TLOG_ENDL;
          packetBuffer_t receiveBuffer;
          receiveBuffer.resize(1500);
          int sts =
...
bool ots::UDPReceiver::getNext_(artdaq::FragmentPtrs &output) {
  ...
  if (packetBuffers_.size() > 0) {
    ...
    TLOG_TRACE("UDPReceiver") << "Calling ProcessData, packetBuffers_.size() == "
                              << std::to_string(packetBuffers_.size()) << ", sz = " << std::to_string(packetBufferSize)
                              << TLOG_ENDL;
    ProcessData_(output);
...
void ots::UDPReceiver::ProcessData_(artdaq::FragmentPtrs &output) {
  ...
  output.emplace_back(artdaq::Fragment::FragmentBytes(initial_payload_size, ev_counter(), fragment_id(),
  ...
  TLOG_TRACE("UDPReceiver") << "Received data, now placing data with UDP sequence number " << std::hex
                            << static_cast<int>((packetBuffers_.front()).at(1)) << " into UDPFragment" << TLOG_ENDL;
