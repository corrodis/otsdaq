...
void ImportSystemAliasConfigurationGroups (int argc, char* argv[])
{
	...
	std::cout << "\n\nusage: Two arguments:\n\n\t otsdaq_import_system_aliases <path_to_import_database_folder> <path_to_active_groups_file> <import_prepend_base_name (optional)> \n\n"
	          ...
	          << "\n\n"
	          << "\t\tfor example:\n\n"
	          << "\t\t\totsdaq_import_system_aliases ~/databaseToImport/filesystemdb/test_db ~/UserDataToImport/ServiceData/ActiveConfigurationGroups.cfg" << __E__;
	...
	std::map<std::string /*table name*/,
	         std::map<std::string /*version alias*/,
	                  ConfigurationVersion /*aliased version*/>>
	    existingTableAliases =
	...
	try
	{
		...
		{
			__SS__ << "\nActive version of " << versionAliasesTableName << " missing! " << versionAliasesTableName << " is a required member of the Backbone configuration group."
			       << "\n\nLikely you need to activate a valid Backbone group." << std::endl;
			__SS_THROW__;
		}
		...
		try
		{
			...
			for (auto& memberPair : memberMap)
			{
				...
				if (newTableVersionMap.find (std::pair<std::string, ConfigurationVersion> (
				        memberPair.first,
				        memberPair.second)) != newTableVersionMap.end ())
				{
	...
	try
	{
		...
		if (activeMap.find (groupAliasesTableName) != activeMap.end ())
		{
			...
			for (auto& aliasPair : originalGroupAliasMap)
			{
				...
				{
					__COUT__ << "Error! Could not find the new entry for the original group " << aliasPair.second.first << "(" << aliasPair.second.second << ")" << __E__;
					continue;
				}
		...
		if (activeMap.find (versionAliasesTableName) != activeMap.end ())
		{
			...
			for (auto& aliasPair : originalTableAliasMap)
			{
				...
				{
					__COUT__ << "Error! Could not find the new entry for the original table " << aliasPair.second.first << "(" << aliasPair.second.second << ")" << __E__;
					continue;
				}
				row = cfgView->addRow ("import_aliases", true /*incrementUniqueData*/);
				cfgView->setValue (
		...
		__COUT__ << "Backing up '" << ConfigurationManager::ACTIVE_GROUP_FILENAME << "' to ... '" << renameFile << "'" << std::endl;
