...
void ImportSystemAliasConfigurationGroups(int argc, char* argv[]) {
  ...
  std::cout << "\n\nusage: Two arguments:\n\n\t otsdaq_import_system_aliases <path_to_import_database_folder> "
               "<path_to_active_groups_file> <import_prepend_base_name (optional)> \n\n"
            << "\t\t Default values: \n\t\t\timport_prepend_base_name = \"" << prependBaseName << "\" "
            << "\n\n"
            << "\t\tfor example:\n\n"
            << "\t\t\totsdaq_import_system_aliases ~/databaseToImport/filesystemdb/test_db "
  ...
  // remove trailing /'s
  while (strlen(argv[1]) && argv[1][strlen(argv[1]) - 1] == '/') argv[1][strlen(argv[1]) - 1] = '\0';
  ...
  if (argc >= 3) pathToImportActiveGroups = argv[2];
  if (argc >= 4) prependBaseName = argv[3];
  ...
  //			. check map of original tableName & tableVersion to new tableName & tableVersion, to prevent making a
  //new table 			. if needed, save active tables as next table version (using interface)
  ...
  //	ConfigurationVersion /*next persisten version*/>
  //tableNextVersionMap;

  std::map<std::string /*importGroupAlias*/,
           /*original*/ std::pair<std::string /*groupName*/, ConfigurationGroupKey>>
      originalGroupAliasMap;

  std::map</*original*/ std::pair<std::string /*groupName*/, ConfigurationGroupKey>,
           /*new*/ ConfigurationGroupKey>
      groupSet;

  std::map<std::string /*importTableAlias*/,
           /*original*/ std::pair<std::string /*tableName*/, ConfigurationVersion>>
  ...
  std::map</*original*/ std::pair<std::string, ConfigurationGroupKey>, std::string /*error string*/> groupErrors;
  ...
  std::map<std::string /*alias*/, std::pair<std::string /*group name*/, ConfigurationGroupKey>> existingGroupAliases =
      cfgMgr->getActiveGroupAliases();

  //	//NOT NEEDED -- just ask db on fly for next version //	- fill map of table name to next persistent table
  //version
  //	{
  ...
  try {
    ...
    for (const auto& activeGroup : activeGroupsMap) {
      ...
      groupSet.insert(std::pair<std::pair<std::string, ConfigurationGroupKey>, ConfigurationGroupKey>(
          std::pair<std::string, ConfigurationGroupKey>(activeGroup.second.first, activeGroup.second.second),
    ...
    for (auto& aliasPair : aliasNodePairs) {
      ...
      //			. check basename+alias for collision with existing group aliases and throw error if
      //collision
      ...
      originalGroupAliasMap[aliasName] = std::pair<std::string, ConfigurationGroupKey>(
          aliasPair.second.getNode("GroupName").getValueAsString(),
          ConfigurationGroupKey(aliasPair.second.getNode("GroupKey").getValueAsString()));
    ...
    for (auto& aliasPair : aliasNodePairs) {
      ...
      //			. check basename+alias for collision with existing table aliases and throw error if
      //collision
      ...
      originalTableAliasMap[aliasName] = std::pair<std::string, ConfigurationVersion>(
  ...
  for (auto& group : groupSet) __COUT__ << "\t" << group.first.first << " " << group.first.second << std::endl;
  __COUT__ << std::endl;
  __COUT__ << std::endl;

  __COUT__ << "Identified group aliases:" << std::endl;
  for (auto& groupAlias : originalGroupAliasMap)
    __COUT__ << "\t" << groupAlias.first << " ==> " << groupAlias.second.first << "-" << groupAlias.second.second
             << std::endl;
  __COUT__ << std::endl;
  __COUT__ << std::endl;
  ...
  ConfigurationInterface* theInterface_ =
      ConfigurationInterface::getInstance(false);  // true for File interface, false for artdaq database;
  ...
  for (auto& groupPair : groupSet) {
    ...
    try {
      cfgMgr->loadConfigurationGroup(groupPair.first.first, groupPair.first.second, true /*doActivate*/,
                                     &memberMap /*memberMap*/, 0 /*progressBar*/, &accumulateErrors, &groupComment,
                                     &groupAuthor, &groupCreateTime, false /*doNotLoadMember*/, 0 /*groupTypeString*/,
                                     &groupAliases);
    } catch (std::runtime_error& e) {
      __COUT__ << "Error was caught loading members for " << groupPair.first.first << "(" << groupPair.first.second
               << ")" << std::endl;
    ...
    if (errDetected) {
      ...
      groupErrors.insert(std::pair<std::pair<std::string, ConfigurationGroupKey>, std::string>(
          std::pair<std::string, ConfigurationGroupKey>(groupPair.first.first, groupPair.first.second),
    ...
    try {
      ...
      for (auto& memberPair : memberMap) {
        ...
        if (newTableVersionMap.find(std::pair<std::string, ConfigurationVersion>(
          ...
          memberPair.second =
              newTableVersionMap[std::pair<std::string, ConfigurationVersion>(memberPair.first, memberPair.second)];
        ...
        // FIXME -- RAR note: I do not understand why this was important.. seems like it will not help since the cache
        // is destroyed for each group  cfgView->setVersion(memberPair.second); //IMPORTANT

        // save new version to modifiedTables
        newTableVersionMap.insert(std::pair<std::pair<std::string, ConfigurationVersion>, ConfigurationVersion>(
            std::pair<std::string, ConfigurationVersion>(memberPair.first, memberPair.second), newVersion));
      ...
      while (groupMetadataTable->getViewP()->getNumberOfRows() > 1) groupMetadataTable->getViewP()->deleteRow(0);
      if (groupMetadataTable->getViewP()->getNumberOfRows() == 0) groupMetadataTable->getViewP()->addRow();

      // columns are uid,comment,author,time
      // ConfigurationManager::METADATA_COL_ALIASES TODO
      groupMetadataTable->getViewP()->setValue(
          StringMacros::mapToString(groupAliases, "," /*primary delimiter*/, ":" /*secondary delimeter*/), 0,
      ...
      newVersion = ConfigurationVersion::getNextVersion(theInterface_->findLatestVersion(groupMetadataTable));
      __COUTV__(newVersion);
      ...
      memberMap[groupMetadataTable->getConfigurationName()] = groupMetadataTable->getViewVersion();

      // memberMap should now consist of members with new flat version, so save group
      newKey = ConfigurationGroupKey::getNextKey(theInterface_->findLatestGroupKey(groupPair.first.first));

      __COUTV__(newKey);

      // memberMap should now consist of members with new flat version, so save
      theInterface_->saveConfigurationGroup(memberMap,
                                            ConfigurationGroupKey::getFullGroupString(groupPair.first.first, newKey));

      // and modify groupSet and activeGroupKeys keys
      groupPair.second = newKey;
    } catch (std::runtime_error& e) {
      __COUT__ << "Error was caught saving group " << groupPair.first.first << " (" << groupPair.first.second << ") "
               << std::endl;
      __COUT__ << e.what() << std::endl;
  ...
  for (auto& tablePair : newTableVersionMap)
    __COUT__ << "\t" << tablePair.first.first << "-v" << tablePair.first.second << " ==> " << tablePair.second
             << std::endl;
  __COUT__ << std::endl;
  __COUT__ << std::endl;
  ...
  __COUT__ << "Modifying the active Backbone table to reflect new table versions and group keys." << std::endl;

  try {
    // modify Group Aliases Configuration and Version Aliases Configuration to
    //	include new groups and tables

    std::string activeBackboneGroupName = cfgMgr->getActiveGroupName(ConfigurationManager::ACTIVE_GROUP_NAME_BACKBONE);
    cfgMgr->loadConfigurationGroup(activeBackboneGroupName,
                                   cfgMgr->getActiveGroupKey(ConfigurationManager::ACTIVE_GROUP_NAME_BACKBONE), true,
                                   &memberMap, 0, &accumulateErrors);
    ...
    if (activeMap.find(groupAliasesTableName) != activeMap.end()) {
      ...
      for (auto& aliasPair : originalGroupAliasMap) {
        auto groupIt = groupSet.find(
            std::pair<std::string, ConfigurationGroupKey>(aliasPair.second.first, aliasPair.second.second));

        if (groupIt == groupSet.end()) {
          __COUT__ << "Error! Could not find the new entry for the original group " << aliasPair.second.first << "("
                   << aliasPair.second.second << ")" << __E__;
          continue;
        }
    ...
    if (activeMap.find(versionAliasesTableName) != activeMap.end()) {
      ...
      for (auto& aliasPair : originalTableAliasMap) {
        auto tableIt = newTableVersionMap.find(
            std::pair<std::string, ConfigurationVersion>(aliasPair.second.first, aliasPair.second.second));

        if (tableIt == newTableVersionMap.end()) {
          __COUT__ << "Error! Could not find the new entry for the original table " << aliasPair.second.first << "("
                   << aliasPair.second.second << ")" << __E__;
          continue;
        }
        row = cfgView->addRow("import_aliases", true /*incrementUniqueData*/);
    ...
    newKey = ConfigurationGroupKey::getNextKey(theInterface_->findLatestGroupKey(activeBackboneGroupName));

    __COUTV__(newKey);

    // memberMap should now consist of members with new flat version, so save
    theInterface_->saveConfigurationGroup(memberMap,
                                          ConfigurationGroupKey::getFullGroupString(activeBackboneGroupName, newKey));

    std::string renameFile = ConfigurationManager::ACTIVE_GROUP_FILENAME + "." + nowTime;
    rename(ConfigurationManager::ACTIVE_GROUP_FILENAME.c_str(), renameFile.c_str());

    __COUT__ << "Backing up '" << ConfigurationManager::ACTIVE_GROUP_FILENAME << "' to ... '" << renameFile << "'"
             << std::endl;
...
CLEAN_UP:
  ...
  __COUT__ << "****************************" << std::endl;
  __COUT__ << "There were " << groupSet.size() << " groups considered, and there were " << groupErrors.size()
