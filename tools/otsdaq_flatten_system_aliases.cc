...
void FlattenActiveSystemAliasConfigurationGroups(int argc, char *argv[])
{
	...
	{
		__COUT__ << "\n\nflatVersion " << ConfigurationVersion(flatVersion) << " is an invalid or temporary version. Skipping to end!" << std::endl;
		goto CLEAN_UP;
	}
	...
	std::cout << "\n\n"
	          << __COUT_HDR_FL__ << "Modifying the active Backbone table to reflect new table versions and group keys." << std::endl;
		...
		if (activeMap.find(versionAliasesName) != activeMap.end())
		{
			...
			for (unsigned int row = 0; row < cfgView->getNumberOfRows(); ++row)
			{
				...
				for (const auto &table : modifiedTables)
					...
					{
						//found a matching group/key pair
						__COUT__ << "Changing row " << row << " for " << cfgView->getDataView()[row][col1] << " version=" << cfgView->getDataView()[row][col2] << " to NEW version=" << table.second << std::endl;
...

	{
		...
		{
			__COUT__ << "Updating persistent active groups to " << ConfigurationManager::ACTIVE_GROUP_FILENAME << " ..." << std::endl;
...
CLEAN_UP:
	...
	__COUT__ << "****************************" << std::endl;
	__COUT__ << "There were " << groupSet.size() << " groups considered, and there were " << groupErrors.size() << " errors found handling those groups." << std::endl;
