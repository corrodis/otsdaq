...
void FlattenActiveSystemAliasConfigurationGroups(int argc, char *argv[]) {
  ...
  if (argc >= 2) sscanf(argv[1], "%d", &flatVersion);
  if (argc >= 3) pathToSwapIn = argv[2];
  ...
  for (const auto &activeGroup : activeGroupsMap) {
    ...
    if (activeGroup.first == ConfigurationManager::ACTIVE_GROUP_NAME_BACKBONE) {
      ...
      std::cout << __COUT_HDR_FL__ << "found activeBackboneGroupName = " << activeBackboneGroupName << std::endl;
    } else if (activeGroup.first == ConfigurationManager::ACTIVE_GROUP_NAME_CONTEXT) {
      activeContextGroupName = activeGroup.second.first;
      std::cout << __COUT_HDR_FL__ << "found activeContextGroupName = " << activeContextGroupName << std::endl;
    } else if (activeGroup.first == ConfigurationManager::ACTIVE_GROUP_NAME_ITERATE) {
      activeIterateGroupName = activeGroup.second.first;
      std::cout << __COUT_HDR_FL__ << "found activeIterateGroupName = " << activeIterateGroupName << std::endl;
      ...
      std::cout << __COUT_HDR_FL__ << "found activeConfigGroupName = " << activeConfigGroupName << std::endl;
    }
  }
  ...
  if (pathToSwapIn != "") {
    ...
    if (fp) {
      __COUT__ << "Swapping active groups file: \t" << activeGroupsFile << std::endl;
      __COUT__ << "\t.. to: \t\t" << ConfigurationManager::ACTIVE_GROUP_FILENAME << std::endl;
  ...
  if (ConfigurationVersion(flatVersion).isInvalid() || ConfigurationVersion(flatVersion).isTemporaryVersion()) {
    ...
    try {
      ...
      for (auto &memberPair : memberMap) {
        ...
        modifiedTables.insert(std::pair<std::pair<std::string, ConfigurationVersion>, ConfigurationVersion>(
            std::pair<std::string, ConfigurationVersion>(memberPair.first, memberPair.second),
      ...
      theInterface_->saveConfigurationGroup(memberMap, ConfigurationGroupKey::getFullGroupString(
                                                           groupPair.first.first, ConfigurationGroupKey(flatVersion)));
      ...
      if (activeGroupKeys.find(groupPair.first.first) != activeGroupKeys.end() &&
        ...
        activeGroupKeys[groupPair.first.first].second = ConfigurationGroupKey(flatVersion);
    } catch (std::runtime_error &e) {
  ...
  std::cout << "\n\n"
            << __COUT_HDR_FL__ << "Modifying the active Backbone table to reflect new table versions and group keys."
            << std::endl;

  {
    cfgMgr->loadConfigurationGroup(activeBackboneGroupName, activeGroupKeys[activeBackboneGroupName].second, true,
        ...
        for (const auto &group : groupSet)
          if (group.second.isInvalid())
            ...
            __COUT__ << "Changing row " << row << " for " << cfgView->getDataView()[row][col1]
                     << " key=" << cfgView->getDataView()[row][col2] << " to NEW key=" << group.second << std::endl;
      ...
      for (unsigned int row = 0; row < cfgView->getNumberOfRows(); ++row) {
        ...
        for (const auto &table : modifiedTables)
          if (cfgView->getDataView()[row][col1] == table.first.first &&
            ...
            // found a matching group/key pair
            __COUT__ << "Changing row " << row << " for " << cfgView->getDataView()[row][col1]
                     << " version=" << cfgView->getDataView()[row][col2] << " to NEW version=" << table.second
                     << std::endl;
    ...
    __COUT__ << "\t to...\t" << groupAliasesName << ":v" << memberMap[groupAliasesName] << std::endl;

    __COUT__ << versionAliasesName << ":v" << memberMap[versionAliasesName] << std::endl;
    // change the version of the active view to flatVersion and save it
    config = cfgMgr->getConfigurationByName(versionAliasesName);
    cfgView = config->getViewP();
    cfgView->setVersion(ConfigurationVersion(flatVersion));
    ...
    __COUT__ << "\t to...\t" << versionAliasesName << ":v" << memberMap[versionAliasesName] << std::endl;

    // memberMap should now consist of members with new flat version, so save
    theInterface_->saveConfigurationGroup(memberMap, ConfigurationGroupKey::getFullGroupString(
                                                         activeBackboneGroupName, ConfigurationGroupKey(flatVersion)));

    activeGroupKeys[activeBackboneGroupName].second = ConfigurationGroupKey(flatVersion);
...

  {
    ...
    __COUT__ << "Backing up file: " << ConfigurationManager::ACTIVE_GROUP_FILENAME << std::endl;
  ...
  __COUT__ << activeBackboneGroupName << " is the " << ConfigurationManager::ACTIVE_GROUP_NAME_BACKBONE << "."
           << std::endl;
  std::cout << "\n\n" << __COUT_HDR_FL__ << "Resulting Active Groups end." << std::endl;
