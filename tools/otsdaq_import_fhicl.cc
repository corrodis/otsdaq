...
void ImportSystemAliasTableGroups(fhicl::ParameterSet pset)
{
	...
	try
	{
		...
		cfgMgr->loadTableGroup(
		    activeBackboneGroupName, cfgMgr->getActiveGroupKey(ConfigurationManager::ACTIVE_GROUP_NAME_BACKBONE), true, &memberMap, 0, &accumulateErrors);
...
{
	auto pset = artdaq::LoadParameterSet(argc, argv, "otsdaq_import_fhicl", "Import a ParameterSet to the otsdaq database");
