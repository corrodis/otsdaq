...
void FixNewTableFields(int argc, char *argv[]) {
  ...
  for (int i = 0; i < argc; i++) std::cout << "argv[" << i << "] = " << argv[i] << __E__;
  ...
  std::map<std::pair<std::string, ConfigurationGroupKey>, ConfigurationGroupKey> groupSet;
  /* <tableName, <origVersion, newVersion> >*/
  std::map<std::pair<std::string, ConfigurationVersion>, ConfigurationVersion> modifiedTables;
  std::map<std::string, std::pair<ConfigurationGroupKey, ConfigurationGroupKey>> activeGroupKeys;
  std::map<std::pair<std::string, ConfigurationGroupKey>, std::string> groupErrors;
  ...
  for (const auto &activeGroup : activeGroupsMap) {
    ...
    activeGroupKeys.insert(std::pair<std::string, std::pair<ConfigurationGroupKey, ConfigurationGroupKey>>(
        activeGroup.second.first,
        std::pair<ConfigurationGroupKey, ConfigurationGroupKey>(activeGroup.second.second, ConfigurationGroupKey())));
  ...
  for (auto &group : groupSet) __COUT__ << group.first.first << " " << group.first.second << __E__;
  __COUT__ << __E__;
  __COUT__ << __E__;
  ...
  if (pathToSwapIn != "")  // move the swap in directory in
  {
    ...
    if (fp) {
      ...
      __COUT__ << "\t.. to: \t\t" << ConfigurationManager::ACTIVE_GROUP_FILENAME << __E__;
      rename(activeGroupsFile.c_str(), ConfigurationManager::ACTIVE_GROUP_FILENAME.c_str());
    }
  ...
  // ConfigurationInterface* theInterface_ = ConfigurationInterface::getInstance(false); //true for File interface,
  // false for artdaq database;
  ...
  for (auto &groupPair : groupSet) {
    ...
    __COUT__ << "****************************" << __E__;
    __COUT__ << "Loading members for " << groupPair.first.first << "(" << groupPair.first.second << ")" << __E__;

    //=========================
    // load group, group metadata, and tables from original DB
    try {
      cfgMgr->loadConfigurationGroup(groupPair.first.first, groupPair.first.second, false /*doActivate*/,
                                     &memberMap /*memberMap*/, 0 /*progressBar*/, &accumulateErrors, &groupComment,
                                     0,  //&groupAuthor,
                                     0,  //&groupCreateTime,
                                     false /*doNotLoadMember*/, 0 /*groupTypeString*/, &groupAliases);
    ...
    try {
      __COUT__ << "Before member map: " << StringMacros::mapToString(memberMap) << __E__;
      ...
      for (auto &memberPair : memberMap) {
        ...
        if (modifiedTables.find(std::pair<std::string, ConfigurationVersion>(memberPair.first, memberPair.second)) !=
            modifiedTables.end()) {
          __COUT__ << "Table was already modified!" << __E__;
          memberPair.second =
              modifiedTables[std::pair<std::string, ConfigurationVersion>(memberPair.first, memberPair.second)];
          __COUT__ << "\t to...\t" << memberPair.first << ":v" << memberPair.second << __E__;
          continue;
        }
        ...
        ConfigurationVersion temporaryVersion =
            cfgMgr->copyViewToCurrentColumns(memberPair.first /*table name*/, memberPair.second /*source version*/
            );

        // then save temporary to persistent version
        ConfigurationVersion persistentVersion =
            cfgMgr->saveNewConfiguration(memberPair.first /*table name*/, temporaryVersion);
        ...
        // save new version to modifiedTables
        modifiedTables.insert(std::pair<std::pair<std::string, ConfigurationVersion>, ConfigurationVersion>(
            std::pair<std::string, ConfigurationVersion>(memberPair.first, memberPair.second), persistentVersion));
      ...
      ConfigurationGroupKey newGroupKey = cfgMgr->saveNewConfigurationGroup(groupPair.first.first /*groupName*/,
                                                                            memberMap, groupComment, &groupAliases);
      ...
      if (activeGroupKeys.find(groupPair.first.first) != activeGroupKeys.end() &&
        ...
        activeGroupKeys[groupPair.first.first].second = ConfigurationGroupKey(newGroupKey);
    } catch (std::runtime_error &e) {
  ...
  __COUT__ << "Modifying the active Backbone table to reflect new table versions and group keys." << __E__;

  {  // start active backbone group handling

    cfgMgr->loadConfigurationGroup(activeBackboneGroupName, activeGroupKeys[activeBackboneGroupName].second,
                                   true /*doActivate*/, &memberMap, 0 /*progressBar*/, &accumulateErrors,
                                   &groupComment);
    ...
    if (activeMap.find(groupAliasesName) != activeMap.end()) {
      ...
      // first copy to new column names
      ConfigurationVersion temporaryVersion = cfgMgr->copyViewToCurrentColumns(
          groupAliasesName /*table name*/, memberMap[groupAliasesName] /*source version*/
      );
      ...
      for (unsigned int row = 0; row < cfgView->getNumberOfRows(); ++row) {
        ...
        for (const auto &group : groupSet)
          if (group.second.isInvalid())
            ...
            __COUT__ << "Changing row " << row << " for " << cfgView->getDataView()[row][col1]
                     << " key=" << cfgView->getDataView()[row][col2] << " to NEW key=" << group.second << __E__;
      ...
      // then save temporary to persistent version
      ConfigurationVersion persistentVersion =
          cfgMgr->saveNewConfiguration(groupAliasesName /*table name*/, temporaryVersion);
      ...
      __COUT__ << "\t to...\t" << groupAliasesName << ":v" << memberMap[groupAliasesName] << __E__;
    ...
    if (activeMap.find(versionAliasesName) != activeMap.end()) {
      ...
      __COUT__ << versionAliasesName << ":v" << memberMap[versionAliasesName] << __E__;

      // first copy to new column names
      ConfigurationVersion temporaryVersion = cfgMgr->copyViewToCurrentColumns(
          versionAliasesName /*table name*/, memberMap[versionAliasesName] /*source version*/
      );
      ...
      for (unsigned int row = 0; row < cfgView->getNumberOfRows(); ++row) {
        ...
        for (const auto &table : modifiedTables)
          if (cfgView->getDataView()[row][col1] == table.first.first &&
            ...
            // found a matching group/key pair
            __COUT__ << "Changing row " << row << " for " << cfgView->getDataView()[row][col1]
                     << " version=" << cfgView->getDataView()[row][col2] << " to NEW version=" << table.second << __E__;
      ...
      // then save temporary to persistent version
      ConfigurationVersion persistentVersion =
          cfgMgr->saveNewConfiguration(versionAliasesName /*table name*/, temporaryVersion);
      ...
      __COUT__ << "\t to...\t" << versionAliasesName << ":v" << memberMap[versionAliasesName] << __E__;
    ...
    ConfigurationGroupKey newGroupKey =
        cfgMgr->saveNewConfigurationGroup(activeBackboneGroupName /*groupName*/, memberMap, groupComment,
    ...
    activeGroupKeys[activeBackboneGroupName].second = ConfigurationGroupKey(newGroupKey);
...

  {
    ...
    if (!fp) {
      __SS__ << "Original active groups file '" << ConfigurationManager::ACTIVE_GROUP_FILENAME << "' not found."
             << __E__;
      goto CLEAN_UP;
    }

    __COUT__ << "Backing up file: " << ConfigurationManager::ACTIVE_GROUP_FILENAME << __E__;
    ...
    {
      __COUT__ << "Updating persistent active groups to " << ConfigurationManager::ACTIVE_GROUP_FILENAME << " ..."
               << __E__;
  ...
  __COUT__ << "Resulting Groups:" << __E__;
  for (const auto &group : groupSet)
    __COUT__ << "\t" << group.first.first << ": " << group.first.second << " => " << group.second << __E__;
  __COUT__ << "Resulting Groups end." << __E__;
  ...
  __COUT__ << "Resulting Active Groups:" << __E__;
  for (const auto &activeGroup : activeGroupKeys)
    __COUT__ << "\t" << activeGroup.first << ": " << activeGroup.second.first << " => " << activeGroup.second.second
             << __E__;
