...
void FlattenActiveConfigurationGroups(int argc, char *argv[])
{
	...
	for (auto &activeGroupPair : activeGroupsMap)
	{
		...
		std::cout << __COUT_HDR_FL__ << "Loading members for " << activeGroupPair.first << "\t" << activeGroupPair.second.first << "(" << activeGroupPair.second.second << ")" << std::endl;
	...
	if (activeMap.find(versionAliasesName) != activeMap.end())
	{
		...
		for (unsigned int row = 0; row < cfgView->getNumberOfRows(); ++row)
			for (auto &activePair : activeMap)
				...
				{
					//found a matching group/key pair
					std::cout << __COUT_HDR_FL__ << "Changing row " << row << " for " << cfgView->getDataView()[row][col1] << " version=" << cfgView->getDataView()[row][col2] << " to flatVersion=" << ConfigurationVersion(flatVersion) << std::endl;
	...
	for (auto &activeGroupMembersPair : activeGroupMembersMap)
	{
		...
		{
			std::cout << __COUT_HDR_FL__ << "\t from...\t" << groupMemberPair.first << ":v" << groupMemberPair.second << std::endl;
			groupMemberPair.second = flatVersion;
		}

		for (auto &groupMemberPair : activeGroupMembersPair.second)
		{
			std::cout << __COUT_HDR_FL__ << "\t to...\t" << groupMemberPair.first << ":v" << groupMemberPair.second << std::endl;
		}
