...
void FlattenActiveConfigurationGroups(int argc, char *argv[]) {
  ...
  for (auto &activeGroupPair : activeGroupsMap) {
    if (activeGroupPair.second.second.isInvalid()) {
      std::cout << __COUT_HDR_FL__ << "Skipping invalid " << activeGroupPair.first << std::endl;
      continue;
    }

    std::cout << __COUT_HDR_FL__ << "Loading members for " << activeGroupPair.first << "\t"
              << activeGroupPair.second.first << "(" << activeGroupPair.second.second << ")" << std::endl;
    ...
    try {
      cfgMgr->loadConfigurationGroup(
          activeGroupPair.second.first, activeGroupPair.second.second, true /*doActivate*/,
          &activeGroupMembersMap[activeGroupPair.second.first] /*memberMap*/, 0 /*progressBar*/, &accumulateErrors,
          &activeGroupCommentMap[activeGroupPair.second.first], &activeGroupAuthorMap[activeGroupPair.second.first],
          &groupCreateTime, false /*doNotLoadMember*/, 0 /*groupTypeString*/,
    ...
    for (unsigned int row = 0; row < cfgView->getNumberOfRows(); ++row)
      for (auto &activeGroupPair : activeGroupsMap)
        if (activeGroupPair.second.second.isInvalid())
          ...
          std::cout << __COUT_HDR_FL__ << "Changing row " << row << " for " << cfgView->getDataView()[row][col1]
                    << " key=" << cfgView->getDataView()[row][col2]
                    << " to DEFAULT=" << ConfigurationGroupKey(ConfigurationGroupKey::getDefaultKey()) << std::endl;
          cfgView->setValue(ConfigurationGroupKey(ConfigurationGroupKey::getDefaultKey()).toString(), row, col2);
          break;
        }
  }
      ...
      for (auto &activePair : activeMap)
        if (cfgView->getDataView()[row][col1] == activePair.first &&
          ...
          std::cout << __COUT_HDR_FL__ << "Changing row " << row << " for " << cfgView->getDataView()[row][col1]
                    ...
                    << " to flatVersion=" << ConfigurationVersion(flatVersion) << std::endl;
  ...
  for (auto &activeGroupMembersPair : activeGroupMembersMap) {
    ...
    for (auto &groupMemberPair : activeGroupMembersPair.second) {
      std::cout << __COUT_HDR_FL__ << "\t from...\t" << groupMemberPair.first << ":v" << groupMemberPair.second
                << std::endl;
      groupMemberPair.second = flatVersion;
    }

    for (auto &groupMemberPair : activeGroupMembersPair.second) {
      std::cout << __COUT_HDR_FL__ << "\t to...\t" << groupMemberPair.first << ":v" << groupMemberPair.second
                << std::endl;
    }
    ...
    groupMetadataTable->getViewP()->setValue(
        StringMacros::mapToString(activeGroupAliasesMap[activeGroupMembersPair.first], "," /*primary delimiter*/,
                                  ":" /*secondary delimeter*/),
        0, ConfigurationManager::METADATA_COL_ALIASES);
    groupMetadataTable->getViewP()->setValue(activeGroupCommentMap[activeGroupMembersPair.first], 0,
                                             ConfigurationManager::METADATA_COL_COMMENT);
    groupMetadataTable->getViewP()->setValue(activeGroupAuthorMap[activeGroupMembersPair.first], 0,
                                             ConfigurationManager::METADATA_COL_AUTHOR);
    groupMetadataTable->getViewP()->setValue(activeGroupCreateTimeMap[activeGroupMembersPair.first], 0,
    ...
    activeGroupMembersPair.second[groupMetadataTable->getConfigurationName()] = groupMetadataTable->getViewVersion();
    ...
    theInterface_->saveConfigurationGroup(
        ...
        ConfigurationGroupKey::getFullGroupString(activeGroupMembersPair.first,
