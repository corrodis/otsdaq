...

{
  ...
  try {
    ...
    xoap::MessageReference reply =
        theApplication_->getApplicationContext()->postSOAP(message, *(theApplication_->getApplicationDescriptor()), *d);
    return reply;
  } catch (xdaq::exception::Exception& e) {
    __COUT__ << "This application failed to send a SOAP message to " << d->getClassName() << " instance "
             << d->getInstance() << " re-throwing exception = " << xcept::stdformat_exception_history(e);
    std::string mystring;
    message->writeTo(mystring);
...

{
  ...
  try {
    ...
    __COUT__ << "This application failed to send a SOAP error message to " << d->getClassName() << " instance "
             << d->getInstance() << " with command = " << cmd
