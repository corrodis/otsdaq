...
ConfigurationTree::ConfigurationTree()
    : configMgr_(0),
      ...
      configView_(0) {
  //__COUT__ << std::endl;
  //__COUT__ << "EMPTY CONSTRUCTOR ConfigManager: " << configMgr_ << " configuration: " << configuration_  << std::endl;
}
//==============================================================================
ConfigurationTree::ConfigurationTree(const ConfigurationManager* const& configMgr,
                                     const ConfigurationBase* const& config)
    : ConfigurationTree(configMgr, config, "" /*groupId_*/, 0 /*linkParentConfig_*/, "" /*linkColName_*/,
                        "" /*linkColValue_*/, ConfigurationView::INVALID /*linkBackRow_*/,
                        ConfigurationView::INVALID /*linkBackCol_*/, "" /*disconnectedTargetName_*/,
                        "" /*disconnectedLinkID_*/, "" /*childLinkIndex_*/, ConfigurationView::INVALID /*row_*/,
                        ConfigurationView::INVALID /*col_*/) {
  //__COUT__ << std::endl;
...
//==============================================================================
ConfigurationTree::ConfigurationTree(const ConfigurationManager* const& configMgr,
                                     const ConfigurationBase* const& config, const std::string& groupId,
                                     const ConfigurationBase* const& linkParentConfig, const std::string& linkColName,
                                     const std::string& linkColValue, const unsigned int linkBackRow,
                                     const unsigned int linkBackCol, const std::string& disconnectedTargetName,
                                     const std::string& disconnectedLinkID, const std::string& childLinkIndex,
    ...
    : configMgr_(configMgr),
      ...
      configView_(0) {
  //__COUT__ << std::endl;
  //__COUT__ << "FULL CONTRUCTOR ConfigManager: " << configMgr_ << " configuration: " << configuration_ << std::endl;
  if (!configMgr_)  // || !configuration_ || !configView_)
  {
    std::stringstream ss;
    ss << __COUT_HDR_FL__ << "Invalid empty pointer given to tree!\n"
       << "\n\tconfigMgr_=" << configMgr_ << "\n\tconfiguration_=" << configuration_
       << "\n\tconfigView_=" << configView_ << std::endl;
    __SS_THROW__;
  }

  if (configuration_) configView_ = &(configuration_->getView());

  // verify UID column exists
  if (configView_ && configView_->getColumnInfo(configView_->getColUID()).getType() != ViewColumnInfo::TYPE_UID) {
...
//==============================================================================
void ConfigurationTree::recursivePrint(const ConfigurationTree& t, unsigned int depth, std::ostream& out,
                                       std::string space) {
  if (t.isValueNode())
    ...
    if (t.isLinkNode()) {
      ...
      out << " (" << (t.isGroupLinkNode() ? "Group" : "U") << "ID=" << t.getValueAsString() << ") : " << std::endl;
    } else
    ...
    if (depth >= 1) {
      ...
      if (!C.empty()) out << space << "{" << std::endl;
      for (auto& c : C) recursivePrint(c.second, depth - 1, out, space + "   ");
      if (!C.empty()) out << space << "}" << std::endl;
    }
  }
}
...
//==============================================================================
std::string ConfigurationTree::handleValidateValueForColumn(const ConfigurationView* configView, std::string value,
                                                            unsigned int col, ots::identity<std::string>) const {
  if (!configView) {
...
void ConfigurationTree::getValue(std::string& value) const {
  ...
  if (row_ != ConfigurationView::INVALID && col_ != ConfigurationView::INVALID)  // this node is a value node
  {
    ...
    try {
      ...
      value = configView_->validateValueForColumn(valueAsTreeNode.getValueAsString(), col_);
    ...
    configView_->getValue(value, row_, col_);
             ...
             col_ == ConfigurationView::INVALID)  // this node is config node maybe with groupId
  {
    if (isLinkNode() && isDisconnected())
      value = (groupId_ == "") ? getValueName() : groupId_;  // a disconnected link still knows its table name or
                                                             // groupId
    else
...
//
////special version of getValue for string type
//	Note: if called without template, necessary because types of std::basic_string<char> cause compiler problems if
//no string specific function
  ...
  if (row_ != ConfigurationView::INVALID && col_ != ConfigurationView::INVALID)  // this node is a value node
  {
    ...
    // extract bit map
    {
      ...
      for (unsigned int i = 0; i < bitmapString.length(); i++) {
        ...
        {
          if (bitmapString[i] == '[') {  // open a new row
            openRow = true;
            ++row;
...
const std::string& ConfigurationTree::getConfigurationName(void) const {
  if (!configuration_) {
    ...
    if (linkParentConfig_) {
      ss << "Error occurred traversing from " << linkParentConfig_->getConfigurationName() << " row " << linkBackRow_
...
std::vector<std::string> ConfigurationTree::getFixedChoices(void) const {
  if (getValueType() != ViewColumnInfo::TYPE_FIXED_CHOICE_DATA && getValueType() != ViewColumnInfo::TYPE_BITMAP_DATA &&
      !isLinkNode()) {
    __SS__ << "Can not get fixed choices of node with value type of '" << getValueType()
           << ".' Node must be a link or a value node with type '" << ViewColumnInfo::TYPE_BITMAP_DATA << "' or '"
  ...
  if (isLinkNode()) {
    ...
    for (const auto& choice : choices) retVec.push_back(choice);
...
const std::string& ConfigurationTree::getUIDAsString(void) const {
  if (isValueNode() || isUIDLinkNode()) return configView_->getDataView()[row_][configView_->getColUID()];

  {
    __SS__ << "Can not get UID of node with type '" << getNodeType() << ".' Node type must be '"
           << ConfigurationTree::NODE_TYPE_VALUE << "' or '" << ConfigurationTree::NODE_TYPE_UID_LINK << ".'"
           << std::endl;
    __SS_ONLY_THROW__;
  }
}
...
bool ConfigurationTree::isDefaultValue(void) const {
  ...
  if (getValueDataType() == ViewColumnInfo::DATATYPE_STRING) {
    if (getValueType() == ViewColumnInfo::TYPE_ON_OFF || getValueType() == ViewColumnInfo::TYPE_TRUE_FALSE ||
...
ConfigurationTree ConfigurationTree::getNode(const std::string& nodeString, bool doNotThrowOnBrokenUIDLinks) const {
  ...
  if (nodeString.length() < 1) {
    __SS__ << "Invalid empty node name! Looking for child node from node '" << getValue() << "'..." << std::endl;
    __SS_THROW__;
  }
  ...
  try {
    ...
    if (!configuration_) {
      ...
      return recurse(configMgr_->getNode(nodeName), childPath, doNotThrowOnBrokenUIDLinks);
    } else if (row_ == ConfigurationView::INVALID && col_ == ConfigurationView::INVALID) {
      // config node

      if (!configView_) {
        __SS__
            << "Missing configView pointer! Likely attempting to access a child node through a disconnected link node."
            << std::endl;
        __SS_THROW__;
      }
      ...
      return recurse(
          ConfigurationTree(configMgr_, configuration_,
                            ...
                            (groupId_ == "") ? configView_->findRow(configView_->getColUID(), nodeName)
                                             : configView_->findRowInGroup(configView_->getColUID(), nodeName, groupId_,
                                                                           childLinkIndex_)),
      ...
      if ((isLink = configView_->getChildLink(c, isGroupLink, linkPair)) && !isGroupLink) {
        //__COUT__ << "nodeName=" << nodeName << " " << nodeName.length() << std::endl;
        ...
        try {
          ...
          //<< 							std::endl;  do not recurse further
          return ConfigurationTree(
              configMgr_, 0, "",
              configuration_,  // linkParentConfig_
              nodeName,
              configView_
                  ->getDataView()[row_][c],  // this the link node field associated value (matches targeted column)
        ...
        try {
          ...
          if (configView_->getDataView()[row_][linkPair.first] != ViewColumnInfo::DATATYPE_LINK_DEFAULT)
            __COUT_WARN__ << "Found disconnected node! Failed link target from nodeName=" << nodeName
                          << " to table:id=" << configView_->getDataView()[row_][linkPair.first] << ":"
                          << configView_->getDataView()[row_][linkPair.second] << std::endl;
        ...
        //__COUT__ << "nodeName=" << nodeName << " " << nodeName.length() << std::endl;
        // return value node
        return ConfigurationTree(configMgr_, configuration_, "", 0 /*linkParentConfig_*/, "", "",
                                 ConfigurationView::INVALID /*linkBackRow_*/,
                                 ConfigurationView::INVALID /*linkBackCol_*/, "", "" /*disconnectedLinkID*/, "", row_,
                                 c);
      }
    }

  } catch (std::runtime_error& e) {
    __SS__ << "\n\nError occurred descending from node '" << getValue() << "' in table '" << getConfigurationName()
...
ConfigurationTree ConfigurationTree::getBackNode(std::string nodeName, unsigned int backSteps) const {
  for (unsigned int i = 0; i < backSteps; i++) nodeName = nodeName.substr(0, nodeName.find_last_of('/'));
...
ConfigurationTree ConfigurationTree::getForwardNode(std::string nodeName, unsigned int forwardSteps) const {
  ...
  for (unsigned int i = 0; i < forwardSteps; i++) s = nodeName.find('/', s) + 1;
...
bool ConfigurationTree::isValueBoolType(void) const {
  return isValueNode() && configView_->getColumnInfo(col_).isBoolType();
}  // end isValueBoolType()
...
bool ConfigurationTree::isValueNumberDataType(void) const {
  return isValueNode() && configView_->getColumnInfo(col_).isNumberDataType();
}  // end isValueBoolType()
...
bool ConfigurationTree::isDisconnected(void) const {
  if (!isLinkNode()) {
    __SS__ << "\n\nError occurred testing link connection at node with value '" << getValue() << "' in table '"
           << getConfigurationName() << "'\n\n"
           << std::endl;
    ss << "This is not a Link node! It is node type '" << getNodeType() << ".' Only a Link node can be disconnected."
       << std::endl;
    __SS_ONLY_THROW__;
  }
...
bool ConfigurationTree::isUIDLinkNode(void) const { return (isLinkNode() && groupId_ == ""); }
...
std::vector<ConfigurationTree::RecordField> ConfigurationTree::getCommonFields(
    ...
    const std::vector<std::string /*relative-path*/>& fieldRejectList, unsigned int depth) const {
  // enforce that starting point is a table node
  ...
  //				mark <field count> as guaranteed -1 (all these fields must be common for UIDs in same
  //table) 			else not first uid record, do not need to check, must be same as first record!
  ...
  //						if isValueNode (value nodes are possible field candidates!)
  //							if first uid record
  //								add field to <field candidates list> if in <field filter
  //list> 								initial mark <field count> as 1
  //							else
  ...
  //							if Link Table/UID pair is not found in <field candidates list> (avoid endless loops
  //through tree) 								recursiveGetCommonFields()
  //				=====================
  //
  //
  ...
  for (unsigned int i = 0; i < recordList.size(); ++i) {
    ...
    for (const auto& fieldNode : recordChildren) {
      ...
      if (fieldNode.second.isValueNode()) {
        ...
        if (fieldNode.second.getColumnInfo().getType() == ViewColumnInfo::TYPE_AUTHOR ||
            fieldNode.second.getColumnInfo().getType() == ViewColumnInfo::TYPE_TIMESTAMP)
          continue;
        ...
        if (!i)  // first uid record
        {
          ...
          if (found) {
            fieldCandidateList.push_back(ConfigurationTree::RecordField(fieldNode.second.getConfigurationName(),
                                                                        recordList[i], fieldNode.first,
      ...
      } else if (depth > 0 && fieldNode.second.isUIDLinkNode() && !fieldNode.second.isDisconnected()) {
        //__COUT__ << "isUIDLinkNode " << fieldNode.first << std::endl;
        fieldNode.second.recursiveGetCommonFields(fieldCandidateList, fieldCount, fieldAcceptList, fieldRejectList,
                                                  depth,
  ...
  for (unsigned int i = 0; i < fieldCandidateList.size(); ++i) {
    ...
    if (fieldCount[i] != -1 && fieldCount[i] != (int)recordList.size()) {
...
std::set<std::string /*unique-value*/> ConfigurationTree::getUniqueValuesForField(
    const std::vector<std::string /*relative-path*/>& recordList, const std::string& fieldName) const {
  // enforce that starting point is a table node
...
void ConfigurationTree::recursiveGetCommonFields(std::vector<ConfigurationTree::RecordField>& fieldCandidateList,
                                                 ...
                                                 unsigned int depth, const std::string& relativePathBase,
  ...
  //			else if depth > 0 and is UID-Link
  //				if Link Table/UID pair is not found in <field candidates list> (avoid endless loops through
  //tree) 					recursiveGetCommonFields()
  //	=====================
  ...
  for (const auto& fieldNode : recordChildren) {
    if (fieldNode.second.isValueNode()) {
      ...
      if (inFirstRecord)  // first uid record
      {
        ...
        if (found) {
          ...
          fieldCandidateList.push_back(
              ConfigurationTree::RecordField(tableName, uid, fieldNode.first,
      ...
      fieldNode.second.recursiveGetCommonFields(fieldCandidateList, fieldCount, fieldAcceptList, fieldRejectList, depth,
  ...
  for (auto& childNamesAtPriority : childrenNamesByPriority) {
    ...
    for (auto& childName : childNamesAtPriority) {
          ...
          try {
            ...
            for (const auto& fieldValue : fieldValues) {
              ...
              __COUT__ << "\t\tCheck: " << filterPair.first << " == " << fieldValue << " => "
                       ...
                       << this->getNode(filterPath).getValueAsString(true) << std::endl;

              if (StringMacros::wildCardMatch(StringMacros::decodeURIComponent(fieldValue),
            ...
            __SS__ << "Failed to access filter path '" << filterPath << "' - aborting." << std::endl;
            __SS_THROW__;
          }
      ...
      retVector[retVector.size() - 1].push_back(
          std::pair<std::string, ConfigurationTree>(childName, this->getNode(childName, true)));
...
std::vector<std::pair<std::string, ConfigurationTree>> ConfigurationTree::getChildren(
    std::map<std::string /*relative-path*/, std::string /*value*/> filterMap, bool byPriority,
...
//==============================================================================
bool ConfigurationTree::isRootNode(void) const { return (!configuration_); }

//==============================================================================
bool ConfigurationTree::isConfigurationNode(void) const {
  return (configuration_ && row_ == ConfigurationView::INVALID && col_ == ConfigurationView::INVALID);
}
...
std::vector<std::vector<std::string>> ConfigurationTree::getChildrenNamesByPriority(bool onlyStatusTrue) const {
  ...
  if (!configView_) {
    __SS__ << "Can not get children names of '" << getValueAsString() << "' with null configuration view pointer!"
           << std::endl;
    if (isLinkNode() && isDisconnected())
      ss << " This node is a disconnected link to " << getDisconnectedTableName() << std::endl;
  ...
  if (row_ == ConfigurationView::INVALID && col_ == ConfigurationView::INVALID) {
    ...
    if (1)  // reshuffle by priority
    {
      try {
        ...
        for (unsigned int r = 0; r < configView_->getNumberOfRows(); ++r)
          if (groupId_ == "" || configView_->isEntryInGroup(r, childLinkIndex_, groupId_)) {
        ...
        __COUT_WARN__ << "Error identifying priority. Assuming all children have equal priority (Error: " << e.what()
                      << __E__;
        retVector.clear();
      }
    }
    ...
    for (unsigned int r = 0; r < configView_->getNumberOfRows(); ++r)
      if (groupId_ == "" || configView_->isEntryInGroup(r, childLinkIndex_, groupId_)) {
        ...
        retVector.push_back(std::vector<std::string /*child name*/>());
        retVector[retVector.size() - 1].push_back(configView_->getDataView()[r][configView_->getColUID()]);
      }
    ...
    for (unsigned int c = 0; c < configView_->getNumberOfColumns(); ++c)
      if (c == configView_->getColUID() ||  // skip UID and linkID columns (only show link column, to avoid duplicates)
          configView_->getColumnInfo(c).isChildLinkGroupID() || configView_->getColumnInfo(c).isChildLinkUID())
        continue;
      else {
        retVector.push_back(std::vector<std::string /*child name*/>());
        retVector[retVector.size() - 1].push_back(configView_->getColumnInfo(c).getName());
      }
...
ConfigurationTree ConfigurationTree::getValueAsTreeNode(void) const {
  ...
  {
    __SS__ << "Invalid value string '" << valueString << "' - must start with a '/' character." << std::endl;
    __SS_ONLY_THROW__;
  }
}
