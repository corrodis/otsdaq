...
void DatabaseConfigurationInterface::fill(ConfigurationBase* configuration, ConfigurationVersion version) const
  ...
  __SS__ << "\n\nDBI Error while filling '" << configuration->getConfigurationName() << "' version '" << versionstring
...
void DatabaseConfigurationInterface::saveActiveVersion(const ConfigurationBase* configuration, bool overwrite) const
    throw(std::runtime_error) {
  auto ifc = db::ConfigurationInterface{default_dbprovider};
...
std::set<ConfigurationVersion> DatabaseConfigurationInterface::getVersions(const ConfigurationBase* configuration) const
  ...
  for (std::string const& version : result) resultSet.insert(ConfigurationVersion(std::stol(version, 0, 10)));
...
std::set<std::string /*name*/> DatabaseConfigurationInterface::getAllConfigurationGroupNames(
...
ConfigurationGroupKey DatabaseConfigurationInterface::findLatestGroupKey(const std::string& groupName) const noexcept {
...
std::set<ConfigurationGroupKey /*key*/> DatabaseConfigurationInterface::getKeys(const std::string& groupName) const {
  ...
  for (auto& n : names)
    if (n.find(groupName) == 0) retSet.insert(ConfigurationGroupKey(n));
  return retSet;
}

//==============================================================================
// return the contents of a configuration group
config_version_map_t DatabaseConfigurationInterface::getConfigurationGroupMembers(std::string const& configurationGroup,
                                                                                  bool includeMetaDataTable) const
    throw(std::runtime_error) try {
  auto ifc = db::ConfigurationInterface{default_dbprovider};
  ...
  auto to_map = [](auto const& inputList, bool includeMetaDataTable) {
    ...
    if (!includeMetaDataTable) {
      ...
      if (metaTable != resultMap.end()) resultMap.erase(metaTable);
    }
    return resultMap;
  };
