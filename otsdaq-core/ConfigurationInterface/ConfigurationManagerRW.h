...
class ConfigurationManagerRW : public ConfigurationManager {
 public:
  ...
  const std::map<std::string, ConfigurationInfo>& getAllConfigurationInfo(bool refresh = false,
                                                                          std::string* accumulatedErrors = 0,
                                                                          const std::string& errorFilterName = "");
  std::map<std::string /*tableName*/, std::map<std::string /*aliasName*/, ConfigurationVersion /*version*/> >
  ...
  ConfigurationBase* getVersionedConfigurationByName(const std::string& configurationName, ConfigurationVersion version,
                                                     bool looseColumnMatching = false);
  ConfigurationBase* getConfigurationByName(const std::string& configurationName);
  ConfigurationGroupKey findConfigurationGroup(
      const std::string& groupName, const std::map<std::string, ConfigurationVersion>& groupMembers,
      const std::map<std::string /*name*/, std::string /*alias*/>& groupAliases =
          std::map<std::string /*name*/, std::string /*alias*/>());
  ConfigurationBase* getMetadataTable(void) {
    return &groupMetadataTable_; /* created for use in otsdaq_flatten_system_aliases, e.g. */
  }

  //==============================================================================
  // modifiers of generic ConfigurationBase

  ConfigurationVersion saveNewConfiguration(const std::string& configurationName,
                                            ConfigurationVersion temporaryVersion = ConfigurationVersion(),
                                            bool makeTemporary = false);  //, bool saveToScratchVersion = false);
  ConfigurationVersion copyViewToCurrentColumns(const std::string& configurationName,
                                                ConfigurationVersion sourceVersion);
  void eraseTemporaryVersion(const std::string& configurationName,
                             ConfigurationVersion targetVersion = ConfigurationVersion());
  ...
  void activateConfigurationGroup(const std::string& configGroupName, ConfigurationGroupKey configGroupKey,
                                  std::string* accumulatedTreeErrors = 0);

  ConfigurationVersion createTemporaryBackboneView(
      ConfigurationVersion sourceViewVersion =
          ConfigurationVersion());  //-1, from MockUp, else from valid backbone view version
  ...
  ConfigurationGroupKey saveNewConfigurationGroup(
...
struct TableEditStruct {
  ...
  TableEditStruct(const std::string& configName, ConfigurationManagerRW* cfgMgr)
      : createdTemporaryVersion_(false), modified_(false), configName_(configName) {
    __COUT__ << "Creating Table-Edit Struct for " << configName_ << std::endl;
    config_ = cfgMgr->getConfigurationByName(configName_);

    if (!(originalVersion_ = config_->getView().getVersion()).isTemporaryVersion()) {
      ...
      cfgMgr->saveNewConfiguration(configName_, temporaryVersion_,
