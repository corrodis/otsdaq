...
std::set<ConfigurationVersion> FileConfigurationInterface::getVersions(const ConfigurationBase* configuration) const {
  ...
  while ((dirp = readdir(dp)) != 0) {
    if (dirp->d_type == isDir && dirp->d_name[0] != '.') {
      ...
      while (it != dirName.end() && std::isdigit(*it)) ++it;

      if (dirName.empty() || it != dirName.end()) {
        std::cout << __COUT_HDR_FL__ << "WARNING: there is a non numeric directory in " << configDir << " called "
                  << dirName << " please remove it from there since only numeric directories are considered."
                  << std::endl;
        continue;
      }
