...
const std::map<std::string, ConfigurationInfo> &ConfigurationManagerRW::getAllConfigurationInfo(
  ...
  if ((pDIR = opendir(path.c_str())) != 0) {
    while ((entry = readdir(pDIR)) != 0) {
      ...
      if (strlen(entry->d_name) < strlen(fileExt) + MIN_CONFIG_NAME_SZ) continue;
      ...
      {
        theInterface_->get(configuration, entry->d_name, 0, 0,
                           true);  // dont fill
      } catch (cet::exception) {
        ...
        if (accumulatedErrors) {
          ...
          try {
            ...
            __COUT__ << "Skipping! Allowing illegal columns didn't work either... " << entry->d_name << "\n";
            continue;
          }
      ...
      if (nameToConfigurationMap_[entry->d_name])  // handle if instance existed
      {
        ...
        std::set<ConfigurationVersion> versions = nameToConfigurationMap_[entry->d_name]->getStoredVersions();
          ...
          if (version.isTemporaryVersion()) {
            ...
            try  // do NOT let ConfigurationView::init() throw here
            {
              ...
              configuration->copyView(  // this calls ConfigurationView::init()
                  nameToConfigurationMap_[entry->d_name]->getView(), version, username_);
  ...
  try {
    ...
    for (auto &groupInfo : allGroupInfo_) {
      try {
        loadConfigurationGroup(groupInfo.first /*groupName*/, groupInfo.second.getLatestKey(), false /*doActivate*/,
                               &groupInfo.second.latestKeyMemberMap_ /*groupMembers*/, 0 /*progressBar*/,
                               0 /*accumulateErrors*/, &groupInfo.second.latestKeyGroupComment_,
                               &groupInfo.second.latestKeyGroupAuthor_, &groupInfo.second.latestKeyGroupCreationTime_,
                               true /*doNotLoadMember*/, &groupInfo.second.latestKeyGroupTypeString_);
      } catch (...) {
        __COUT_WARN__ << "Error occurred loading latest group info into cache for '" << groupInfo.first << "'..."
                      << __E__;
...
void ConfigurationManagerRW::activateConfigurationGroup(const std::string &configGroupName,
  ...
  if (accumulatedTreeErrors && *accumulatedTreeErrors != "") {
    __COUT_ERR__ << "Errors were accumulated so de-activating group: " << configGroupName << " (" << configGroupKey
                 << ")" << std::endl;
  ...
  __COUT_INFO__ << "Updating persistent active groups to " << ConfigurationManager::ACTIVE_GROUP_FILENAME << " ..."
                << std::endl;
  __MOUT_INFO__ << "Updating persistent active groups to " << ConfigurationManager::ACTIVE_GROUP_FILENAME << " ..."
                << std::endl;

  std::string fn = ConfigurationManager::ACTIVE_GROUP_FILENAME;
  FILE *fp = fopen(fn.c_str(), "w");
  if (!fp) {
    __SS__ << "Fatal Error! Unable to open the file " << ConfigurationManager::ACTIVE_GROUP_FILENAME
...
ConfigurationVersion ConfigurationManagerRW::createTemporaryBackboneView(ConfigurationVersion sourceViewVersion) {
  ...
  ConfigurationVersion tmpVersion =
      ConfigurationVersion::getNextTemporaryVersion();  // get the default temporary version
  ...
  for (auto &name : backboneMemberNames) {
    ...
    if (retTmpVersion < tmpVersion) tmpVersion = retTmpVersion;
  }
...
ConfigurationBase *ConfigurationManagerRW::getConfigurationByName(const std::string &configurationName) {
  if (nameToConfigurationMap_.find(configurationName) == nameToConfigurationMap_.end()) {
    ...
    if ((f = configurationName.find(' ')) != std::string::npos)
      ss << "There was a space character found in the configuration name needle at position " << f
...
void ConfigurationManagerRW::eraseTemporaryVersion(const std::string &configurationName,
  ...
  if (allConfigurationInfo_.find(configurationName) == allConfigurationInfo_.end()) return;
...
void ConfigurationManagerRW::clearAllCachedVersions() {
  for (auto configInfo : allConfigurationInfo_) configInfo.second.configurationPtr_->trimCache(0);
}
...
ConfigurationVersion ConfigurationManagerRW::copyViewToCurrentColumns(const std::string &configurationName,
  ...
  ConfigurationBase *config =
      getVersionedConfigurationByName(configurationName, ConfigurationVersion(sourceVersion), true);

  // copy from source version to a new temporary version
  ConfigurationVersion newTemporaryVersion = config->copyView(config->getView(), ConfigurationVersion(), username_);
...
//==============================================================================
// cacheGroupKey
void ConfigurationManagerRW::cacheGroupKey(const std::string &groupName, ConfigurationGroupKey key) {
...
ConfigurationGroupKey ConfigurationManagerRW::findConfigurationGroup(
    const std::string &groupName, const std::map<std::string, ConfigurationVersion> &groupMemberMap,
  ...
  if (groupInfo.keys_.size()) keyMinToCheck = groupInfo.keys_.rbegin()->key();
  ...
  for (const auto &key : groupInfo.keys_) {
    ...
    loadConfigurationGroup(groupName, key, false /*doActivate*/, &compareToMemberMap /*memberMap*/, 0, 0, 0, 0,
                           0, /*null pointers*/
                           true /*doNotLoadMember*/, 0 /*groupTypeString*/, &compareToGroupAliases);
    ...
    for (auto &memberPair : groupMemberMap) {
      ...
      if (groupAliases.find(memberPair.first) != groupAliases.end()) {
        ...
        if (compareToGroupAliases.find(memberPair.first) == compareToGroupAliases.end() ||      // alias is missing
            groupAliases.at(memberPair.first) != compareToGroupAliases.at(memberPair.first)) {  // then different
    ...
    if (groupMemberMap.size() != compareToMemberMap.size())
      continue;  // different size, so not same (groupMemberMap is a subset of memberPairs)
...
ConfigurationGroupKey ConfigurationManagerRW::saveNewConfigurationGroup(
    ...
    const std::string &groupComment, std::map<std::string /*table*/, std::string /*alias*/> *groupAliases) {
  // steps:
  ...
  // determine new group key
  ConfigurationGroupKey newKey = ConfigurationGroupKey::getNextKey(theInterface_->findLatestGroupKey(groupName));
  ...
  for (auto &memberPair : groupMembers) {
    ...
    if (allCfgInfo.find(memberPair.first) == allCfgInfo.end()) {
      ...
      if (groupMetadataTable_.getConfigurationName() == memberPair.first) {
    ...
    if (allCfgInfo[memberPair.first].versions_.find(memberPair.second) ==
      ...
      __SS__ << "Group member  \"" << memberPair.first << "\" version \"" << memberPair.second
  ...
  try {
    ...
    if (groupAliases)
      groupAliasesString =
          StringMacros::mapToString(*groupAliases, "," /*primary delimeter*/, ":" /*secondary delimeter*/);
    __COUT__ << "Metadata: " << username_ << " " << time(0) << " " << groupComment << " " << groupAliasesString
             << std::endl;
    ...
    groupMembers[groupMetadataTable_.getConfigurationName()] = groupMetadataTable_.getViewVersion();

    theInterface_->saveConfigurationGroup(groupMembers, ConfigurationGroupKey::getFullGroupString(groupName, newKey));
...
ConfigurationVersion ConfigurationManagerRW::saveNewBackbone(ConfigurationVersion temporaryVersion) {
  ...
  for (auto &name : backboneMemberNames) {
    ...
    if (retNewVersion > newVersion) newVersion = retNewVersion;
  }
  ...
  for (auto &name : backboneMemberNames) {
    ...
    retNewVersion =
        getConfigurationInterface()->saveNewVersion(getConfigurationByName(name), temporaryVersion, newVersion);
    if (retNewVersion != newVersion) {
      __SS__ << "Failure! New view requested was " << newVersion << ". Mismatched new view created: " << retNewVersion
             << std::endl;
      __COUT_ERR__ << ss.str();
      __SS_THROW__;
    }
...
void ConfigurationManagerRW::testXDAQContext() {
  ...
  //				//	ConfigurationVersion> &groupMembers, ConfigurationGroupKey
  //previousVersion=ConfigurationGroupKey()); 				newKey =
  ...
  //			//	ConfigurationGroupKey configGroupKey, bool doActivate=false, ProgressBar* progressBar=0,
