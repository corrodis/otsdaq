...
TimeFormatter::TimeFormatter(std::string source) {
  if (!USE_TIMER) return;
  origin_ = source;
  std::cout << __COUT_HDR_FL__ << "[TimeFormatter::TimeFormatter()]\t\t    Time counter started for " << origin_
            << std::endl
            << std::endl;
...
void TimeFormatter::stopTimer(void) {
  ...
  std::cout << __COUT_HDR_FL__ << "[TimeFormatter::stopTimer()]\t\t\t    Elapsed time: " << stop - start
...
std::string TimeFormatter::getTime(void) {
  ...
  sprintf(theDate, "%d-%02d-%02d %02d:%02d:%02d", thisTime->tm_year + 1900, thisTime->tm_mon + 1, thisTime->tm_mday,
