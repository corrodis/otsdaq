  ...
  public:
	          ...
	          bool                                         looseColumnMatching = false)
	{
		...
		else if (!dontFill)
		{
			...
			{
				__COUT__ << "FATAL ERROR: Can't ask to fill a configuration object with a negative version! " << configurationName << std::endl;
				__SS__ << "FATAL ERROR: Invalid latest version." << std::endl
				       << std::endl
				       << std::endl
				       << "*******************" << std::endl
				       << "Suggestion: If you expect a version to exist for this configuration, perhaps this is your first time running with the artdaq database. (and your old configurations have not been transferred?) " << std::endl
				       << "Try running this once:\n\n\totsdaq_database_migrate" << std::endl
				       << std::endl
				       << "This will migrate the old ots file system configuration to the artdaq database approach." << std::endl
				       << std::endl
				       << std::endl;
				__SS_THROW__;
			}
		...
		try  //to fill
		{
			...
			{
				__COUT__ << "View Table Name mismatch!! " << configuration->getViewP ()->getTableName () << " vs " << configuration->getMockupViewP ()->getTableName () << std::endl;
				throw;
			}
		...
		{
			__COUT__ << "Error occurred while getting and filling Configuration \"" << configurationName << "\" version:" << version << std::endl;
