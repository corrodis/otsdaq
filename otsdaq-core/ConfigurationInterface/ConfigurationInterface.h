...
class ConfigurationInterface {
 ...
 public:
  ...
  //
  void get(ConfigurationBase*& configuration, const std::string configurationName,
           std::shared_ptr<const ConfigurationGroupKey> groupKey = 0, const std::string* groupName = 0,
           bool dontFill = false, ConfigurationVersion version = ConfigurationVersion(), bool resetConfiguration = true,
    ...
    if (groupKey != 0 && groupName != 0) {  // FIXME -- new ConfigurationGroup and ConfigurationGroupKey should be used!
      ...
      if (version == ConfigurationVersion::INVALID &&
        ...
        __SS__ << "FATAL ERROR: Invalid latest version." << std::endl
               ...
               << std::endl
               << "*******************" << std::endl
               << "Suggestion: If you expect a version to exist for this configuration, perhaps this is your first "
                  "time running with the artdaq database. (and your old configurations have not been transferred?) "
               << std::endl
    ...
    {                        // EXCEPT, keep temporary views! (call ConfigurationBase::reset to really reset all)
      ...
      std::set<ConfigurationVersion> versions = configuration->getStoredVersions();
    ...
    try  // to fill
    {
      if (version.isTemporaryVersion()) {
        __COUT_ERR__ << "FATAL ERROR: Can not use interface to fill a configuration object with a temporary version!"
                     << std::endl;
      ...
      if (configuration->getViewP()->getVersion() != version) {
        __COUT__ << "Version mismatch!! " << configuration->getViewP()->getVersion() << " vs " << version << std::endl;
        throw;
      }
      ...
      for (nameIsMatchIndex = 0, nameIsMatchStorageIndex = 0;
           nameIsMatchIndex < configuration->getViewP()->getTableName().size(); ++nameIsMatchIndex) {
        ...
        if (configuration->getViewP()->getTableName()[nameIsMatchIndex] == '_') continue;  // skip to next character
      ...
      {
        __COUT__ << "View Table Name mismatch!! " << configuration->getViewP()->getTableName() << " vs "
      ...
      __COUT__ << "Error occurred while getting and filling Configuration \"" << configurationName
  ...
  // table handling
  virtual std::set<std::string /*name*/> getAllConfigurationNames() const throw(std::runtime_error) {
    __SS__;
    __THROW__(ss.str() +
              "ConfigurationInterface::... Must only call findAllGlobalConfigurations in a mode with this "
              "functionality implemented (e.g. DatabaseConfigurationInterface).");
  }
  ...
  ConfigurationVersion saveNewVersion(ConfigurationBase* configuration, ConfigurationVersion temporaryVersion,
                                      ConfigurationVersion newVersion = ConfigurationVersion());

  // group handling
  virtual std::set<std::string /*name*/> getAllConfigurationGroupNames(const std::string& filterString = "") const
  ...
  // Caution: getConfigurationGroupMembers must be carefully used.. the table versions are as initially defined for
  // table versions aliases, i.e. not converted according to the metadata groupAliases!
  virtual std::map<std::string /*name*/, ConfigurationVersion /*version*/> getConfigurationGroupMembers(
      std::string const& /*globalConfiguration*/, bool includeMetaDataTable = false) const throw(std::runtime_error) {
    __SS__;
    __THROW__(ss.str() +
              "ConfigurationInterface::... Must only call findAllGlobalConfigurations in a mode with this "
              "functionality implemented (e.g. DatabaseConfigurationInterface).");
  ...
  virtual void saveConfigurationGroup(
      std::map<std::string /*name*/, ConfigurationVersion /*version*/> const& /*configurationMap*/,
 ...
 public:  // was protected,.. unfortunately, must be public to allow otsdaq_database_migrate and
          // otsdaq_import_system_aliases to compile
  virtual ConfigurationGroupKey findLatestGroupKey(const std::string& groupName) const
  /* return INVALID if no existing versions */ {
    __SS__;
    __THROW__(ss.str() +
              "ConfigurationInterface::... Must only call findLatestGroupKey in a mode with this functionality "
              "implemented (e.g. DatabaseConfigurationInterface).");
  }
  virtual ConfigurationVersion findLatestVersion(
      const ConfigurationBase* configuration) const = 0;  // return INVALID if no existing versions
