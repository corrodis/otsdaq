...
bool ConfigurationHandler::validateNode(XMLCh* tagName, xercesc::DOMNode* node, const std::string& expectedValue) {
  ...
  if (XML_TO_STRING(node->getFirstChild()->getNodeValue()) != expectedValue) {
    __COUT__ << "The tag " << XML_TO_CHAR(tagName) << " with value "
             << XML_TO_CHAR(node->getFirstChild()->getNodeValue()) << " doesn't match the expected value "
...
xercesc::DOMNode* ConfigurationHandler::getNode(XMLCh* tagName, xercesc::DOMElement* parent, unsigned int itemNumber) {
  ...
  //        __COUT__<< "Value: " << XML_TO_CHAR(nodeList->item(itemNumber)->getFirstChild()->getNodeValue()) <<
  //        std::endl;
  return nodeList->item(itemNumber);
}

//==============================================================================
xercesc::DOMElement* ConfigurationHandler::getElement(XMLCh* tagName, xercesc::DOMNode* parent,
                                                      unsigned int itemNumber) {
  return dynamic_cast<xercesc::DOMElement*>(getNode(tagName, parent, itemNumber));
}

//==============================================================================
xercesc::DOMElement* ConfigurationHandler::getElement(XMLCh* tagName, xercesc::DOMElement* parent,
                                                      unsigned int itemNumber) {
  return dynamic_cast<xercesc::DOMElement*>(getNode(tagName, parent, itemNumber));
}

//==============================================================================
void ConfigurationHandler::readXML(ConfigurationBase* configuration, ConfigurationVersion version) {
...
void ConfigurationHandler::readXML(ConfigurationBase& configuration, ConfigurationVersion version) {
  ...
  __COUT__ << "Into View with Table Name: " << configuration.getViewP()->getTableName() << std::endl;
  ...
  parser->setDoSchema(
      false);  // RAR set to false to get rid of "error reading primary document *.xsd"
               // uses if true:
               // rootElement->setAttribute(CONVERT_TO_XML("xsi:noNamespaceSchemaLocation"),CONVERT_TO_XML("ConfigurationBase.xsd"));
  ...
  try {
    ...
    if (!validateNode(extensionTableNameTag_, extensionTableNameNode, configuration.getView().getTableName()))
      throw(std::runtime_error(std::string("The document is missing the mandatory tag: ") +
    ...
    assert(validateNode(runTypeTag_, runTypeNode, configuration.getConfigurationName()));
    ...
    if (!locationNode)
      throw(
          std::runtime_error(std::string("The document is missing the mandatory tag: ") + XML_TO_STRING(locationTag_)));
    ...
    if (!datasetElement)
      throw(
          std::runtime_error(std::string("The document is missing the mandatory tag: ") + XML_TO_STRING(datasetTag_)));
    ...
    {
      throw(std::runtime_error(std::string("Missing version tag: ") +
    ...
    } else  // else verify version tag matches parameter version
    {
      ...
      if (strcmp(tmpVersionStr, XML_TO_CHAR(versionNode->getFirstChild()->getNodeValue())) != 0)
        throw(std::runtime_error(std::string("Mis-matched version tag: ") +
                                 XML_TO_CHAR(versionNode->getFirstChild()->getNodeValue()) + " vs " + tmpVersionStr));
    }
    // version is valid
    ...
    if (commentDescriptionNode->getFirstChild() != 0)
      configuration.getViewP()->setComment(XML_TO_CHAR(commentDescriptionNode->getFirstChild()->getNodeValue()));
    ...
    if (!dataNodeList) throw(std::runtime_error(std::string("Can't find ") + XML_TO_STRING(dataTag_) + " tag!"));
      ...
      for (XMLSize_t col = 0; col < columnNodeList->getLength(); col++) {
        ...
        if (configuration.getView().getColumnInfo(colNumber).getStorageName() !=
          ...
          error << __COUT_HDR__ << std::endl
                ...
                << configuration.getView().getColumnInfo(colNumber).getStorageName() << " defined in the view "
                << configuration.getView().getTableName() << " doesn't match the file column order, since the "
                << colNumber + 1 << (colNumber == 0 ? "st" : (colNumber == 1 ? "nd" : (colNumber == 2 ? "rd" : "th")))
                << " element found in the file at " << XML_TO_CHAR(dataTag_) << " tag number " << row << " is "
        ...
        configuration.getViewP()->setValueAsString(
            XML_TO_STRING(columnNodeList->item(col)->getFirstChild()->getNodeValue()), row, colNumber);
...
//==============================================================================
std::string ConfigurationHandler::writeXML(const ConfigurationBase& configuration) {
  initPlatform();

  std::string configFile = getXMLFileName(
      configuration, configuration.getViewVersion());  // std::string(getenv("CONFIGURATION_DATA_PATH")) + "/" +
                                                       // configuration.getConfigurationName() + "_write.xml";
  ...
  if (implementation != 0) {
    try {
      ...
      rootElement->setAttribute(CONVERT_TO_XML("xmlns:xsi"),
                                CONVERT_TO_XML("http://www.w3.org/2001/XMLSchema-instance"));
      rootElement->setAttribute(
          CONVERT_TO_XML("xsi:noNamespaceSchemaLocation"),
          CONVERT_TO_XML("ConfigurationBase.xsd"));  // configFile.substr(configFile.rfind("/")+1).c_str())); //put the
                                                     // file name here..? ConfigurationBase.xsd"));
      //${OTSDAQ_DIR}/otsdaq/ConfigurationDataFormats/ConfigurationInfo.xsd
      ...
      xercesc::DOMText* extensionTableNameValue =
          document->createTextNode(CONVERT_TO_XML(configuration.getView().getTableName().c_str()));
      ...
      xercesc::DOMText* nameValue =
          document->createTextNode(CONVERT_TO_XML(configuration.getConfigurationName().c_str()));
      ...
      xercesc::DOMText* runNumberValue = document->createTextNode(
          CONVERT_TO_XML("1"));  // This is dynamic and need to be created when I write the file
      ...
      xercesc::DOMText* runBeginTimestampValue = document->createTextNode(CONVERT_TO_XML(
          TimeFormatter::getTime().c_str()));  // This is dynamic and need to be created when I write the files
      ...
      xercesc::DOMText* locationValue = document->createTextNode(
          CONVERT_TO_XML("CERN P5"));  // This is dynamic and need to be created when I write the file
      ...
      xercesc::DOMText* versionValue =
          document->createTextNode(CONVERT_TO_XML(configuration.getView().getVersion().version()));
      ...
      xercesc::DOMText* commentDescriptionValue =
          document->createTextNode(CONVERT_TO_XML(configuration.getView().getComment().c_str()));
      ...
      xercesc::DOMText* createdByUserValue =
          document->createTextNode(CONVERT_TO_XML(configuration.getView().getAuthor().c_str()));
      createdByUserElement->appendChild(createdByUserValue);
      // for(ConfigurationView::iterator it=configuration.getView().begin(); it!=configuration.getView().end(); it++)

      for (unsigned int row = 0; row < configuration.getView().getNumberOfRows(); row++) {
        xercesc::DOMElement* dataElement = document->createElement(dataTag_);
        datasetElement->appendChild(dataElement);

        for (unsigned int col = 0; col < configuration.getView().getNumberOfColumns(); col++) {
          xercesc::DOMElement* element = document->createElement(
              CONVERT_TO_XML(configuration.getView().getColumnInfo(col).getStorageName().c_str()));
          dataElement->appendChild(element);
          xercesc::DOMText* value =
              document->createTextNode(CONVERT_TO_XML(configuration.getView().getDataView()[row][col].c_str()));
...
void ConfigurationHandler::outputXML(xercesc::DOMDocument* pmyDOMDocument, std::string fileName) {
  ...
  // Return the first registered implementation that has the desired features. In this case, we are after a DOM
  // implementation that has the LS feature... or Load/Save.  DOMImplementation *implementation =
...
//==============================================================================
std::string ConfigurationHandler::writeXML(const ConfigurationBase* configuration) { return writeXML(*configuration); }

//==============================================================================
std::string ConfigurationHandler::getXMLFileName(const ConfigurationBase& configuration, ConfigurationVersion version) {
  std::stringstream fileName;
  fileName << getXMLDir(&configuration) << version << '/' << configuration.getConfigurationName() << "_v" << version
           << ".xml";
  return fileName.str();
}

//==============================================================================
std::string ConfigurationHandler::getXMLDir(const ConfigurationBase* configuration) {
