...
class ConfigurationTree {
 ...
 public:
  ...
  ConfigurationTree& operator=(const ConfigurationTree& a) {
    __COUT__ << "OPERATOR= COPY CONSTRUCTOR ConfigManager: " << configMgr_ << " configuration: " << configuration_
             << std::endl;
    ...
    __COUT__ << "OPERATOR= COPY CONSTRUCTOR CANNOT BE USED - ConfigurationTree is a const class. SO YOUR CODE IS "
                "WRONG! You should probably instantiate and initialize another ConfigurationTree, rather than "
                "assigning to an existing ConfigurationTree. Crashing now."
             << std::endl;
    __COUT__ << "OPERATOR= COPY CONSTRUCTOR CANNOT BE USED - ConfigurationTree is a const class. SO YOUR CODE IS "
                "WRONG! You should probably instantiate and initialize another ConfigurationTree, rather than "
                "assigning to an existing ConfigurationTree. Crashing now."
 ...
 private:
  template <typename T>
  T handleValidateValueForColumn(const ConfigurationView* configView, std::string value, unsigned int col,
                                 ots::identity<T>) const;  // defined in included .icc source
  std::string handleValidateValueForColumn(const ConfigurationView* configView, std::string value, unsigned int col,
 ...
 public:
  ...
  std::vector<std::vector<std::pair<std::string, ConfigurationTree>>> getChildrenByPriority(
      std::map<std::string /*relative-path*/, std::string /*value*/> filterMap =
          std::map<std::string /*relative-path*/, std::string /*value*/>(),
      bool onlyStatusTrue = false) const;
  std::vector<std::pair<std::string, ConfigurationTree>> getChildren(
      std::map<std::string /*relative-path*/, std::string /*value*/> filterMap =
          std::map<std::string /*relative-path*/, std::string /*value*/>(),
 ...
 protected:
  ...
  struct RecordField {
    RecordField(const std::string& table, const std::string& uid, const std::string& columnName,
                const std::string& relativePath, const ViewColumnInfo* columnInfo)
        : tableName_(table), columnName_(columnName), relativePath_(relativePath), columnInfo_(columnInfo) {}
  ...
  std::vector<ConfigurationTree::RecordField> getCommonFields(
      ...
      const std::vector<std::string /*relative-path*/>& fieldRejectList, unsigned int depth = -1) const;
 ...
 private:
  ...
  ConfigurationTree(const ConfigurationManager* const& configMgr, const ConfigurationBase* const& config,
                    const std::string& groupId, const ConfigurationBase* const& linkParentConfig,
                    const std::string& linkColName, const std::string& linkColValue, const unsigned int linkBackRow,
  ...
  // static bool					wildCardMatch (const std::string& needle, const std::string&
  // haystack);

  void recursiveGetCommonFields(std::vector<ConfigurationTree::RecordField>& fieldCandidateList,
                                std::vector<int>& fieldCount,
                                const std::vector<std::string /*relative-path*/>& fieldAcceptList,
                                const std::vector<std::string /*relative-path*/>& fieldRejectList, unsigned int depth,
                                const std::string& relativePathBase, bool inFirstRecord) const;
  ...
  const ConfigurationBase*
      linkParentConfig_;           // link node parent config pointer (could be used to traverse backwards through tree)
