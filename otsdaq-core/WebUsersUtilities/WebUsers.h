...
class WebUsers {
 public:
  ...
  struct RequestUserInfo {
    ...
    RequestUserInfo(const std::string& requestType, const std::string& cookieCode)
        : requestType_(requestType),
          ...
          uid_(-1)  // init to invalid user, since only WebUser owner will have access to uid. RemoteWebUsers will see
                    // invalid uid.
    {}
    ...
    bool setGroupPermissionLevels(const std::string& groupPermissionLevelsString) {
      ...
      StringMacros::getMapFromString(  // re-factor membership string to set
          groupPermissionLevelsString, groupPermissionLevelMap_);
    ...
    //------- getters --------//
    const std::map<std::string /*groupName*/, WebUsers::permissionLevel_t>& getGroupPermissionLevels() const {
    ...
    const WebUsers::permissionLevel_t& getGroupPermissionLevel() {
      ...
      if (!groupsAllowed_.size()) {
        for (const auto& userGroupPair : groupPermissionLevelMap_)
          if (StringMacros::inWildCardSet(userGroupPair.first, groupsDisallowed_)) {
  ...
  bool xmlRequestOnGateway(cgicc::Cgicc& cgi, std::ostringstream* out, HttpXmlDocument* xmldoc,
 ...
 public:
  ...
  static bool checkRequestAccess(cgicc::Cgicc& cgi, std::ostringstream* out, HttpXmlDocument* xmldoc,
                                 WebUsers::RequestUserInfo& userInfo, bool isWizardMode = false);
  ...
  uint64_t attemptActiveSession(const std::string& uuid, std::string& jumbledUser, const std::string& jumbledPw,
                                std::string& newAccountCode, const std::string& ip);
  uint64_t attemptActiveSessionWithCert(const std::string& uuid, std::string& jumbledEmail, std::string& cookieCode,
                                        std::string& username, const std::string& ip);
  ...
  bool cookieCodeIsActiveForRequest(
      std::string& cookieCode, std::map<std::string /*groupName*/, WebUsers::permissionLevel_t>* userPermissions = 0,
      uint64_t* uid = 0, const std::string& ip = "0", bool refresh = true, std::string* userWithLock = 0,
      uint64_t* activeUserSessionIndex = 0);
  uint64_t cookieCodeLogout(const std::string& cookieCode, bool logoutOtherUserSessions, uint64_t* uid = 0,
  ...
  void changeSettingsForUser(uint64_t uid, const std::string& bgcolor, const std::string& dbcolor,
  ...
  static void tooltipCheckForUsername(const std::string& username, HttpXmlDocument* xmldoc, const std::string& srcFile,
                                      const std::string& srcFunc, const std::string& srcId);
  static void tooltipSetNeverShowForUsername(const std::string& username, HttpXmlDocument* xmldoc,
                                             const std::string& srcFile, const std::string& srcFunc,
                                             const std::string& srcId, bool doNeverShow, bool temporarySilence);

  void modifyAccountSettings(uint64_t actingUid, uint8_t cmd_type, const std::string& username,
                             const std::string& displayname, const std::string& email, const std::string& permissions);
  ...
  bool getUserInfoForCookie(std::string& cookieCode, std::string* userName, std::string* displayName = 0,
 ...
 private:
  ...
  inline bool isInactiveForGroup(std::map<std::string /*groupName*/, WebUsers::permissionLevel_t>& permissionMap,
                                 const std::string& groupName = WebUsers::DEFAULT_USER_GROUP);
  inline bool isAdminForGroup(std::map<std::string /*groupName*/, WebUsers::permissionLevel_t>& permissionMap,
                              const std::string& groupName = WebUsers::DEFAULT_USER_GROUP);
  ...
  void saveToDatabase(FILE* fp, const std::string& field, const std::string& value,
                      uint8_t type = DB_SAVE_OPEN_AND_CLOSE, bool addNewLine = true);
  ...
  static std::string getTooltipFilename(const std::string& username, const std::string& srcFile,
                                        const std::string& srcFunc, const std::string& srcId);
  ...
  enum {
    ACTIVE_SESSION_EXPIRATION_TIME =
        120 * 60,  // 120 minutes, cookie is changed every half period of ACTIVE_SESSION_EXPIRATION_TIME
  ...
  std::vector<std::string> UsersUsernameVector, UsersUserEmailVector, UsersDisplayNameVector, UsersSaltVector,
      UsersLastModifierUsernameVector;
