...
//				{
//					bool automaticCommand = 0; //automatic commands should not refresh cookie code.. only
//user initiated commands should! 					bool checkLock = true;
...
//							,!automaticCommand			//true/false refresh cookie
//code
//							,USER_PERMISSIONS_THRESHOLD //set access level requirement to pass
//gateway
//							,checkLock					//true/false enable check that
//system is unlocked or this user has the lock
//							,lockRequired				//true/false requires this user has the
//lock to proceed
...
//Name
//							,0//,&activeSessionIndex	//acquire user's session index associated with
//the cookieCode
//							))
//					{	//failure
//						//std::cout << out->str() << std::endl; //could print out return string on
//failure 						return;
//					}
//				}
...
//				//xmldoc.outputXmlDocument((std::ostringstream*) out, true); //true to also print to
//std::cout
//			}
//
//
...
bool RemoteWebUsers::xmlRequestToGateway(cgicc::Cgicc& cgi, std::ostringstream* out, HttpXmlDocument* xmldoc,
...
HANDLE_ACCESS_FAILURE:
  ...
  //		__COUT__ << "User " << tmpUsername_ << " is locked out. " << tmpUserWithLock_ << " has lock." <<
  ...
  //		__COUT__ << "User " << tmpUsername_ << " must have lock to proceed. (" << tmpUserWithLock_ << " has lock.)"
  //<< std::endl; 		return false;
  //	}
  //
  //	return true;
...
std::string RemoteWebUsers::getActiveUserList(XDAQ_CONST_CALL xdaq::ApplicationDescriptor* supervisorDescriptor) {
  if (1 || time(0) - ActiveUserLastUpdateTime_ > ACTIVE_USERS_UPDATE_THRESHOLD)  // need to update
  {
    ...
    xoap::MessageReference retMsg =
        ots::SOAPMessenger::sendWithSOAPReply(supervisorDescriptor, "SupervisorGetActiveUsers");
...
bool RemoteWebUsers::getUserInfoForCookie(XDAQ_CONST_CALL xdaq::ApplicationDescriptor* supervisorDescriptor,
                                          std::string& cookieCode, std::string* userName, std::string* displayName,
                                          uint64_t* activeSessionIndex) {
  __COUT__ << std::endl;
  ...
  xoap::MessageReference retMsg = SOAPMessenger::sendWithSOAPReply(supervisorDescriptor, "SupervisorGetUserInfo",
                                                                   SOAPParameters("CookieCode", cookieCode));
...
bool RemoteWebUsers::cookieCodeIsActiveForRequest(XDAQ_CONST_CALL xdaq::ApplicationDescriptor* supervisorDescriptor,
                                                  std::string& cookieCode, uint8_t* userPermissions, std::string ip,
  ...
  //__COUT__ << "CookieCode: " << cookieCode << std::endl;
  xoap::MessageReference retMsg =
      SOAPMessenger::sendWithSOAPReply(supervisorDescriptor, "SupervisorCookieCheck", parameters);
  ...
  return cookieCode.length() ==
         WebUsers::COOKIE_CODE_LENGTH;  // proper cookieCode has length WebUsers::COOKIE_CODE_LENGTH
}
//========================================================================================================================
// sendSystemMessage
//	send system message to toUser through Supervisor
//	toUser wild card * is to all users
void RemoteWebUsers::sendSystemMessage(XDAQ_CONST_CALL xdaq::ApplicationDescriptor* supervisorDescriptor,
                                       const std::string& toUser, const std::string& msg) {
  SOAPParameters parameters;
  parameters.addParameter("ToUser", toUser);
  parameters.addParameter("Message", msg);

  xoap::MessageReference retMsg =
      SOAPMessenger::sendWithSOAPReply(supervisorDescriptor, "SupervisorSystemMessage", parameters);
}

//========================================================================================================================
// makeSystemLogbookEntry
//	make system logbook through Supervisor
void RemoteWebUsers::makeSystemLogbookEntry(XDAQ_CONST_CALL xdaq::ApplicationDescriptor* supervisorDescriptor,
                                            const std::string& entryText) {
  SOAPParameters parameters;
  parameters.addParameter("EntryText", entryText);

  xoap::MessageReference retMsg =
      SOAPMessenger::sendWithSOAPReply(supervisorDescriptor, "SupervisorSystemLogbookEntry", parameters);
}
