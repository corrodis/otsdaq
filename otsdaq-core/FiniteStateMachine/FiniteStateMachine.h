...
class FiniteStateMachine : public toolbox::fsm::FiniteStateMachine {
 public:
  ...
  void addStateTransition(toolbox::fsm::State from, toolbox::fsm::State to, const std::string& input,
                          const std::string& transitionName, OBJECT* obj,
  ...
  template <class OBJECT>
  void addStateTransition(toolbox::fsm::State from, toolbox::fsm::State to, const std::string& input,
                          const std::string& transitionName, const std::string& transitionParameter, OBJECT* obj,
                          void (OBJECT::*func)(toolbox::Event::Reference))
 ...
 protected:
  ...
  // If you don't mark it volatile, the generated code might optimize the value into a registry and your thread will
  // never see the change  The atomicity has nothing to do with the visibility between threads...  just because an
  // operation is executed in one CPU cycle (atomic) it doesn't mean that the result will be visible to the other threads
  ...
  std::map<toolbox::fsm::State, std::map<std::string, std::string, std::less<std::string> > >
