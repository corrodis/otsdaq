...
RunControlStateMachine::RunControlStateMachine(const std::string& name)
    : theStateMachine_(name), asyncFailureReceived_(false), asyncSoftFailureReceived_(false) {
  ...
  RunControlStateMachine::addStateTransition('F', 'H', "Halt", "Halting", this,
                                             &RunControlStateMachine::transitionHalting);
  RunControlStateMachine::addStateTransition('F', 'X', "Shutdown", "Shutting Down", this,
                                             &RunControlStateMachine::transitionShuttingDown);

  RunControlStateMachine::addStateTransition('H', 'C', "Configure", "Configuring", "ConfigurationAlias", this,
                                             &RunControlStateMachine::transitionConfiguring);
  RunControlStateMachine::addStateTransition('H', 'X', "Shutdown", "Shutting Down", this,
                                             &RunControlStateMachine::transitionShuttingDown);
  RunControlStateMachine::addStateTransition('X', 'I', "Startup", "Starting Up", this,
                                             &RunControlStateMachine::transitionStartingUp);

  // Every state can transition to halted
  RunControlStateMachine::addStateTransition('I', 'H', "Initialize", "Initializing", this,
                                             &RunControlStateMachine::transitionInitializing);
  RunControlStateMachine::addStateTransition('H', 'H', "Halt", "Halting", this,
                                             &RunControlStateMachine::transitionHalting);
  RunControlStateMachine::addStateTransition('C', 'H', "Halt", "Halting", this,
                                             &RunControlStateMachine::transitionHalting);
  RunControlStateMachine::addStateTransition('R', 'H', "Abort", "Aborting", this,
                                             &RunControlStateMachine::transitionHalting);
  RunControlStateMachine::addStateTransition('P', 'H', "Abort", "Aborting", this,
                                             &RunControlStateMachine::transitionHalting);

  RunControlStateMachine::addStateTransition('R', 'P', "Pause", "Pausing", this,
                                             &RunControlStateMachine::transitionPausing);
  RunControlStateMachine::addStateTransition('P', 'R', "Resume", "Resuming", this,
                                             &RunControlStateMachine::transitionResuming);
  RunControlStateMachine::addStateTransition('C', 'R', "Start", "Starting", this,
                                             &RunControlStateMachine::transitionStarting);
  RunControlStateMachine::addStateTransition('R', 'C', "Stop", "Stopping", this,
                                             &RunControlStateMachine::transitionStopping);
  RunControlStateMachine::addStateTransition('P', 'C', "Stop", "Stopping", this,
                                             &RunControlStateMachine::transitionStopping);
...
void RunControlStateMachine::reset(void) {
  __COUT__ << "Resetting RunControlStateMachine with name '" << theStateMachine_.getStateMachineName() << "'..."
           << __E__;
...

{
  ...
  // get iteration index
  try {
    StringMacros::getNumber(SOAPUtilities::translate(message).getParameters().getValue("iterationIndex"),
                            iterationIndex_);
  ...
  try {
    StringMacros::getNumber(SOAPUtilities::translate(message).getParameters().getValue("subIterationIndex"),
                            subIterationIndex_);
  ...
  try {
    if (SOAPUtilities::translate(message).getParameters().getValue("retransmission") == "1") {
      ...
      if (lastIterationCommand_ == command && lastIterationIndex_ == iterationIndex_ &&
        ...
        __COUT__ << "Looks like Gateway command '" << command << "' was lost - attempting to handle retransmission."
                 << __E__;
    }
  ...
  if (iterationIndex_ == 0 && subIterationIndex_ == 0) {
    ...
    __COUT__ << "Starting state for " << theStateMachine_.getStateMachineName() << " is " << currentState
             << " and attempting to " << command << std::endl;
  } else {
    currentState = theStateMachine_.getStateName(lastIterationState_);

    __COUT__ << "Iteration index " << iterationIndex_ << "." << subIterationIndex_ << " for "
             << theStateMachine_.getStateMachineName() << " from " << currentState << " attempting to " << command
             << std::endl;
  }
  ...
  if (command == "Error" || command == "Fail") {
    ...
    {
      asyncSoftFailureReceived_ =
          true;  // mark flag, to be used to avoid double pausing and identify pause was due to soft error
  ...
  try {
    ...
    if (iterationIndex_ || subIterationIndex_) {
      ...
      // call inheriting transition function based on last state and command
      {
        ...
        __COUT__ << "Iterating on the transition function from " << currentState << " through " << lastIterationCommand_
                 << __E__;

        auto itFrom = stateTransitionFunctionTable_.find(lastIterationState_);
        if (itFrom == stateTransitionFunctionTable_.end()) {
          __SS__ << "Cannot find transition function from '" << currentState << "' with transition '"
                 << lastIterationCommand_ << "!'" << __E__;
          __COUT_ERR__ << ss.str();
          XCEPT_RAISE(toolbox::fsm::exception::Exception, ss.str());
        }
    ...
    __SS__ << "Run Control Message Handling encountered an unknown error." << theStateMachine_.getErrorMessage()
           << __E__;
    __COUT_ERR__ << ss.str();
    theStateMachine_.setErrorMessage(ss.str());
  ...
  if (currentState == RunControlStateMachine::FAILED_STATE_NAME) {
    result = command + " " + RunControlStateMachine::FAILED_STATE_NAME + ": " + theStateMachine_.getErrorMessage();
    __COUT_ERR__ << "Unexpected Failure state for " << theStateMachine_.getStateMachineName() << " is " << currentState
                 << std::endl;
  ...
  if (!iterationWorkFlag_) theProgressBar_.complete();
