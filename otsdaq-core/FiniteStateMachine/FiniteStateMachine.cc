                                         ...
                                         const xoap::MessageReference& message)
{
	...
	if (transition == "fail")
	{
		...
		while (inTransition_)
		{
			__COUT__ << "Currently in a transition executed from current state " << getProvenanceStateName () << ". Attempting to wait for the transition to complete." << __E__;
			sleep (1);
		}
		sleep (1);
		...
		for (const auto& transitionPair : transitions)
		{
			...
			{
				std::ostringstream error;
				error << "Transition " << transition << " was not executed from current state " << getStateName (getCurrentState ()) << ". There was an error: " << e.what ();
