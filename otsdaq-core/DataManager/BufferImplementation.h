...
class BufferImplementation {
  struct ConsumerStruct {
    ConsumerStruct()
        : priority_(CircularBufferBase::LowConsumerPriority), readPointer_(0), subBuffersStatus_(nullptr) {}
 ...
 public:
  ...
  int read(D*& buffer, H*& header, const std::string& consumer);
  int setReadSubBuffer(const std::string& consumer);  // Must be used in conjunction with attachToEmptySubBuffer because
                                                      // it attach to the nextWritePointer buffer
 ...
 private:
  enum { ErrorBufferFull = -1, ErrorBufferLocked = -2, ErrorBufferNotAvailable = -3, ErrorReadBufferOutOfSync = -4 };
