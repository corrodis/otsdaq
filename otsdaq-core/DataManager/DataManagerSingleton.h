...
class DataManagerSingleton {
 public:
  ...
  template <class C>
  static C* getInstance(const ConfigurationTree& configurationTree, const std::string& supervisorConfigurationPath,
                        const std::string& instanceUID) {
    if (theInstances_.find(instanceUID) == theInstances_.end()) {
      __COUT__ << "Creating supervisor application UID: " << instanceUID << " POINTER: " << theInstances_[instanceUID]
               << std::endl;
 ...
 private:
  ...
  // static std::string composeUniqueName(std::string supervisorContextUID, std::string supervisorApplicationUID){return
