...
class DataManager : public VStateMachine, public Configurable {
 public:
  ...
  void registerProducer(const std::string& bufferUID,
                        DataProducerBase* producer);  // The data manager becomes the owner of the producer object!
  void registerConsumer(const std::string& bufferUID,
                        DataConsumer* consumer);  // The data manager becomes the owner of the consumer object!
 ...
 protected:
  ...
  // void destroyBuffer     		(const std::string& bufferUID);//!!!!!Delete all the pointers of the producers and
  // consumers
 ...
 public:
  bool parentSupervisorHasFrontends_;  // if parent supervisor has front-ends, then allow no producers... that will be
                                       // checked later by parent supervisor
