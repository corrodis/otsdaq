...
class RawDataSaverConsumerBase : public DataConsumer, public Configurable {
 public:
  RawDataSaverConsumerBase(std::string supervisorApplicationUID, std::string bufferUID, std::string processorUID,
 ...
 protected:
  ...
  virtual void writePacketHeader(const std::string& data) {
    ;
  }  // unsigned char quadWordsCount = (data.length()-2)/8; outFile_.write( (char*)&quadWordsCount, 1);}
