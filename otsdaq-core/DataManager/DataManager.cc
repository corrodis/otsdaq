  ...
  *out << "Buffer count: " << buffers_.size() << __E__;
  for (auto& bufferPair : buffers_) {
    *out << "\t"
         ...
         << " producers=" << bufferPair.second.producers_.size() << " consumers=" << bufferPair.second.consumers_.size()
         << __E__;
    ...
    *out << "\t\t"
    ...
    for (auto& producer : bufferPair.second.producers_) {
      *out << "\t\t\t" << producer->getProcessorID() << " ["
           << bufferPair.second.buffer_->getProducerBufferSize(producer->getProcessorID()) << "]" << __E__;
    }
    *out << "\t\t"
         << "Consumers:" << __E__;
...
void DataManager::configure(void) {
  ...
  for (const auto& buffer : theXDAQContextConfigTree_.getNode(theConfigurationPath_ + "/" + COL_NAME_bufferGroupLink)
                                .getChildren())  //"/LinkToDataManagerTable").getChildren())
  {
    ...
    if (buffer.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) {
      ...
      auto bufferConfigurationList =
          buffer.second.getNode(COL_NAME_processorGroupLink).getChildren();  //"LinkToDataBufferTable").getChildren();
      ...
      for (auto& producerLocation : producersVectorLocation) {
        ...
        __CFG_COUT__ << "Creating producer... " << bufferConfigurationList[producerLocation].first << std::endl;
        //				__CFG_COUT__ <<
        //bufferConfigurationMap[producer].getNode(COL_NAME_processorPlugin).getValue<std::string>() << std::endl;
        //				__CFG_COUT__ << bufferConfigurationMap[producer].getNode("LinkToProcessorTable") <<
        //std::endl;
        ...
        try {
          ...
          DataProducerBase* tmpCastCheck = dynamic_cast<DataProducerBase*>(makeDataProcessor(
              ...
              theXDAQContextConfigTree_.getBackNode(theConfigurationPath_)
                  ...
                  .getValue<std::string>(),
              buffer.first, bufferConfigurationList[producerLocation].first, theXDAQContextConfigTree_,
              theConfigurationPath_ + "/" + COL_NAME_bufferGroupLink + "/" + buffer.first + "/" +
                  COL_NAME_processorGroupLink + "/" + bufferConfigurationList[producerLocation].first + "/" +
      ...
      for (auto& consumerLocation : consumersVectorLocation) {
        //				__CFG_COUT__ << theConfigurationPath_ << std::endl;
        //				__CFG_COUT__ << buffer.first << std::endl;
        __CFG_COUT__ << "Creating consumer... " << bufferConfigurationList[consumerLocation].first << std::endl;
        //				__CFG_COUT__ <<
        //bufferConfigurationMap[consumer].getNode(COL_NAME_processorPlugin).getValue<std::string>() << std::endl;
        //				__CFG_COUT__ << bufferConfigurationMap[consumer].getNode("LinkToProcessorTable") <<
        //std::endl;
        ...
        try {
          ...
          DataConsumer* tmpCastCheck = dynamic_cast<DataConsumer*>(makeDataProcessor(
              ...
              theXDAQContextConfigTree_.getBackNode(theConfigurationPath_)
                  .getNode(COL_NAME_appUID)
                  .getValue<std::string>(),
              buffer.first, bufferConfigurationList[consumerLocation].first, theXDAQContextConfigTree_,
              theConfigurationPath_ + "/" + COL_NAME_bufferGroupLink + "/" + buffer.first + "/" +
                  COL_NAME_processorGroupLink + "/" + bufferConfigurationList[consumerLocation].first + "/" +
                  COL_NAME_processorLink));  //));
...
void DataManager::destroyBuffers(void) {
  ...
  for (auto& bufferPair : buffers_) {
    ...
    for (auto& producer : bufferPair.second.producers_) delete producer;
    bufferPair.second.producers_.clear();

    for (auto& consumer : bufferPair.second.consumers_) delete consumer;
...
void DataManager::unregisterFEProducer(const std::string& bufferID, const std::string& feProducerID) {
  ...
  auto bufferIt = buffers_.find(bufferID);
  if (bufferIt == buffers_.end()) {
    __CFG_SS__ << "While Un-Registering FE-producer '" << feProducerID << ",' buffer '" << bufferID << "' not found!"
               << __E__;
    __CFG_SS_THROW__;
  }
  ...
  // just destroy consumer, and it unregisters itself
  for (auto feProducerIt = bufferIt->second.producers_.begin(); feProducerIt != bufferIt->second.producers_.end();
       feProducerIt++) {
...
void DataManager::registerProducer(const std::string& bufferUID, DataProducerBase* producer) {
  __CFG_COUT__ << "Registering producer '" << producer->getProcessorID() << "' to buffer '" << bufferUID << "'..."
               << __E__;
  ...
  if (bufferIt == buffers_.end()) {
    ...
    for (const auto bufferPair : buffers_) ss << bufferPair.first << __E__;
    ss << "\n\n";
...
void DataManager::registerConsumer(const std::string& bufferUID, DataConsumer* consumer) {
  __CFG_COUT__ << "Registering consumer '" << consumer->getProcessorID() << "' to buffer '" << bufferUID << "'..."
               << __E__;
...
void DataManager::startAllBuffers(const std::string& runNumber) {
  for (auto it = buffers_.begin(); it != buffers_.end(); it++) startBuffer(it->first, runNumber);
}

//========================================================================================================================
void DataManager::stopAllBuffers(void) {
  for (auto it = buffers_.begin(); it != buffers_.end(); it++) stopBuffer(it->first);
}

//========================================================================================================================
void DataManager::resumeAllBuffers(void) {
  for (auto it = buffers_.begin(); it != buffers_.end(); it++) resumeBuffer(it->first);
}

//========================================================================================================================
void DataManager::pauseAllBuffers(void) {
  for (auto it = buffers_.begin(); it != buffers_.end(); it++) pauseBuffer(it->first);
}
...
void DataManager::startBuffer(const std::string& bufferUID, std::string runNumber) {
  ...
  for (auto& it : buffers_[bufferUID].consumers_) it->startProcessingData(runNumber);
  for (auto& it : buffers_[bufferUID].producers_) it->startProcessingData(runNumber);
...
void DataManager::stopBuffer(const std::string& bufferUID) {
  ...
  for (auto& it : buffers_[bufferUID].producers_) it->stopProcessingData();
  ...
  if (totalSleepTime < 5000000) totalSleepTime = 5000000;         // At least 5 seconds
  while (!buffers_[bufferUID].buffer_->isEmpty()) {
    usleep(sleepTime);
    timeOut += sleepTime;
    if (timeOut > totalSleepTime) {
      std::cout << "Couldn't flush all buffers! Timing out after " << totalSleepTime / 1000000. << " seconds!"
                << std::endl;
      buffers_[bufferUID].buffer_->isEmpty();
      break;
    }
  }
  std::cout << "Stopping consumers, buffer MUST BE EMPTY. Is buffer empty? " << buffers_[bufferUID].buffer_->isEmpty()
            << std::endl;
  for (auto& it : buffers_[bufferUID].consumers_) it->stopProcessingData();
  buffers_[bufferUID].buffer_->reset();
...
void DataManager::resumeBuffer(const std::string& bufferUID) {
  ...
  for (auto& it : buffers_[bufferUID].consumers_) it->resumeProcessingData();
  for (auto& it : buffers_[bufferUID].producers_) it->resumeProcessingData();
  buffers_[bufferUID].status_ = Running;
}
...
void DataManager::pauseBuffer(const std::string& bufferUID) {
  ...
  for (auto& it : buffers_[bufferUID].producers_) it->pauseProcessingData();
  // Wait until all buffers are flushed
  unsigned int timeOut = 0;
  ...
  while (!buffers_[bufferUID].buffer_->isEmpty()) {
    ...
    timeOut += sleepTime;
    if (timeOut >
        sleepTime * buffers_[bufferUID].buffer_->getTotalNumberOfSubBuffers())  // 1 milliseconds for each buffer!!!!
    {
      std::cout << "Couldn't flush all buffers! Timing out after "
                << buffers_[bufferUID].buffer_->getTotalNumberOfSubBuffers() * sleepTime / 1000000. << " seconds!"
                << std::endl;
      break;
    }
  }
  for (auto& it : buffers_[bufferUID].consumers_) it->pauseProcessingData();
  buffers_[bufferUID].status_ = Initialized;
}
