  ...
  if (begin != std::string::npos) {
    ...
    } else    // else using $NAME syntax
    {
      ...
      for (end = begin + 1; end < data.size(); ++end)
        if (!((data[end] >= '0' && data[end] <= '9') || (data[end] >= 'A' && data[end] <= 'Z') ||
              (data[end] >= 'a' && data[end] <= 'z') || data[end] == '-' || data[end] == '_' || data[end] == '.' ||
              data[end] == ':'))
          break;  // found end
...
//		ignoring whitespace
void StringMacros::getSetFromString(const std::string& inputString, std::set<std::string>& setToReturn,
                                    const std::set<char>& delimiter, const std::set<char>& whitespace) {
  unsigned int i = 0;
  unsigned int j = 0;
...
void StringMacros::getVectorFromString(const std::string& inputString, std::vector<std::string>& listToReturn,
                                       const std::set<char>& delimiter, const std::set<char>& whitespace,
  ...
  for (; c < inputString.size(); ++c) {
    ...
    } else if (isDelimiter)  // delimiter is end of element
    {
      ...
      if (listOfDelimiters && listToReturn.size())  // || foundLeadingDelimiter)) //accept leading delimiter (especially
                                                    // for case of leading negative in math parsing)
      {
    ...
    if (isDelimiter) lastDelimiter = *delimeterSearchIt;
    //__COUTV__(lastDelimiter);
  }
...
//		ignoring whitespace
void StringMacros::getMapFromString(const std::string& inputString, std::map<std::string, std::string>& mapToReturn,
    ...
    if (!needValue)  // finding name
    {
      if ((whitespace.find(inputString[j]) != whitespace.end() ||  // ignore leading white space or delimiter
        ...
        ++i;
                    ...
                    whitespace.end() ||  // trailing white space or delimiter indicates end
    ...
    } else  // finding value
    {
               ...
               i != j)  // assume end of value name
      {
        ...
        auto /*pair<it,success>*/ emplaceReturn = mapToReturn.emplace(std::pair<std::string, std::string>(
            name, validateValueForDefaultStringDataType(inputString.substr(i, j - i))  // value
            ));
  ...
  __SS__ << "Error while extracting a map from the string '" << inputString << "'... is it a valid map?" << __E__
         << e.what() << __E__;
  __SS_THROW__;
}
