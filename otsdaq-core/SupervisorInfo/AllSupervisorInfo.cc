...
//========================================================================================================================
AllSupervisorInfo::AllSupervisorInfo(xdaq::ApplicationContext* applicationContext) : AllSupervisorInfo() {
...
void AllSupervisorInfo::init(xdaq::ApplicationContext* applicationContext) {
  ...
  //						"" /* config app name */,"" /* config parent context name */ //skip configuration
  //info
  //						);
  //
  ...
  for (const auto& descriptor : allDescriptors) {
    SupervisorInfo tempSupervisorInfo(descriptor.second /* descriptor */, "" /* config app name */,
  ...
  const XDAQContextConfiguration* contextConfig = isWizardMode ? 0 : cfgMgr->__GET_CONFIG__(XDAQContextConfiguration);
  ...
  for (const auto& descriptor : allDescriptors) {
    auto /*<iterator,bool>*/ emplacePair = allSupervisorInfo_.emplace(std::pair<unsigned int, SupervisorInfo>(
        ...
        SupervisorInfo(
            descriptor.second /* descriptor */,
            contextConfig ? contextConfig->getApplicationUID(descriptor.second->getContextDescriptor()->getURL(),
                                                             descriptor.second->getLocalId())
                          : "" /* config app name */,
            contextConfig ? contextConfig->getContextUID(descriptor.second->getContextDescriptor()->getURL())
  ...
  //		for(const auto& descriptor:allDescriptors)
  //		{
  //			auto /*<iterator,bool>*/ emplacePair = allSupervisorInfo_.emplace(std::pair<unsigned int,
  //SupervisorInfo>( 					descriptor.second->getLocalId(),//descriptor.first,
  //					SupervisorInfo(
  //							descriptor.second /* descriptor */,
  ...
  if ((!theWizardInfo_ && !theSupervisorInfo_) || (theWizardInfo_ && theSupervisorInfo_)) {
    __SS__ << "Error! Must have one " << XDAQContextConfiguration::GATEWAY_SUPERVISOR_CLASS << " OR one "
           << XDAQContextConfiguration::WIZARD_SUPERVISOR_CLASS << " as part of the context configuration! "
...
//========================================================================================================================
void AllSupervisorInfo::setSupervisorStatus(xdaq::Application* app, const std::string& status) {
  setSupervisorStatus(app->getApplicationDescriptor()->getLocalId(), status);
}
//========================================================================================================================
void AllSupervisorInfo::setSupervisorStatus(const SupervisorInfo& appInfo, const std::string& status) {
  setSupervisorStatus(appInfo.getId(), status);
}
//========================================================================================================================
void AllSupervisorInfo::setSupervisorStatus(const unsigned int& id, const std::string& status) {
      ...
      if (context.status_)
        for (const auto& app : context.applications_) {
          ...
          if (it == app.stateMachineCommandPriority_.end())
            orderedByPriority[XDAQContextConfiguration::XDAQApplication::DEFAULT_PRIORITY].push_back(
                ...
                // XDAQContextConfiguration::XDAQApplication::DEFAULT_PRIORITY
            orderedByPriority[it->second ? it->second : XDAQContextConfiguration::XDAQApplication::DEFAULT_PRIORITY]
                .push_back(app.id_);

          //__COUT__ << "app.id_ " << app.id_ << __E__;
        }
  } catch (...) {
    __COUT_ERR__ << "SupervisorDescriptorInfoBase could not access the XDAQ Context and Application configuration "
                    "through the Configuration Context Group."
                 << __E__;
    throw;
  }
  ...
  for (const auto& priorityAppVector : orderedByPriority) {
    ...
    for (const auto& priorityApp : priorityAppVector.second) {
      ...
      __COUT__ << it->second.getName() << " [LID=" << it->second.getId() << "]: "
               << " priority " << (unsigned int)priorityAppVector.first << " count " << retVec[retVec.size() - 1].size()
               << __E__;
    }
