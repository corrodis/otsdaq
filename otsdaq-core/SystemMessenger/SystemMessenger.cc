...
void SystemMessenger::addSystemMessage(std::string targetUser, std::string msg) {
  ...
  if (sysMsgTargetUser_.size() && sysMsgTargetUser_[sysMsgTargetUser_.size() - 1] == targetUser &&
...
void SystemMessenger::sysMsgCleanup() {
  ...
  // std::cout << __COUT_HDR_FL__ << "Remaining System Messages: " << sysMsgTargetUser_.size() <<  std::endl <<
  // std::endl;
}
