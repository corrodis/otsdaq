...
void DQMHistosOuterTracker::book()
{
	...
	for(const auto& it: consumerList)
	{
		...
		if((dataBufferConfiguration->getConsumerClass(itBuffers,it) == "UDPDataStreamerConsumer") && (it.find("Purdue") != std::string::npos || it.find("OtsUDP") != std::string::npos))
		{
			//FIXME This is very bad since I am getting the PC IP Hardcoded!!!!!!!
			std::string ipAddress = NetworkConverters::nameToStringIP(dataStreamerConsumerConfiguration->getIPAddress(it));
			//FIXME the streamer should have it's own configured port and not fedport +1
			std::string port = NetworkConverters::unsignedToStringPort(dataStreamerConsumerConfiguration->getPort(it));
			planeOccupancies_[ipAddress][port] = std::map<unsigned int, TH1*);
			std::cout << __COUT_HDR_FL__ << "IP: " << NetworkConverters::stringToNameIP(ipAddress) << " Port:----" << NetworkConverters::stringToUnsignedPort(port) << "----" << std::endl;
			std::vector<std::string> interfaceList = feConfiguration->getListOfFEIDs();
			for(const auto& it: interfaceList)
			{
				...
				{
					//const OtsUDPFERConfiguration* OtsUDPFERConfiguration = theConfigurationManager_->__GET_CONFIG__(OtsUDPFERConfiguration);
					const std::vector<std::string> rocList = DetectorToFEConfiguration->getFERROCsList(it);
					std::cout << __COUT_HDR_FL__ << "List of rocs for FER: " << it  << " has size: " << rocList.size() << std::endl;
					for(const auto& itROC: rocList)
					{
						...
						if(ROCType == "FSSR")
						{
							unsigned int fedChannel = DetectorToFEConfiguration->getFERChannel(itROC);
							if(planeOccupancies_[ipAddress][port].find(fedChannel) == planeOccupancies_[ipAddress][port].end())
							{
								name.str("");
								title.str("");
								name << "Plane_FE" << it << "_Channel"<< fedChannel << "_Occupancy";
								title << "Plane FE" << it << " Channel"<< fedChannel << " Occupancy";
								std::cout << __COUT_HDR_FL__ << "Adding:" << name.str() << std::endl;
								planeOccupancies_[ipAddress][port][fedChannel] = new TH1F(name.str().c_str(), title.str().c_str(), 640, -0.5, 639.5);
							}
						}
						else if (ROCType == "VIPIC")
						{
							...
							if(planeOccupancies_[ipAddress][port].find(fedChannel) == planeOccupancies_[ipAddress][port].end())
							{
								name.str("");
								title.str("");
								name << "Plane_FER" << it << "_Channel"<< fedChannel << "_Occupancy";
								title << "Plane FER" << it << " Channel"<< fedChannel << " Occupancy";
								std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Adding:" << name << std::endl;
								planeOccupancies_[ipAddress][port][fedChannel] = new TH2F(name.str().c_str(), title.str().c_str(), 64, -0.5, 63.5, 64, -0.5, 63.5);
							}
						}
					}
				}
...
void DQMHistosOuterTracker::fill(std::string& buffer, std::map<std::string, std::string> header)
{
	...
	//int triggerHigh = 0;
	//int triggerLow = 0;
	//unsigned long  ipAddress = (((header["IPAddress"][0]&0xff)<<24) + ((header["IPAddress"][1]&0xff)<<16) + ((header["IPAddress"][2]&0xff)<<8) + (header["IPAddress"][3]&0xff)) & 0xffffffff;
	...
	while (!convertedBuffer_.empty())
	{
		...
		//					&& planeOccupancies_[ipAddress][port].find(detectorDataVIPIC->getChannelNumber()) != planeOccupancies_[ipAddress][port].end()
		//			)
		//				planeOccupancies_[ipAddress][port][detectorDataVIPIC->getChannelNumber()]->Fill(detectorDataVIPIC->getCol(), detectorDataVIPIC->getRow());
		//			else
		//				std::cout << __COUT_HDR_FL__ << "ERROR: I haven't book histos for streamer " << NetworkConverters::stringToNameIP(ipAddress)
		//			<< " port number: " << NetworkConverters::stringToUnsignedPort(port)
	...
	//      std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Strip: " << detectorData->getStripNumber() << std::endl;
