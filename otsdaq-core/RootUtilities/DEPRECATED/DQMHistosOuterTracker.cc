...
//========================================================================================================================
DQMHistosOuterTracker::DQMHistosOuterTracker(std::string supervisorApplicationUID, std::string bufferUID,
...
void DQMHistosOuterTracker::book() {
  ...
  for (const auto& itInterfaces : interfaceList) {
    ...
    if (feConfiguration->getFEInterfaceType(itInterfaces).find("FSSRInterface") != std::string::npos) {
      const ConfigurationBase* interfaceConfiguration = theConfigurationManager_->getConfigurationByName(
        ...
        if (dataBufferConfiguration->getProducerClass(itBuffers, itProducers) == "UDPDataListenerProducer") {
          const UDPDataListenerProducerConfiguration* listerConfiguration =
              static_cast<const UDPDataListenerProducerConfiguration*>(
                  theConfigurationManager_->getConfigurationByName("UDPDataListenerProducerConfiguration"));
            ...
            for (const auto& itConsumers : consumerList) {
              ...
              if (dataBufferConfiguration->getConsumerClass(itBuffers, itConsumers) == "UDPDataStreamerConsumer") {
                ...
                std::string ipAddress =
                    NetworkConverters::nameToStringIP(dataStreamerConsumerConfiguration->getIPAddress(itConsumers));
                // FIXME the streamer should have it's own configured port and not fedport +1
                std::string port =
                    NetworkConverters::unsignedToStringPort(dataStreamerConsumerConfiguration->getPort(itConsumers));
                planeOccupancies_[ipAddress][port] = std::map<unsigned int, TH1*>();
                std::cout << __COUT_HDR_FL__ << "IP: " << NetworkConverters::stringToNameIP(ipAddress) << " Port:----"
                          << NetworkConverters::stringToUnsignedPort(port) << "----" << std::endl;
                const std::vector<std::string> rocList =
                    detectorToFEConfiguration->getFEReaderDetectorList(itInterfaces);
                std::cout << __COUT_HDR_FL__ << "List of rocs for FER: " << itInterfaces
                ...
                for (const auto& itROC : rocList) {
                  ...
                  if (ROCType == "FSSR") {
                    ...
                    if (planeOccupancies_[ipAddress][port].find(fedChannel) ==
                      ...
                      planeOccupancies_[ipAddress][port][fedChannel] =
  ...
  for(const auto& it: consumerList)
  {
          ...
          if((dataBufferConfiguration->getConsumerClass(itBuffers,it) == "UDPDataStreamerConsumer") &&
(it.find("Purdue") != std::string::npos || it.find("OtsUDP") != std::string::npos))
          {
                  //FIXME This is very bad since I am getting the PC IP Hardcoded!!!!!!!
                  std::string ipAddress =
...
NetworkConverters::unsignedToStringPort(dataStreamerConsumerConfiguration->getPort(it));
                  planeOccupancies_[ipAddress][port] = std::map<unsigned int, TH1*);
                  std::cout << __COUT_HDR_FL__ << "IP: " << NetworkConverters::stringToNameIP(ipAddress) << " Port:----"
<< NetworkConverters::stringToUnsignedPort(port) << "----" << std::endl; std::vector<std::string> interfaceList =
feConfiguration->getListOfFEIDs(); for(const auto& it: interfaceList)
                  {
                          std::cout << __COUT_HDR_FL__ << "Configuring histos for FER: " << it << std::endl;
                          if(feConfiguration->getFEInterfaceType(it).find( "FSSRInterface" ) != std::string::npos)
                          {
                                  //const OtsUDPFERConfiguration* OtsUDPFERConfiguration =
theConfigurationManager_->__GET_CONFIG__(OtsUDPFERConfiguration); const std::vector<std::string> rocList =
DetectorToFEConfiguration->getFERROCsList(it); std::cout << __COUT_HDR_FL__ << "List of rocs for FER: " << it  << " has
size: " << rocList.size() << std::endl; for(const auto& itROC: rocList)
                                  {
                                          std::string ROCType = detectorConfiguration->getROCType(itROC);
                                          //std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "ROC type: " <<
ROCType << std::endl; if(ROCType == "FSSR")
                                          {
                                                  unsigned int fedChannel =
DetectorToFEConfiguration->getFERChannel(itROC); if(planeOccupancies_[ipAddress][port].find(fedChannel) ==
planeOccupancies_[ipAddress][port].end())
                                                  {
                                                          name.str("");
                                                          title.str("");
                                                          name << "Plane_FE" << it << "_Channel"<< fedChannel <<
"_Occupancy"; title << "Plane FE" << it << " Channel"<< fedChannel << " Occupancy"; std::cout << __COUT_HDR_FL__ <<
"Adding:" << name.str() << std::endl; planeOccupancies_[ipAddress][port][fedChannel] = new TH1F(name.str().c_str(),
...
"_Occupancy"; title << "Plane FER" << it << " Channel"<< fedChannel << " Occupancy"; std::cout << __COUT_HDR_FL__ <<
__PRETTY_FUNCTION__ << "Adding:" << name << std::endl; planeOccupancies_[ipAddress][port][fedChannel] = new
TH2F(name.str().c_str(), title.str().c_str(), 64, -0.5, 63.5, 64, -0.5, 63.5);
  ...
  for(unsigned int p=0; p<4; p++)
  {
      ...
      planeOccupancies_.push_back(new TH1I(name.str().c_str(), title.str().c_str(), 640, 0, 640));
  }
   */
...
void DQMHistosOuterTracker::fill(std::string& buffer, std::map<std::string, std::string> header) {
  ...
  // int triggerHigh = 0;
  // int triggerLow = 0;
  // unsigned long  ipAddress = (((header["IPAddress"][0]&0xff)<<24) + ((header["IPAddress"][1]&0xff)<<16) +
  // ((header["IPAddress"][2]&0xff)<<8) + (header["IPAddress"][3]&0xff)) & 0xffffffff;
  std::string ipAddress = header["IPAddress"];
  std::string port = header["Port"];

  // std::cout << __COUT_HDR_FL__ << "Got data from IP: " << NetworkConverters::stringToNameIP(ipAddress) << " port: "
  // << NetworkConverters::stringToUnsignedPort(port) << std::endl;
  ...
  while (!convertedBuffer_.empty()) {
    ...
    if (theDataDecoder_.isFSSRData(convertedBuffer_.front())) {
      ...
      //				std::cout << __COUT_HDR_FL__ << "There is a copy FSSR: " << std::hex <<
      //convertedBuffer_.front() << " counter: " << bufferCounter << std::dec << std::endl;
      ...
      //{
      //    std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Wrong channel: " << detectorData->getChannelNumber()
      //    << std::endl; return;
      //}
      // std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Receiving data from: " << hex << ipAddress << " port: "
      // << port << std::endl;
      if (planeOccupancies_.find(ipAddress) != planeOccupancies_.end() &&
          ...
          planeOccupancies_[ipAddress][port].find(detectorDataFSSR->getChannelNumber()) !=
              planeOccupancies_[ipAddress][port].end())
        planeOccupancies_[ipAddress][port][detectorDataFSSR->getChannelNumber()]->Fill(
            detectorDataFSSR->getSensorStrip());
      else
        std::cout << __COUT_HDR_FL__ << "ERROR: I haven't book histos for streamer "
                  << NetworkConverters::stringToNameIP(ipAddress)
                  << " port number: " << NetworkConverters::stringToUnsignedPort(port)
                  << " channel: " << detectorDataFSSR->getChannelNumber() << " data: " << std::hex
...
profile_->Fill(px, pz);
    ...
    //			theDataDecoder_.decodeData(convertedBuffer_.front(), (DetectorDataBase**)&detectorDataVIPIC);
    //			//if(detectorData->getChannelNumber() >= 4)
    //			//{
    ...
    //			//    return;
    //			//}
    //			//std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Receiving data from: " << hex << ipAddress << "
    //port: " << port << std::endl; 			if (
    //					planeOccupancies_.find(ipAddress) != planeOccupancies_.end()
    //					&& planeOccupancies_[ipAddress].find(port) != planeOccupancies_[ipAddress].end()
    ...
    //			//    }
    //			//std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << processorUID_ << " filling histograms!" <<
    //std::endl;
    //			//  Float_t px, py, pz;
    //			//  const Int_t kUPDATE = 1000;
    //			//   std::cout << __COUT_HDR_FL__ << mthn << "Filling..." << std::endl;
...
//std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Number of Triggers: " << triggerNumber << std::endl;
//if(triggerNumber != numberOfTriggers_->GetEntries())
//  std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "---------------------------- ERROR: different trigger number
" << triggerNumber << " entries: " << numberOfTriggers_->GetEntries() << std::endl;
}
     */
    convertedBuffer_.pop();
    bufferCounter++;
...
if (dynamic_cast<FSSRData*>(detectorData) != 0)
  ...
  //      std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Strip: " << detectorData->getStripNumber() <<
  //      std::endl;
  // std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << "Strip: " << detectorData->getChannelNumber() << std::endl;
}
