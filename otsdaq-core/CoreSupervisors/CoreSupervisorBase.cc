...
CoreSupervisorBase::CoreSupervisorBase(xdaq::ApplicationStub* s)
  ...
  xoap::bind(this, &CoreSupervisorBase::stateMachineErrorMessageRequest, "StateMachineErrorMessageRequest",
             XDAQ_NS_URI);
...
void CoreSupervisorBase::destroy(void) {
  ...
  for (auto& it : theStateMachineImplementation_) delete it;
...
void CoreSupervisorBase::defaultPage(xgi::Input* in, xgi::Output* out) {
  ...
  *out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame src='" << pagess.str()
       << "'></frameset></html>";
}
...

{
  ...
  if (!theRemoteWebUsers_.xmlRequestToGateway(cgiIn, out, &xmlOut, CorePropertySupervisorBase::allSupervisorInfo_,
                                              userInfo))
  ...
  if (!userInfo.automatedCommand_) __SUP_COUT__ << "requestType: " << requestType << __E__;
...
void CoreSupervisorBase::nonXmlRequest(const std::string& requestType, cgicc::Cgicc& cgiIn, std::ostream& out,
                                       const WebUsers::RequestUserInfo& userInfo) {
  __SUP_COUT__ << "This is the empty Core Supervisor non-xml request. Supervisors should override this function."
               << __E__;
...
void CoreSupervisorBase::preStateMachineExecutionLoop(void) {
  ...
  if (RunControlStateMachine::getIterationIndex() == 0 && RunControlStateMachine::getSubIterationIndex() == 0) {
    ...
    for (unsigned int i = 0; i < stateMachinesIterationDone_.size(); ++i) stateMachinesIterationDone_[i] = false;
  } else
    __SUP_COUT__ << "Iteration " << RunControlStateMachine::getIterationIndex() << "."
                 << RunControlStateMachine::getSubIterationIndex() << "(" << subIterationWorkStateMachineIndex_ << ")"
                 << __E__;
}
  ...
  try {
    ...
    for (unsigned int i = 0; i < theStateMachineImplementation_.size(); ++i) {
      ...
      theStateMachineImplementation_[i]
          ->configure();  // e.g. for FESupervisor, this is configure of FEVInterfacesManager
    ...
    throw toolbox::fsm::exception::Exception("Transition Error" /*name*/, ss.str() /* message*/,
                                             "CoreSupervisorBase::transitionConfiguring" /*module*/, __LINE__ /*line*/,
  ...
  try {
    ...
    } else  // if not previously in Failed state, then fail
    {
      ...
      throw toolbox::fsm::exception::Exception("Transition Error" /*name*/, ss.str() /* message*/,
                                               ...
                                               __LINE__ /*line*/, __FUNCTION__ /*function*/
      );
    }
...
void CoreSupervisorBase::transitionStarting(toolbox::Event::Reference e) {
  ...
  try {
    ...
    for (unsigned int i = 0; i < theStateMachineImplementation_.size(); ++i) {
      ...
      preStateMachineExecution(i);
      theStateMachineImplementation_[i]->start(
          runNumber);  // e.g. for FESupervisor, this is transition of FEVInterfacesManager
      postStateMachineExecution(i);
    }
    postStateMachineExecutionLoop();
