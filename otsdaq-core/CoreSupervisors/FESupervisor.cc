...
FESupervisor::FESupervisor(xdaq::ApplicationStub* s) : CoreSupervisorBase(s) {
  ...
  CoreSupervisorBase::theStateMachineImplementation_.push_back(new FEVInterfacesManager(
      CorePropertySupervisorBase::getContextTreeNode(), CorePropertySupervisorBase::supervisorConfigurationPath_));
...
xoap::MessageReference FESupervisor::frontEndCommunicationRequest(xoap::MessageReference message) try {
  ...
  if (type == "feSend") {
    ...
    // mutex scope
    {
      ...
      __SUP_COUT__ << "Number of target interface ID '" << targetInterfaceID << "' buffers: "
                   << theFEInterfacesManager_->frontEndCommunicationReceiveBuffer_[targetInterfaceID].size() << __E__;
      __SUP_COUT__ << "Number of source interface ID '" << requester << "' values received: "
                   << theFEInterfacesManager_->frontEndCommunicationReceiveBuffer_[targetInterfaceID][requester].size()
                   << __E__;
    }
    ...
    try {
      ...
      __SUP_SS__ << "In Supervisor with LID=" << getApplicationDescriptor()->getLocalId() << " the FE Macro named '"
                 << feMacroName << "' with target FE '" << targetInterfaceID << "' failed. Here is the error:\n\n"
                 << e.what() << std::endl;
      __SUP_SS_THROW__;
    } catch (...) {
      __SUP_SS__ << "In Supervisor with LID=" << getApplicationDescriptor()->getLocalId() << " the FE Macro named '"
                 << feMacroName << "' with target FE '" << targetInterfaceID << "' failed due to an unknown error."
                 << __E__;
      __SUP_SS_THROW__;
    }
           ...
           type == "macroMultiDimensionalStart")      // from iterator
  {
    ...
    {
      try {
        theFEInterfacesManager_->startMacroMultiDimensional(requester, targetInterfaceID, macroName, macroString,
                                                            enableSavingOutput, outputFilePath, outputFileRadix,
                                                            inputArgs);
      } catch (std::runtime_error& e) {
        __SUP_SS__ << "In Supervisor with LID=" << getApplicationDescriptor()->getLocalId() << " the Macro named '"
                   << macroName << "' with target FE '" << targetInterfaceID
...
xoap::MessageReference FESupervisor::macroMakerSupervisorRequest(xoap::MessageReference message) {
  ...
  __SUP_COUT__ << "Address: " << addressStr << " Data: " << dataStr << " InterfaceID: " << InterfaceID << std::endl;
  ...
  try {
    if (request == "GetInterfaces") {
      if (theFEInterfacesManager_)
        retParameters.addParameter("FEList", theFEInterfacesManager_->getFEListString(
      ...
      // unsigned int index = stoi(indexStr);	// As long as the supervisor has only one interface, this index will
      // remain 0?
      ...
      unsigned int i = 0;
      for (; i < addressStr.size() && i / 2 < theFEInterfacesManager_->getInterfaceUniversalAddressSize(InterfaceID);
           i += 2) {
      ...
      i = 0;
      for (; i < dataStr.size() && i / 2 < theFEInterfacesManager_->getInterfaceUniversalDataSize(InterfaceID);
           i += 2) {
      ...
      if (dataSz <= 8) {
        ...
        str8.resize(8);
        __SUP_COUT__ << "decResult[" << dataSz << " bytes]: " << *((unsigned long long*)(&str8[0])) << std::endl;
      }
      ...
      if (theFEInterfacesManager_)
        retParameters.addParameter(
            "FEMacros", theFEInterfacesManager_->getFEMacrosString(
                            supervisorApplicationUID_, std::to_string(getApplicationDescriptor()->getLocalId())));
      else
      ...
      try {
        ...
        __SUP_SS__ << "In Supervisor with LID=" << getApplicationDescriptor()->getLocalId() << " the FE Macro named '"
                   << feMacroName << "' with target FE '" << InterfaceID << "' failed. Here is the error:\n\n"
                   << e.what() << std::endl;
        __SUP_COUT_ERR__ << "\n" << ss.str();
        success = false;
        outputArgs = ss.str();
        ...
        __SUP_SS__ << "In Supervisor with LID=" << getApplicationDescriptor()->getLocalId() << " the FE Macro named '"
                   << feMacroName << "' with target FE '" << InterfaceID << "' failed due to an unknown error."
                   << __E__;
        __SUP_COUT_ERR__ << "\n" << ss.str();
        success = false;
        outputArgs = ss.str();
...
xoap::MessageReference FESupervisor::workLoopStatusRequest(xoap::MessageReference message) {
  ...
  return SOAPUtilities::makeSOAPMessageReference((theFEInterfacesManager_->allFEWorkloopsAreDone()
