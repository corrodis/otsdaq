...
FEDataManagerSupervisor::FEDataManagerSupervisor(xdaq::ApplicationStub* s, bool artdaqDataManager) : FESupervisor(s) {
  ...
  if (artdaqDataManager) {
    ...
    CoreSupervisorBase::theStateMachineImplementation_.push_back(DataManagerSingleton::getInstance<ARTDAQDataManager>(
    ...
    CoreSupervisorBase::theStateMachineImplementation_.push_back(DataManagerSingleton::getInstance<DataManager>(
        CorePropertySupervisorBase::getContextTreeNode(), CorePropertySupervisorBase::supervisorConfigurationPath_,
        CorePropertySupervisorBase::supervisorApplicationUID_));
  }
...
void FEDataManagerSupervisor::transitionConfiguring(toolbox::Event::Reference e) {
  ...
  if (theStateMachineImplementation_.size() != 2) {
    __SUP_SS__ << "State machine size is not 2! It is " << theStateMachineImplementation_.size() << ". What happened??"
               << __E__;
    __SUP_SS_THROW__;
  }
