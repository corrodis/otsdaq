    ...
    , propertiesAreSetup_(false)
{
	...
	{
		__SUP_COUT__ << "Wiz mode detected. So skipping configuration location work for supervisor of class '" << supervisorClass_ << "'" << __E__;
	...
	__SUP_COUT__ << "Getting configuration specific info for supervisor '" << (allSupervisorInfo_.getSupervisorInfo(application).getName()) << "' of class " << supervisorClass_ << "." << __E__;
	...
	try
	{
		CorePropertySupervisorBase::supervisorContextUID_ =
		    theConfigurationManager_->__GET_CONFIG__(XDAQContextConfiguration)->getContextUID(application->getApplicationContext()->getContextDescriptor()->getURL());
	}
	catch (...)
	{
		__SUP_COUT_ERR__ << "XDAQ Supervisor could not access it's configuration through the theConfigurationManager_." <<
		    ...
		    ". The getApplicationContext()->getContextDescriptor()->getURL() = " << application->getApplicationContext()->getContextDescriptor()->getURL() << std::endl;
		throw;
	}
	...
	try
	{
		CorePropertySupervisorBase::supervisorApplicationUID_ =
		    theConfigurationManager_->__GET_CONFIG__(XDAQContextConfiguration)->getApplicationUID(application->getApplicationContext()->getContextDescriptor()->getURL(), application->getApplicationDescriptor()->getLocalId());
	}
	catch (...)
	{
		__SUP_COUT_ERR__ << "XDAQ Supervisor could not access it's configuration through the theConfigurationManager_."
				 << " The supervisorContextUID_ = " << supervisorContextUID_ << ". The supervisorApplicationUID = " << supervisorApplicationUID_ << std::endl;
		throw;
	}
...
void CorePropertySupervisorBase::indicateOtsAlive(const CorePropertySupervisorBase* properties)
{
	...
	if (properties)
	{
		unsigned int port = properties->getContextTreeNode().getNode(
									properties->supervisorContextUID_)
					.getNode(
					    "Port")
...
void CorePropertySupervisorBase::checkSupervisorPropertySetup()
{
	...
	if (allSupervisorInfo_.isWizardMode())
		__SUP_COUT__ << "Wiz mode detected. Skipping setup of supervisor properties for supervisor of class '" << supervisorClass_ << "'" << __E__;
	else
	...
	//check cookieCode, sequence, userWithLock, and permissions access all in one shot!
	{
		...
		catch (std::runtime_error& e)
		{
			if (!userInfo.automatedCommand_)
				__SUP_COUT__ << "No explicit permissions threshold for request '" << userInfo.requestType_ << "'... Defaulting to max threshold = " << (unsigned int)userInfo.permissionsThreshold_ << __E__;
		}
