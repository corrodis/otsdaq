...
CorePropertySupervisorBase::CorePropertySupervisorBase(xdaq::Application* application)
    : theConfigurationManager_(new ConfigurationManager),
      ...
      supervisorClassNoNamespace_(supervisorClass_.substr(
          supervisorClass_.find_last_of(":") + 1, supervisorClass_.length() - supervisorClass_.find_last_of(":"))),
  ...
  __SUP_COUT__ << "Getting configuration specific info for supervisor '"
               << (allSupervisorInfo_.getSupervisorInfo(application).getName()) << "' of class " << supervisorClass_
               << "." << __E__;
  ...
  try {
    CorePropertySupervisorBase::supervisorContextUID_ =
        theConfigurationManager_->__GET_CONFIG__(XDAQContextConfiguration)
            ->getContextUID(application->getApplicationContext()->getContextDescriptor()->getURL());
  } catch (...) {
...
void CorePropertySupervisorBase::indicateOtsAlive(const CorePropertySupervisorBase* properties) {
  ...
  if (properties) {
    ...
    if (i != std::string::npos) hostname = hostname.substr(i + 2);
  ...
  std::string filename =
      std::string(getenv("OTSDAQ_LOG_DIR")) + "/otsdaq_is_alive-" + hostname + "-" + portStr + ".dat";
...
void CorePropertySupervisorBase::setSupervisorPropertyDefaults(void) {
  // This can be done in the constructor because when you start xdaq it loads the configuration that can't be changed
  // while running!
  ...
  CorePropertySupervisorBase::setSupervisorProperty(
      CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.UserPermissionsThreshold, "*=1");
...
void CorePropertySupervisorBase::checkSupervisorPropertySetup() {
  ...
  CorePropertySupervisorBase::extractPermissionsMapFromString(
      getSupervisorProperty(CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.UserPermissionsThreshold),
      propertyStruct_.UserPermissionsThreshold);

  propertyStruct_.UserGroupsAllowed.clear();
  StringMacros::getMapFromString(
      getSupervisorProperty(CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.UserGroupsAllowed),
      propertyStruct_.UserGroupsAllowed);

  propertyStruct_.UserGroupsDisallowed.clear();
  StringMacros::getMapFromString(
      getSupervisorProperty(CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.UserGroupsDisallowed),
  ...
  for (auto& property : propertyMap_) __SUP_COUT__ << "\t" << property.first << " = " << property.second << __E__;
}
...
void CorePropertySupervisorBase::loadUserSupervisorProperties(void) {
  ...
  try {
    ...
    __SUP_COUT__ << "No user supervisor property settings found in the configuration tree, going with the defaults."
                 << __E__;
  }
...
WebUsers::permissionLevel_t CorePropertySupervisorBase::getSupervisorPropertyUserPermissionsThreshold(
  ...
  //	{
  //		__SUP_SS__ << "Could not find requestType named " << requestType << " in UserPermissionsThreshold map." <<
  //__E__;
  //		__SS_THROW__; //__SUP_SS_THROW__;
  //	}
  //	return it->second;
...
void CorePropertySupervisorBase::getRequestUserInfo(WebUsers::RequestUserInfo& userInfo) {
  ...
  //__SUP_COUT__ << "userInfo.requestType_ " << userInfo.requestType_ << " files: " << cgiIn.getFiles().size() <<
  //std::endl;

  userInfo.automatedCommand_ = StringMacros::inWildCardSet(
      userInfo.requestType_, propertyStruct_.AutomatedRequestTypes);  // automatic commands should not refresh cookie
                                                                      // code.. only user initiated commands should!
  userInfo.NonXMLRequestType_ = StringMacros::inWildCardSet(
      userInfo.requestType_,
      propertyStruct_.NonXMLRequestTypes);  // non-xml request types just return the request return string to client
  userInfo.NoXmlWhiteSpace_ =
      StringMacros::inWildCardSet(userInfo.requestType_, propertyStruct_.NoXmlWhiteSpaceRequestTypes);
  ...
  // check cookieCode, sequence, userWithLock, and permissions access all in one shot!
  {
    ...
    userInfo.requireLock_ =
        StringMacros::inWildCardSet(userInfo.requestType_, propertyStruct_.RequireUserLockRequestTypes);
    userInfo.allowNoUser_ =
        StringMacros::inWildCardSet(userInfo.requestType_, propertyStruct_.AllowNoLoginRequestTypes);

    userInfo.permissionsThreshold_ = -1;  // default to max
    try {
      userInfo.permissionsThreshold_ =
          CorePropertySupervisorBase::getSupervisorPropertyUserPermissionsThreshold(userInfo.requestType_);
    ...
    try {
      StringMacros::getSetFromString(
          StringMacros::getWildCardMatchFromMap(userInfo.requestType_, propertyStruct_.UserGroupsAllowed),
      ...
      //					 userInfo.requestType_ << "'... Defaulting to empty groups allowed. " <<
      //__E__;
    }
    try {
      StringMacros::getSetFromString(
          StringMacros::getWildCardMatchFromMap(userInfo.requestType_, propertyStruct_.UserGroupsDisallowed),
      ...
      //					 userInfo.requestType_ << "'... Defaulting to empty groups disallowed. "
      //<< __E__;
    }
