...
HttpXmlDocument WorkLoopManager::processRequest(std::string workLoopName, const xoap::MessageReference* message) {
  ...
  if (message != 0) workLoops_[requestNumber].message = *message;
  try {
...
bool WorkLoopManager::report(toolbox::task::WorkLoop* workLoop, std::string result, float progress, bool status) {
  ...
  /*
      __COUT__ << "Reporting result for request " << getWorkLoopRequest(workLoop) << " with req#: " << requestNumber <<
     std::endl; if(workLoops_.find(requestNumber) == workLoops_.end())
      {
...
bool WorkLoopManager::getRequestResult(cgicc::Cgicc& cgi, HttpXmlDocument& xmlDocument) {
  ...
  __COUT__ << "Request: " << requestName_ << " RequestNumber=" << requestNumberString << " assigned # " << requestNumber
           << std::endl;

  if (workLoops_.find(requestNumber) == workLoops_.end()) {
    reportStream << "Can't find request " << requestNumber << " within the currently active " << workLoops_.size()
                 << " requests!";
...
bool WorkLoopManager::removeWorkLoop(RequestNumber requestNumber) {
  ...
  try {
    ...
    // ATTENTION!
    // If the workloop job thread returns false, then the workloop job is automatically removed and it can't be removed
    // again Leaving this try catch allows me to be general in the job threads so I can return true (repeat loop) or
    // false ( loop only once) without crashing
  ...
  toolbox::task::getWorkLoopFactory()->removeWorkLoop(
      ...
      cWorkLoopType_);  // delete workLoops_[requestNumber].workLoop; is done by the factory
  workLoops_.erase(requestNumber);
  return true;
}
  ...
  for (; it != workLoops_.end(); it++)
    if (it->second.done && difftime(now, it->second.requestLastTimeChecked) > timeOutInSeconds) {
      __COUT__ << "Removing timed out request #" << it->first << " after "
               << difftime(now, it->second.requestLastTimeChecked) << " seconds." << std::endl;
      removeWorkLoop(it->first);
...
std::string WorkLoopManager::composeWorkLoopName(RequestNumber requestNumber, cgicc::Cgicc& cgi) {
  ...
  __COUT__ << "Request: " << requestName_ << " Value=" << cgi.getElement(requestName_)->getValue()
...
std::string WorkLoopManager::composeWorkLoopName(RequestNumber requestNumber, const xoap::MessageReference& message) {
  ...
  __COUT__ << "Request: " << requestName_ << " Value=" << soapCommand.getCommand() << " WLName: " << name.str()
           << std::endl;
  if (soapCommand.hasParameters()) {
    name << '<';
    char separator = ',';
    for (SOAPParameters::const_iterator it = soapCommand.getParameters().begin();
         it != soapCommand.getParameters().end(); it++) {
      if (it != soapCommand.getParameters().begin()) name << separator;
...
std::string WorkLoopManager::getWorkLoopRequest(std::string workLoopName) {
  return workLoopName.substr(workLoopName.find('-') + 1,
                             workLoopName.find(std::string("/") + cWorkLoopType_) - workLoopName.find('-') - 1);
}
...
void WorkLoopManager::translateWorkLoopName(toolbox::task::WorkLoop* workLoop, SOAPCommand& soapCommand) {
  ...
  if (request.find('<') == std::string::npos) {
    ...
    while ((commaPosition = request.find(',', begin)) != std::string::npos) {
      ...
      soapCommand.setParameter(request.substr(begin, orPosition - begin),
