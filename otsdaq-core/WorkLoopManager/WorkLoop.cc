...
WorkLoop::~WorkLoop(void) {
  ...
  if (stopWorkLoop()) toolbox::task::getWorkLoopFactory()->removeWorkLoop(workLoopName_, workLoopType_);
...
void WorkLoop::startWorkLoop(void) {
  ...
  if (workLoop_->isActive())
    return;  // This might be a consumer producer running at the same time so it has been activated already
  ...
  try {
    ...
    __COUT__ << "ERROR: Can't activate WorkLoop job for " << workLoopName_ << " Very likely because the name "
             << workLoopName_ << " is not unique!" << __E__;
    stopWorkLoop();
  }
}  // end startWorkLoop()
...
bool WorkLoop::stopWorkLoop() {
  ...
  continueWorkLoop_ = false;
  if (workLoop_ == 0) {
    __COUT__ << "MEASSAGE: WorkLoop " << workLoopName_
