...
//========================================================================================================================
FESlowControlsChannel::FESlowControlsChannel(const std::string &interfaceUID, const std::string &channelName,
                                             const std::string &dataType, unsigned int universalDataSize,
                                             unsigned int universalAddressSize, const std::string &universalAddress,
                                             unsigned int universalDataBitOffset,

                                             bool readAccess, bool writeAccess, bool monitoringEnabled,
                                             bool recordChangesOnly, time_t delayBetweenSamples, bool saveEnabled,
                                             const std::string &savePath, const std::string &saveFileRadix,
                                             bool saveBinaryFormat, bool alarmsEnabled, bool latchAlarms,
                                             const std::string &lolo, const std::string &lo, const std::string &hi,
    ...
    : interfaceUID_(interfaceUID),
      ...
      hiAlarmed_(false),
      hihiAlarmed_(false),
      saveFullFileName_(savePath_ + "/" + saveFileRadix_ + "-" + underscoreString(fullChannelName_) + "-" +
  ...
  if (dataType_[dataType_.size() - 1] == 'b')  // if ends in 'b' then take that many bits
    ...
    __SS__ << "Data type '" << dataType_ << "' is invalid. "
           ...
           << "#b (# bits)"
           << ", char (" << sizeof(char) << "B), unsigned char (" << sizeof(unsigned char) << "B), short ("
           << sizeof(short) << "B), unsigned short (" << sizeof(unsigned short) << "B), int (" << sizeof(int)
           << "B), unsigned int (" << sizeof(unsigned int) << "B), long long (" << sizeof(long long)
           << "B), unsigned long long (" << sizeof(unsigned long long) << "B), float (" << sizeof(float)
  ...
  if (sizeOfDataTypeBits_ > 64) {
    __SS__ << "Invalid Data Type '" << dataType_ << "' (" << sizeOfDataTypeBits_
           << "-bits)"
  ...
  if (universalDataSize * 8 < sizeOfDataTypeBits_) {
    __SS__ << "Invalid Data Type '" << dataType_ << "' (" << sizeOfDataTypeBits_ << "-bits) or Universal Data Size of "
           << universalDataSize * 8 << "-bits. Data Type size must be less than or equal to Universal Data Size."
           << std::endl;
    __COUT_ERR__ << "\n" << ss.str();
    __SS_THROW__;
  }
...
void FESlowControlsChannel::convertStringToBuffer(const std::string &inString, std::string &buffer, bool useDataType) {
  ...
  if (inString.size()) {
    ...
    } else  // treat as decimal value
    {
      ...
      for (unsigned int i = 0; i < sizeof(long long) && i < buffer.size(); ++i) buffer[i] = ((char *)&val)[i];
...
void FESlowControlsChannel::handleSample(const std::string &universalReadValue, std::string &txBuffer,
  ...
  // check alarms
  if (alarmsEnabled_) alarmMask = checkAlarms(txBuffer);
    ...
    if (aggregateIsBinaryFormat) {
      ...
      if (alarmMask)  // if any alarms
        for (time_t i = 1; i < 5; ++i, alarmMask >>= 1)
          if (alarmMask & 1) {
            {
              fwrite(&i, sizeof(lastSampleTime_), 1,
                     fpAggregate);  //	8B save any alarms by marking with time = 1, 2, 3, 4
          ...
          if (alarmMask & checkMask) {
            ...
            if (dataType_[dataType_.size() - 1] == 'b')  // if ends in 'b' then take that many bits
            {
              ...
              for (unsigned int j = 0; j < (*alarmValueArray[i - 1]).size(); ++i)
                ss << std::hex << (int)(((*alarmValueArray[i - 1])[j] >> 4) & 0xF)
...
void FESlowControlsChannel::extractSample(const std::string &universalReadValue) {
  ...
  {  // print
    ...
    for (unsigned int i = 0; i < universalReadValue.size(); ++i)
      ss << std::hex << (int)((universalReadValue[i] >> 4) & 0xF) << (int)((universalReadValue[i]) & 0xF) << " "
         << std::dec;
    ss << std::endl;
    __COUT__ << "\n" << ss.str();
  }
  ...
  if (bitOffset)  // copy last partial byte
  {
    ...
    tmp |= (((unsigned long long)(universalReadValue[byteOffset])) & (0xFF >> (8 - bitsLeft)))
  ...
  for (unsigned int i = 0; i < (sizeOfDataTypeBits_ / 8 + ((universalDataBitOffset_ % 8) ? 1 : 0)); ++i)
...
void FESlowControlsChannel::clearAlarms(int targetAlarm) {
  if (targetAlarm == -1 || targetAlarm == 0) loloAlarmed_ = false;
  if (targetAlarm == -1 || targetAlarm == 1) loAlarmed_ = false;
  if (targetAlarm == -1 || targetAlarm == 2) hiAlarmed_ = false;
  if (targetAlarm == -1 || targetAlarm == 3) hihiAlarmed_ = false;
}
