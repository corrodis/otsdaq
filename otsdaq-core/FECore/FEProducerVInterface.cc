...
FEProducerVInterface::FEProducerVInterface(const std::string& interfaceUID,
    ...
    : FEVInterface(interfaceUID, theXDAQContextConfigTree, interfaceConfigurationPath),
      DataProducerBase(theXDAQContextConfigTree.getBackNode(interfaceConfigurationPath, 4).getValueAsString(),
                       theXDAQContextConfigTree.getNode(interfaceConfigurationPath + "/" + "LinkToDataBufferTable", 4)
                           .getValueAsString(),
                       interfaceUID /*processorID*/, 100 /*bufferSize*/) {
...
FEProducerVInterface::~FEProducerVInterface(void) {
  ...
  __COUT__ << "FEProducer '" << DataProcessor::processorUID_
           << "' is unregistering from DataManager Supervisor Buffer '" << DataProcessor::supervisorApplicationUID_
...
//			std::string buffer;
//			buffer.resize(8); //NOTE: this is inexpensive according to Lorenzo/documentation in C++11 (only
//increases size once and doesn't decrease size) 			memcpy((void *)&buffer /*dest*/,(void *)&value
///*src*/, 8 /*numOfBytes*/);
//
...
void FEProducerVInterface::copyToNextBuffer(const std::string& dataToWrite) {
  ...
  //	__FE_COUT__ << "Have next buffer size " << FEProducerVInterface::dataP_->size() << __E__;
  //
  //
  ...
  //			BinaryStringMacros::binaryTo8ByteHexString(*FEProducerVInterface::dataP_) << __E__;
  //
  //
  //	FEProducerVInterface::writeCurrentBuffer();
