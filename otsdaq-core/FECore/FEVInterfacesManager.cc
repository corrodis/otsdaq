...
//========================================================================================================================
FEVInterfacesManager::FEVInterfacesManager(const ConfigurationTree& theXDAQContextConfigTree,
...
void FEVInterfacesManager::destroy(void) {
  for (auto& it : theFEInterfaces_) it.second.reset();
...
void FEVInterfacesManager::createInterfaces(void) {
  ...
  for (const auto& interface : feChildren) {
    ...
    __CFG_COUT__ << "Interface Plugin Name: " << interface.second.getNode(COL_NAME_fePlugin).getValue<std::string>()
                 << __E__;
    __CFG_COUT__ << "Interface Name: " << interface.first << __E__;
    __CFG_COUT__ << "XDAQContext Node: " << theXDAQContextConfigTree_ << __E__;
    __CFG_COUT__ << "Path to configuration: "
                 << (theConfigurationPath_ + "/" + COL_NAME_feGroupLink + "/" + interface.first + "/" +
                     COL_NAME_feTypeLink)
                 << __E__;

    try {
      theFEInterfaces_[interface.first] = makeInterface(
          interface.second.getNode(COL_NAME_fePlugin).getValue<std::string>(), interface.first,
      ...
      __CFG_SS__ << "Failed to instantiate plugin named '" << interface.first << "' of type '"
                 << interface.second.getNode(COL_NAME_fePlugin).getValue<std::string>() << "' due to an unknown error."
                 << __E__;
      __MOUT_ERR__ << ss.str();
...
void FEVInterfacesManager::resume(void) {
  ...
  for (unsigned int i = 0; i < theFENamesByPriority_.size(); ++i) {
    ...
    if (postStateMachineExecution(i)) fe->startWorkLoop();
...
void FEVInterfacesManager::startMacroMultiDimensional(const std::string& requester, const std::string& interfaceID,
                                                      const std::string& macroName, const std::string& macroString,
                                                      const bool enableSavingOutput, const std::string& outputFilePath,
  ...
  __CFG_COUT__ << "Starting multi-dimensional Macro '" << macroName << "' for interface '" << interfaceID << ".'"
               << __E__;
  ...
  // start thread
  std::thread(
      [](FEVInterfacesManager* feMgr, const std::string interfaceID, const std::string macroName,
         const std::string macroString, const bool enableSavingOutput, const std::string outputFilePath,
         const std::string outputFileRadix, const std::string inputArgsStr) {
        ...
        try {
          ...
          if (enableSavingOutput) {
            std::string filename =
                outputFilePath + "/" + outputFileRadix + macroName + "_" + std::to_string(time(0)) + ".txt";
          ...
          //	inputs:
          //		- inputArgs: dimensional semi-colon-separated,
          //			comma separated: dimension iterations and arguments (colon-separated name/value/stepsize
          //sets)
          ...
          using longParamMap_t =
              std::map<std::string /*name*/,
                       std::pair<long /*current value*/, std::pair<long /*initial value*/, long /*step value*/>>>;
          ...
          if (0)  // example
          {
            ...
            longDimensionParameters.back().emplace(std::make_pair(
                "myOtherArg",
                std::make_pair(3 /*current value*/, std::make_pair(3 /*initial value*/, 4 /*step value*/))));
          }  // end example
          ...
          if (dimensionArgs.size() == 0) {
            ...
            for (unsigned int d = 0; d < dimensionArgs.size(); ++d) {
              ...
              for (unsigned int a = 1; a < args.size(); ++a) {
                ...
                if ((argPieces[1].size() &&
                     (argPieces[1][argPieces[1].size() - 1] == 'f' || argPieces[1].find('.') != std::string::npos)) ||
                    (argPieces[2].size() &&
                     (argPieces[2][argPieces[2].size() - 1] == 'f' || argPieces[2].find('.') != std::string::npos))) {
                  // handle as double
                  ...
                  __GEN_SS__ << "Error! Only integer aruments allowed for Macros. "
                             << "Double style arugment found: " << argPieces[0] << "' := " << startValue << ", "
                             << stepSize << __E__;
                  __GEN_SS_THROW__;
                  //							doubleDimensionParameters.back().emplace(
                  //									std::make_pair(argPieces[0],
                  //											std::make_pair( startValue /*current
                  //value*/,
                  //													std::make_pair( startValue /*initial
                  //value*/,
                  //															stepSize /*step
                  //value*/))));
                } else {
                  ...
                  __GEN_COUT__ << "Creating long argument '" << argPieces[0] << "' := " << startValue << ", "
                               << stepSize << __E__;

                  longDimensionParameters.back().emplace(std::make_pair(
                      argPieces[0],
                      std::make_pair(startValue /*current value*/,
                                     std::make_pair(startValue /*initial value*/, stepSize /*step value*/))));
                }
          ...
          if (dimensionIterations.size() != longDimensionParameters.size()) {
            __GEN_SS__ << "Impossible vector size mismatch! " << dimensionIterations.size() << " - "
          ...
          // output header
          {
            std::stringstream outSS;
            {
              ...
              for (unsigned int i = 0; i < dimensionIterations.size(); ++i) {
                outSS << "\t\t"
                      ...
                      << (longDimensionParameters[i].size()) << " arguments." << __E__;
              ...
              for (const auto& inputArgName : macro.namesOfInputArguments_) outSS << "\t" << inputArgName << __E__;
              outSS << "\nOutput argument names:" << __E__;
              for (const auto& outputArgName : macro.namesOfOutputArguments_) outSS << "\t" << outputArgName << __E__;
            ...
            __GEN_COUT__ << "\n" << outSS.str();
            if (outputFilePointer) fprintf(outputFilePointer, outSS.str().c_str());
          }  // end output header
          ...
          std::function<void(const unsigned int /*dimension*/
              ...
              localRecurse = [&dimensionIterations, &dimensionIterationCnt, &iterationCount, &longDimensionParameters,
                              &fe, &macro, &variableMap, &outputFilePointer,
                  ...
                  // set argsIn to current value
                  {
                    ...
                    for (unsigned int j = 0; j < dimensionIterations.size(); ++j) {
                      for (auto& longParam : longDimensionParameters[j]) {
                        __GEN_COUT__ << "Assigning argIn '" << longParam.first << "' to current long value '"
                                     << longParam.second.first << "' from dimension " << j << " parameter." << __E__;
                ...
                if (dimension >= dimensionIterationCnt.size()) dimensionIterationCnt.push_back(0);
                ...
                for (dimensionIterationCnt[dimension] = 0;  // reset each time through dimension loop
                  ...
                  __GEN_COUT__ << "dimension[" << dimension << "] index := " << dimensionIterationCnt[dimension]
                               << __E__;
                  ...
                  // update current value to next value for this dimension's parameters
                  {
                    for (auto& longPair : longDimensionParameters[dimension]) {
                      ...
                      __GEN_COUT__ << "arg '" << longPair.first << "' current value: " << longPair.second.first
                                   << __E__;
                    }  // end long loop
                ...
                __GEN_COUT__ << "Completed dimension[" << dimension
                             << "] number of iterations := " << dimensionIterationCnt[dimension] << " of "
          ...
          // close output file
          if (outputFilePointer) fclose(outputFilePointer);
  ...
  __CFG_COUT__ << "Started multi-dimensional Macro '" << macroName << "' for interface '" << interfaceID << ".'"
               << __E__;
...
//	outputs:
//		- throws exception on failure
void FEVInterfacesManager::startFEMacroMultiDimensional(const std::string& requester, const std::string& interfaceID,
                                                        const std::string& feMacroName, const bool enableSavingOutput,
  ...
  __CFG_COUT__ << "Starting multi-dimensional FE Macro '" << feMacroName << "' for interface '" << interfaceID << ".'"
               << __E__;
  __CFG_COUTV__(inputArgs);
  ...
  // start thread
  std::thread(
      [](FEVInterfacesManager* feMgr, const std::string interfaceID, const std::string feMacroName,
         const bool enableSavingOutput, const std::string outputFilePath, const std::string outputFileRadix,
        ...
        try {
          ...
          if (FEMacroIt == fe->getMapOfFEMacroFunctions().end()) {
            __GEN_SS__ << "FE Macro '" << feMacroName << "' of interfaceID '" << interfaceID << "' was not found!"
                       << __E__;
            __GEN_SS_THROW__;
          }
          ...
          if (enableSavingOutput) {
            std::string filename =
                outputFilePath + "/" + outputFileRadix + feMacroName + "_" + std::to_string(time(0)) + ".txt";
            __GEN_COUT__ << "Opening file... " << filename << __E__;
          ...
          using doubleParamMap_t =
              std::map<std::string /*name*/,
                       std::pair<double /*current value*/, std::pair<double /*initial value*/, double /*step value*/>>>;
            ...
            for (unsigned int d = 0; d < dimensionArgs.size(); ++d) {
              ...
              for (unsigned int a = 1; a < args.size(); ++a) {
                ...
                if ((argPieces[1].size() &&
                  ...
                  __GEN_COUT__ << "Creating double argument '" << argPieces[0] << "' := " << startValue << ", "
                               << stepSize << __E__;
          ...
          if (dimensionIterations.size() != longDimensionParameters.size() ||
            ...
            __GEN_SS__ << "Impossible vector size mismatch! " << dimensionIterations.size() << " - "
                       << longDimensionParameters.size() << " - " << doubleDimensionParameters.size() << __E__;
            __GEN_SS_THROW__;
          }
          ...
          // output header
          {
            std::stringstream outSS;
            {
              ...
              for (unsigned int i = 0; i < dimensionIterations.size(); ++i) {
                outSS << "\t\t"
                      << "dimension[" << i << "] has " << dimensionIterations[i] << " iterations and "
                      << (longDimensionParameters[i].size() + doubleDimensionParameters[i].size()) << " arguments."
                      << __E__;
          ...
          std::function<void(const unsigned int /*dimension*/
              ...
              localRecurse = [&dimensionIterations, &dimensionIterationCnt, &iterationCount, &longDimensionParameters,
                              &doubleDimensionParameters, &fe, &feMacro, &outputFilePointer, &argsIn, &argsOut,
                              &localRecurse](const unsigned int dimension) {
                ...
                if (dimension >= dimensionIterations.size()) {
                  ...
                  for (unsigned int i = 0; i < argsIn.size(); ++i) {
                    ...
                    for (unsigned int j = 0; j < dimensionIterations.size(); ++j) {
                      ...
                      if (longIt == longDimensionParameters[j].end()) continue;

                      // else found long!
                      __GEN_COUT__ << "Assigning argIn '" << argsIn[i].first << "' to current long value '"
                                   << longIt->second.first << "' from dimension " << j << " parameter." << __E__;
                    ...
                    for (unsigned int j = 0; j < dimensionIterations.size(); ++j) {
                      ...
                      if (doubleIt == doubleDimensionParameters[j].end()) continue;

                      // else found long!
                      __GEN_COUT__ << "Assigning argIn '" << argsIn[i].first << "' to current double value '"
                                   << doubleIt->second.first << "' from dimension " << j << " parameter." << __E__;
                    ...
                    __GEN_SS__ << "ArgIn '" << argsIn[i].first << "' was not assigned a value "
                               << "by any dimensional loop parameter sets. This is illegal. FEMacro '"
                               << feMacro.feMacroName_ << "' requires '" << argsIn[i].first
                               << "' as an input argument. Either remove the input argument from this FEMacro, "
                               << "or define a value as a dimensional loop parameter." << __E__;
                    __GEN_SS_THROW__;
                  }  // done building argsIn
                  ...
                  // output results
                  {
                    std::stringstream outSS;
                    {
                      ...
                      for (auto& argIn : argsIn) outSS << "\t\t" << argIn.first << " = " << argIn.second << __E__;

                      outSS << "\n"
                            << "\t"
                            << "Output arguments (count: " << argsOut.size() << "):" << __E__;
                      for (auto& argOut : argsOut) outSS << "\t\t" << argOut.first << " = " << argOut.second << __E__;
                    }  // end outputs stringstream results
                ...
                // update current value to initial value for this dimension's parameters
                {
                  ...
                  for (auto& doublePair : doubleDimensionParameters[dimension]) {
                    ...
                    __GEN_COUT__ << "arg '" << doublePair.first << "' current value: " << doublePair.second.first
                                 << __E__;
                  }  // end double loop
                }    // end update current value to initial value for all dimensional parameters
  ...
  __CFG_COUT__ << "Started multi-dimensional FE Macro '" << feMacroName << "' for interface '" << interfaceID << ".'"
               << __E__;
...
bool FEVInterfacesManager::checkMacroMultiDimensional(const std::string& interfaceID, const std::string& macroName) {
  ...
  if (statusIt == macroMultiDimensionalStatusMap_.end()) {
    __CFG_SS__ << "Status missing for multi-dimensional launch of Macro '" << macroName << "' for interface '"
               << interfaceID << ".'" << __E__;
    __CFG_SS_THROW__;
  } else if (statusIt->second == "Done") {
    __CFG_COUT__ << "Completed multi-dimensional launch of Macro '" << macroName << "' for interface '" << interfaceID
                 << ".'" << __E__;
    ...
    __CFG_COUT__ << "Still running multi-dimensional launch of Macro '" << macroName << "' for interface '"
                 << interfaceID << ".'" << __E__;
    return false;
  }
  // else //assume error

  __CFG_SS__ << "Error occured during multi-dimensional launch of Macro '" << macroName << "' for interface '"
             << interfaceID << "':" << statusIt->second << __E__;
  __CFG_SS_THROW__;
...
void FEVInterfacesManager::runFEMacroByFE(const std::string& callingInterfaceID, const std::string& interfaceID,
  ...
  if (!StringMacros::inWildCardSet(callingInterfaceID, allowedFEsSet)) {
    __CFG_SS__ << "FE Macro '" << feMacroName << "' of interfaceID '" << interfaceID
               << "' does not allow access to calling interfaceID '" << callingInterfaceID
...
//		- throws exception on failure
//		- outputArgs: colon-separate name/value pairs, and then comma-separated
void FEVInterfacesManager::runFEMacro(const std::string& interfaceID, const std::string& feMacroName,
...
void FEVInterfacesManager::runFEMacro(const std::string& interfaceID,
  ...
  for (unsigned int i = 0; i < argsIn.size(); ++i)
    if (argsIn[i].first != feMacro.namesOfInputArguments_[i]) {
      __CFG_SS__ << "FE Macro '" << feMacro.feMacroName_ << "' of interfaceID '" << interfaceID
                 << "' was attempted with a mismatch in"
                 << " a name of an input argument. " << argsIn[i].first << " was given. "
                 << feMacro.namesOfInputArguments_[i] << " expected." << __E__;
      __CFG_COUT_ERR__ << "\n" << ss.str();
      __CFG_SS_THROW__;
    }
  ...
  if (feMacro.namesOfOutputArguments_.size() != argsOut.size()) {
    __CFG_SS__ << "FE Macro '" << feMacro.feMacroName_ << "' of interfaceID '" << interfaceID
               ...
               << feMacro.namesOfOutputArguments_.size() << " expected." << __E__;
    __CFG_COUT_ERR__ << "\n" << ss.str();
    __CFG_SS_THROW__;
  }
  for (unsigned int i = 0; i < argsOut.size(); ++i)
    if (argsOut[i].first != feMacro.namesOfOutputArguments_[i]) {
      __CFG_SS__ << "FE Macro '" << feMacro.feMacroName_ << "' of interfaceID '" << interfaceID
                 << "' was attempted with a mismatch in"
                 << " a name of an output argument. " << argsOut[i].first << " were given. "
                 << feMacro.namesOfOutputArguments_[i] << " expected." << __E__;
      __CFG_COUT_ERR__ << "\n" << ss.str();
      __CFG_SS_THROW__;
    }

  __CFG_COUT__ << "# of input args = " << argsIn.size() << __E__;
  for (auto& argIn : argsIn) __CFG_COUT__ << argIn.first << ": " << argIn.second << __E__;
  ...
  __CFG_COUT__ << "# of output args = " << argsOut.size() << __E__;
  for (const auto& arg : argsOut) __CFG_COUT__ << arg.first << ": " << arg.second << __E__;
...
std::string FEVInterfacesManager::getFEMacrosString(const std::string& supervisorName,
  ...
  for (const auto& it : theFEInterfaces_) {
    ...
    retList +=
        supervisorName + ":" + supervisorLid + ":" + it.second->getInterfaceType() + ":" + it.second->getInterfaceUID();
    ...
    for (const auto& macroPair : it.second->getMapOfFEMacroFunctions()) {
      ...
      for (const auto& name : macroPair.second.namesOfInputArguments_) retList += ":" + name;

      retList += ":" + std::to_string(macroPair.second.namesOfOutputArguments_.size());
      for (const auto& name : macroPair.second.namesOfOutputArguments_) retList += ":" + name;
    }
...
void FEVInterfacesManager::preStateMachineExecutionLoop(void) {
  ...
  if (VStateMachine::getIterationIndex() == 0 && VStateMachine::getSubIterationIndex() == 0) {
    ...
    for (const auto& FEPair : theFEInterfaces_) stateMachinesIterationDone_[FEPair.first] = false;  // init to not done
  } else
...
void FEVInterfacesManager::preStateMachineExecution(unsigned int i) {
  ...
  __CFG_COUT__ << "theStateMachineImplementation Iteration " << fe->VStateMachine::getIterationIndex() << "."
