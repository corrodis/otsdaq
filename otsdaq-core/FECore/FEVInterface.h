...
//	The features include configuration hooks, finite state machine handlers, Front-end Macros for web accessible C++
//handlers, slow controls hooks, as well as universal write and read for 	Macro Maker compatibility.
//
...
class FEVInterface : public VStateMachine, public WorkLoop, public Configurable {
 public:
  FEVInterface(const std::string& interfaceUID, const ConfigurationTree& theXDAQContextConfigTree,
  ...
  bool running(void) { /*while(WorkLoop::continueWorkLoop_){;}*/
    return false;
  }
  ...
  // using	frontEndMacroConstArg_t	= std::pair<const std::string /* input arg name */ , const std::string /* arg
  // input value */ >;
  using frontEndMacroConstArgs_t = const std::vector<frontEndMacroArg_t>&;
  using frontEndMacroFunction_t = void (ots::FEVInterface::*)(
      frontEndMacroConstArgs_t, frontEndMacroArgs_t);  // void function (vector-of-inputs, vector-of-outputs)
  struct frontEndMacroStruct_t                         // members fully define a front-end macro function
  {
    frontEndMacroStruct_t(
        const std::string& feMacroName, const frontEndMacroFunction_t& feMacroFunction,
        const std::vector<std::string>& namesOfInputArgs, const std::vector<std::string>& namesOfOutputArgs,
  ...
  struct macroStruct_t {
    ...
    std::vector<std::pair<unsigned int /*op type*/, unsigned int /*index in specific type vector*/> > operations_;
  ...
  //
  //    } //end sendToFrontEnd()
  void runFrontEndMacro(const std::string& targetInterfaceID, const std::string& feMacroName,
                        const std::vector<FEVInterface::frontEndMacroArg_t>& inputArgs,
                        std::vector<FEVInterface::frontEndMacroArg_t>& outputArgs) const;
  void runSelfFrontEndMacro(const std::string& feMacroName,
                            const std::vector<FEVInterface::frontEndMacroArg_t>& inputArgs,
  ...
  void receiveFromFrontEnd(const std::string& requester, std::string& retValue,
