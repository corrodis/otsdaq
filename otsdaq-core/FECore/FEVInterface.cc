...
void FEVInterface::configureSlowControls(void)
{
	...
	for (auto& groupLinkChild : groupLinkChildren)
	{
		...
		__FE_COUT__ << "Channel:" << getInterfaceUID() << "/" << groupLinkChild.first << "\t Type:" << groupLinkChild.second.getNode("ChannelDataType") << __E__;
...
{
	if (isSoftError)
		__COUT__ << "SOAP message failure indicating front-end asynchronous running SOFT error back to Gateway: " << e.what() << __E__;
	else
		__COUT__ << "SOAP message failure indicating front-end asynchronous running error back to Gateway: " << e.what() << __E__;
}
catch (...)
{
	if (isSoftError)
                                    ...
                                    std::vector<FEVInterface::frontEndMacroArg_t>&       outputArgs) const
{
	...
	{
		//error occurred!
		__FE_SS__ << "Error transmitting request to target interface '" << targetInterfaceID << "' from '" << FEVInterface::interfaceUID_ << ".' " << error << __E__;
		__FE_SS_THROW__;
	}
...
void FEVInterface::receiveFromFrontEnd(const std::string& requester, std::string& retValue, unsigned int timeoutInSeconds) const
{
	...
	while (1)
	{
		//mutex scope
		{
			...
			{
				__FE_COUT__ << "Number of source buffers found for front-end '" << FEVInterface::interfaceUID_ << "': " << receiveBuffersForTargetIt->second.size() << __E__;
