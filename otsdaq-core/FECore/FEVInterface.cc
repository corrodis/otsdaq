...
//========================================================================================================================
FEVInterface::FEVInterface(const std::string& interfaceUID, const ConfigurationTree& theXDAQContextConfigTree,
  ...
  // NOTE!! be careful to not decorate with __FE_COUT__ because in the constructor the base class versions of function
  // (e.g. getInterfaceType) are called because the derived class has not been instantiate yet!
  __COUT__ << "'" << interfaceUID << "' Constructed." << __E__;
}

//========================================================================================================================
FEVInterface::~FEVInterface(void) {
  // NOTE:: be careful not to call __FE_COUT__ decoration because it uses the tree and it may already be destructed
  // partially
...
void FEVInterface::configureSlowControls(void) {
  ConfigurationTree slowControlsGroupLink =
      theXDAQContextConfigTree_.getBackNode(theConfigurationPath_).getNode("LinkToSlowControlChannelsTable");
  ...
  __FE_COUT__ << "slowControlsGroupLink is valid! Configuring slow controls..." << __E__;
  ...
  for (auto& groupLinkChild : groupLinkChildren) {
    ...
    mapOfSlowControlsChannels_.insert(std::pair<std::string, FESlowControlsChannel>(
        ...
        FESlowControlsChannel(getInterfaceUID(), groupLinkChild.first,
                              groupLinkChild.second.getNode("ChannelDataType").getValue<std::string>(),
                              universalDataSize_, universalAddressSize_,
...
bool FEVInterface::slowControlsRunning(void) {
  ...
  ConfigurationTree slowControlsInterfaceLink = FEInterfaceNode.getNode("LinkToSlowControlsMonitorTable");

  std::unique_ptr<UDPDataStreamerBase> txSocket;

  if (slowControlsInterfaceLink.isDisconnected()) {
    __FE_COUT__ << "slowControlsInterfaceLink is disconnected, so no socket made." << __E__;
  } else {
    __FE_COUT__ << "slowControlsInterfaceLink is valid! Create tx socket..." << __E__;
    txSocket.reset(
        new UDPDataStreamerBase(FEInterfaceNode.getNode("SlowControlsTxSocketIPAddress").getValue<std::string>(),
  ...
  if (FEInterfaceNode.getNode("SlowControlsLocalAggregateSavingEnabled").getValue<bool>()) {
    ...
    __FE_COUT_INFO__ << "Slow Controls Aggregate Saving turned On BinaryFormat=" << aggregateFileIsBinaryFormat
                     << __E__;
  ...
  while (slowControlsWorkLoop_.getContinueWorkLoop()) {
    ...
    if (txBuffer.size()) __FE_COUT__ << "txBuffer sz=" << txBuffer.size() << __E__;
    ...
    for (auto& slowControlsChannelPair : mapOfSlowControlsChannels_) {
      ...
      __FE_COUT__ << "Channel:" << getInterfaceUID() << "/" << slowControlsChannelPair.first << __E__;
...
void FEVInterface::sendAsyncErrorToGateway(FEVInterface* fe, const std::string& errorMessage, bool isSoftError) try {
  ...
  if (isSoftError)
    ...
    __COUT__ << "SOAP message failure indicating front-end asynchronous running error back to Gateway: " << e.what()
             << __E__;
} catch (...) {
  if (isSoftError)
    __COUT__ << "Unknown error encounter indicating front-end asynchronous running SOFT error back to Gateway."
             << __E__;
  else
...
bool FEVInterface::workLoopThread(toolbox::task::WorkLoop* workLoop) {
  ...
  } catch (...)                    //
  {
    ...
    try {
      ...
      ss << "Caught an error during running at FE Interface '" << Configurable::theConfigurationRecordName_
         << "': " << e.what() << __E__;
    } catch (...) {
    ...
    std::thread([](FEVInterface* fe, const std::string errorMessage,
...
void FEVInterface::registerFEMacroFunction(const std::string& feMacroName, frontEndMacroFunction_t feMacroFunction,
                                           const std::vector<std::string>& namesOfInputArgs,
                                           const std::vector<std::string>& namesOfOutputArgs,
                                           uint8_t requiredUserPermissions, const std::string& allowedCallingFEs) {
  ...
  mapOfFEMacroFunctions_.insert(std::pair<std::string, frontEndMacroStruct_t>(
      feMacroName, frontEndMacroStruct_t(feMacroName, feMacroFunction, namesOfInputArgs, namesOfOutputArgs,
                                         requiredUserPermissions, allowedCallingFEs)));
}
...
template <>
std::string getFEMacroConstArgumentValue<std::string>(FEVInterface::frontEndMacroConstArgs_t& args,
...
//	helper function for getting the copy of the value of an argument
template <>
std::string getFEMacroArgumentValue<std::string>(FEVInterface::frontEndMacroArgs_t& args, const std::string& argName) {
...
std::string& FEVInterface::getFEMacroArgument(frontEndMacroArgs_t& args, const std::string& argName) {
  for (std::pair<const std::string /* output arg name */, std::string /* arg output value */>& pair : args) {
    if (pair.first == argName) return pair.second;
  }
    ...
    try {
      if (configSeqLink.isDisconnected())
        ...
        for (const auto& child : childrenMap) {
          ...
          if (writeHistory.find(writeAddress) == writeHistory.end()) writeHistory[writeAddress] = 0;  // init to 0

          writeHistory[writeAddress] &= bitMask;     // clear incoming bits
          writeHistory[writeAddress] |= writeValue;  // add incoming bits

          sprintf(msg, "\t Writing %s: \t %ld(0x%lX) \t %ld(0x%lX)", child.first.c_str(), writeAddress, writeAddress,
...
void FEVInterface::runSelfFrontEndMacro(
  ...
  if (FEMacroIt == this->getMapOfFEMacroFunctions().end()) {
    __CFG_SS__ << "FE Macro '" << feMacroName << "' of interfaceID '" << getInterfaceUID() << "' was not found!"
               << __E__;
    __CFG_COUT_ERR__ << "\n" << ss.str();
    __CFG_SS_THROW__;
  }
  ...
  for (unsigned int i = 0; i < argsIn.size(); ++i)
    if (argsIn[i].first != feMacro.namesOfInputArguments_[i]) {
      __CFG_SS__ << "FE Macro '" << feMacro.feMacroName_ << "' of interfaceID '" << getInterfaceUID()
...
void FEVInterface::runFrontEndMacro(const std::string& targetInterfaceID, const std::string& feMacroName,
  ...
  if (error != "") {
    // error occurred!
    __FE_SS__ << "Error transmitting request to target interface '" << targetInterfaceID << "' from '"
              << FEVInterface::interfaceUID_ << ".' " << error << __E__;
    __FE_SS_THROW__;
  }
  ...
  outputArgs.clear();
  for (auto& mapPair : mapToReturn) outputArgs.push_back(mapPair);
...
void FEVInterface::receiveFromFrontEnd(const std::string& requester, std::string& retValue,
  ...
  while (1) {
    // mutex scope
    {
      ...
      auto receiveBuffersForTargetIt =
          parentInterfaceManager_->frontEndCommunicationReceiveBuffer_.find(FEVInterface::interfaceUID_);
      if (receiveBuffersForTargetIt != parentInterfaceManager_->frontEndCommunicationReceiveBuffer_.end()) {
        ...
        for (auto& buffPair : receiveBuffersForTargetIt->second) __FE_COUTV__(buffPair.first);
      ...
      if (!timeoutInSeconds) {
        __FE_SS__ << "Timeout (" << timeoutInSeconds << " s) waiting for front-end communication from " << requester
                  << "." << __E__;
        __FE_SS_THROW__;
      }
    ...
    __FE_COUT__ << "Waiting for front-end communication from " << requester << " for " << timeoutInSeconds
                << " more seconds..." << __E__;
...
// receiveFromFrontEnd
//	specialized template function for T=std::string
//	Note: if called without template <T> syntax, necessary because types of std::basic_string<char> cause compiler
//problems if no string specific function
...
FEVInterface::macroStruct_t::macroStruct_t(const std::string& macroString) {
  ...
  for (auto& command : sequenceCommands) {
    ...
    {
      __SS__ << "Invalid command type '" << commandPieces[1][0] << "' specified with " << commandPieces.size()
             << " components." << __E__;
      __SS_THROW__;
    }
  ...
  for (const auto& inputArg : namesOfInputArguments_) __COUT__ << "\t" << inputArg << __E__;

  __COUT__ << "Output arguments: " << __E__;
  for (const auto& outputArg : namesOfOutputArguments_) __COUT__ << "\t" << outputArg << __E__;

}  // end macroStruct_t constructor

//========================================================================================================================
// runMacro
void FEVInterface::runMacro(FEVInterface::macroStruct_t& macro,
                            std::map<std::string /*name*/, uint64_t /*value*/>& variableMap) {
  // Similar to FEVInterface::runSequenceOfCommands()

  __FE_COUT__ << "Running Macro '" << macro.macroName_ << "' of " << macro.operations_.size() << " operations."
              << __E__;
