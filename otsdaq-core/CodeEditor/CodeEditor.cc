...
CodeEditor::CodeEditor()
    : ALLOWED_FILE_EXTENSIONS_({"h", "hh", "hpp", "hxx", "c", "cc", "cpp", "cxx", "icc", "txt", "sh", "css", "html",
                                "htm", "js", "py", "fcl", "xml"}) {
...
void CodeEditor::xmlRequest(const std::string& option, cgicc::Cgicc& cgiIn, HttpXmlDocument* xmlOut) try {
  __COUTV__(option);
...
std::string CodeEditor::safePathString(const std::string& path) {
  ...
  for (unsigned int i = 0; i < path.length(); ++i)
    if ((path[i] >= 'a' && path[i] <= 'z') || (path[i] >= 'A' && path[i] <= 'Z') || path[i] >= '_' || path[i] >= '-' ||
        path[i] >= ' ' || path[i] >= '/')
      fullpath += path[i];
  //__COUTV__(fullpath);
  if (!fullpath.length()) {
...
void CodeEditor::getDirectoryContent(cgicc::Cgicc& cgiIn, HttpXmlDocument* xmlOut) {
  ...
  std::string specialTypeNames[] = {"Front-End Plugins", "Data Processor Plugins", "Controls Interface Plugins",
                                    "Tools and Scripts"};
  std::string specialTypes[] = {SPECIAL_TYPE_FEInterface, SPECIAL_TYPE_DataProcessor, SPECIAL_TYPE_ControlsInterface,
                                SPECIAL_TYPE_Tools};

  std::string pathMatchPrepend = "/";  // some requests come in with leading "/" and "//"
  if (path.length() > 1 && path[1] == '/') pathMatchPrepend += '/';
  ...
  if (path == "/") {
    ...
    for (unsigned int i = 0; i < numOfTypes; ++i) xmlOut->addTextElementToData("special", specialTypeNames[i]);
  }
  ...
  struct InsensitiveCompare {
    bool operator()(const std::string& as, const std::string& bs) const {
      ...
      while ((d = (std::toupper(*a) - std::toupper(*b))) == 0 && *a) ++a, ++b;
  ...
  while ((entry = readdir(pDIR))) {
    ...
    if (name[0] != '.' && (type == 0 ||  // 0 == UNKNOWN (which can happen - seen in SL7 VM)
      ...
      if (type == 4) isDir = true;  // flag directory types
  ...
  for (const auto& name : orderedDirectories) xmlOut->addTextElementToData("directory", name);
  for (const auto& name : orderedFiles) xmlOut->addTextElementToData("file", name);
...
//========================================================================================================================
// writeFile
void CodeEditor::writeFile(const std::string& path, const std::string& contents, const unsigned long long& insertPos,
  ...
  // log changes
  {
    ...
    fprintf(fp, "time=%lld old-size=%lld new-size=%lld path=%s\n", (long long)time(0), oldSize,
            (long long)contents.size(), fullpath.c_str());
...
std::map<std::string /*special type*/, std::set<std::string> /*special file paths*/> CodeEditor::getSpecialsMap(void) {
  ...
  std::string specialFolders[] = {"FEInterfaces", "DataProcessorPlugins", "ControlsInterfacePlugins",
                                  "FEInterfacePlugins", "tools"};
  std::string specialMapTypes[] = {CodeEditor::SPECIAL_TYPE_FEInterface, CodeEditor::SPECIAL_TYPE_DataProcessor,
                                   CodeEditor::SPECIAL_TYPE_ControlsInterface, CodeEditor::SPECIAL_TYPE_FEInterface,
        ...
        while ((entry = readdir(pDIR))) {
          ...
          if (name[0] != '.' && (type == 0 ||  // 0 == UNKNOWN (which can happen - seen in SL7 VM)
            ...
            if (isDir) {
              ...
              if (name.find(".h") == name.length() - 2 || name.find(".cc") == name.length() - 3 ||
                  name.find(".txt") == name.length() - 4 || name.find(".sh") == name.length() - 3 ||
