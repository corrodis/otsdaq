...
//========================================================================================================================
UDPDataListenerProducer::UDPDataListenerProducer(std::string supervisorApplicationUID, std::string bufferUID,
...
{
  //__CFG_COUT__DataProcessor::processorUID_ << " running, because workloop: " << WorkLoop::continueWorkLoop_ <<
  //std::endl;
  fastWrite();
...
void UDPDataListenerProducer::slowWrite(void) {
  ...
  if (ReceiverSocket::receive(data_, ipAddress_, port_) != -1) {
    ...
    //__CFG_COUT__processorUID_ << " -> Got some data. From: " << std::hex << fromIPAddress << " port: " << fromPort <<
    //std::dec << std::endl;
...
void UDPDataListenerProducer::fastWrite(void) {
  ...
  if (0)  // test data buffers
  {
    ...
    buffer.resize(8);  // NOTE: this is inexpensive according to Lorenzo/documentation in C++11 (only increases size
  ...
  if (ReceiverSocket::receive(*dataP_, ipAddress_, port_, 1, 0, true) != -1) {
    ...
    if ((requestedPort_ == 47000 || requestedPort_ == 47001 || requestedPort_ == 47002) && dataP_->length() > 2) {
      unsigned char seqId = (*dataP_)[1];
      if (!(lastSeqId_ + 1 == seqId || (lastSeqId_ == 255 && seqId == 0))) {
        __CFG_COUT__ << requestedPort_ << "?????? NOOOO Missing Packets: " << (unsigned int)lastSeqId_ << " v "
