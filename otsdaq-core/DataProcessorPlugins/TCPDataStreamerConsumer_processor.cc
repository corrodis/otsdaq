...
//========================================================================================================================
TCPDataStreamerConsumer::TCPDataStreamerConsumer(std::string supervisorApplicationUID, std::string bufferUID,
...
void TCPDataStreamerConsumer::fastRead(void) {
  ...
  // unsigned int reconverted = (((*headerP_)["IPAddress"][0]&0xff)<<24) + (((*headerP_)["IPAddress"][1]&0xff)<<16) +
  // (((*headerP_)["IPAddress"][2]&0xff)<<8) + ((*headerP_)["IPAddress"][3]&0xff);  std::cout << __COUT_HDR_FL__ <<
  // __PRETTY_FUNCTION__ << processorUID_ << " -> Got some data. From: "  << std::hex << reconverted << std::dec <<
  // std::endl;
...
void TCPDataStreamerConsumer::slowRead(void) {
  ...
  // unsigned int reconverted = ((header_["IPAddress"][0]&0xff)<<24) + ((header_["IPAddress"][1]&0xff)<<16) +
  // ((header_["IPAddress"][2]&0xff)<<8) + (header_["IPAddress"][3]&0xff);  std::cout << __COUT_HDR_FL__ <<
  // __PRETTY_FUNCTION__ << processorUID_ << " -> Got some data. From: "  << std::hex << reconverted << std::dec <<
  // std::endl;
