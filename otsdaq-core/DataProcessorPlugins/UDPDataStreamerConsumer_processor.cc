...
//========================================================================================================================
UDPDataStreamerConsumer::UDPDataStreamerConsumer(std::string supervisorApplicationUID, std::string bufferUID,
...
void UDPDataStreamerConsumer::fastRead(void) {
  ...
  // (((*headerP_)["IPAddress"][2]&0xff)<<8) + ((*headerP_)["IPAddress"][3]&0xff);
...
void UDPDataStreamerConsumer::slowRead(void) {
  ...
  // ((header_["IPAddress"][2]&0xff)<<8) + (header_["IPAddress"][3]&0xff);
  //__COUT__ << processorUID_ << " -> Got some data. From: "  << std::hex << reconverted << std::dec << std::endl;
