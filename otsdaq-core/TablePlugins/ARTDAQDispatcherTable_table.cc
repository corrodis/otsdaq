...
//========================================================================================================================
ARTDAQDispatcherTable::ARTDAQDispatcherTable(void) : TableBase("ARTDAQDispatcherTable")
{
	//////////////////////////////////////////////////////////////////////
...
//========================================================================================================================
std::string ARTDAQDispatcherTable::getFHICLFilename(
    const ConfigurationTree& dispatcherNode)
{
	__COUT__ << "ARTDAQ Dispatcher UID: " << dispatcherNode.getValue() << std::endl;
	std::string filename = ARTDAQ_FCL_PATH + ARTDAQ_FILE_PREAMBLE + "-";
	std::string uid      = dispatcherNode.getValue();
                                        ...
                                        const XDAQContextTable*  contextConfig)
{
	   ...
	   2097152}
	      ...
	      metrics: {
	        aggFile: {
	          ...
	          level: 3
	          fileName: "/tmp/dispatcher/agg_%UID%_metrics.log"
	          uniquify: true
	        }
	        # ganglia: {
	...
	std::string filename = getFHICLFilename(dispatcherNode);
	...
	if(out.fail())
	{
		__SS__ << "Failed to open ARTDAQ Dispatcher fcl file: " << filename << std::endl;
		__SS_THROW__;
	}
	...
	OUT << "###########################################################" << __E__;
	OUT << "#" << __E__;
	OUT << "# artdaq dispatcher fcl configuration file produced by otsdaq." << __E__;
	OUT << "# 	Creation timestamp: " << StringMacros::getTimestampString() << __E__;
	OUT << "# 	Original filename: " << filename << __E__;
	OUT << "#	otsdaq-ARTDAQ Dispatcher UID: " << dispatcherNode.getValue() << __E__;
	OUT << "#" << __E__;
	OUT << "###########################################################" << __E__;
	OUT << "\n\n";
	...
	//--------------------------------------
	// handle services
	auto services = dispatcherNode.getNode("servicesLink");
	...
	//--------------------------------------
	// handle daq
	auto daq = dispatcherNode.getNode("daqLink");
	if(!daq.isDisconnected())
	{
		...
		auto parametersLink = daq.getNode("daqDispatcherParametersLink");
		if(!parametersLink.isDisconnected())
		{
			...
			for(auto& parameter : parameters)
				if(parameter.second.getNode("daqParameterKey").getValue() ==
				       "is_dispatcher" &&
			...
			if(requiredParameterKeys.size())
			{
				...
				{
					__SS__ << "Dispatcher required parameters missing: "
					       << StringMacros::setToString(requiredParameterKeys) << __E__;
					__SS_THROW__;
				}
			}
		}
		OUT << "\n";  // end daq dispatcher parameters

		OUT << "sources: {\n";

		PUSHTAB;
		auto sourcesGroup = daq.getNode("daqDispatcherSourcesLink");
		if(!sourcesGroup.isDisconnected())
		{
				...
				for(auto& source : sources)
				{
					...
					catch(const std::runtime_error& e)
					{
						__MCOUT_WARN__("Are the DAQ sources valid? "
						               ...
						               << "Dispatcher DAQ source context '"
						               << sourceContextUID << "' for UID '"
						               << dispatcherNode.getValue() << "': " << e.what()
						               << __E__);
						continue;
					}
			...
			catch(const std::runtime_error& e)
			{
				__SS__ << "Are the DAQ sources valid? Error occurred looking for "
				          "Dispatcher DAQ sources for UID '"
				       << dispatcherNode.getValue() << "': " << e.what() << std::endl;
		...
		PUSHTAB;
		// other destinations
		auto destinationsGroup = daq.getNode("daqDispatcherDestinationsLink");
		if(!destinationsGroup.isDisconnected())
		{
				...
				for(auto& destination : destinations)
				{
					...
					catch(const std::runtime_error& e)
					{
						__MCOUT_WARN__(
						    ...
						    << "Dispatcher DAQ source context '" << destinationContextUID
						    << "' for UID '" << dispatcherNode.getValue()
						    << "': " << e.what() << __E__);
						continue;
					}
					std::string host =
			...
			catch(const std::runtime_error& e)
			{
				__SS__ << "Are the DAQ destinations valid? Error occurred looking for "
				          "Dispatcher DAQ destinations for UID '"
				       << dispatcherNode.getValue() << "': " << e.what() << std::endl;
				__COUT_ERR__ << ss.str() << std::endl;
				__SS_THROW__;
			}
	...
	//--------------------------------------
	// handle source
	auto source = dispatcherNode.getNode("sourceLink");
	...
	//--------------------------------------
	// handle outputs
	auto outputs = dispatcherNode.getNode("outputsLink");
	...
	//--------------------------------------
	// handle physics
	auto physics = dispatcherNode.getNode("physicsLink");
	...
	{
		__SS__ << "The field "
		          "ARTDAQGlobalTableForProcessNameLink/processNameForDispatchers could "
		          "not be accessed. "
		       << "Make sure the link is valid from dispatcher '"
		       << dispatcherNode.getValue() << ".'" << __E__;
		__SS_THROW__;
	}
