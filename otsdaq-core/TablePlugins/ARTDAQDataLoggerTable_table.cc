...
//========================================================================================================================
ARTDAQDataLoggerTable::ARTDAQDataLoggerTable(void) : TableBase("ARTDAQDataLoggerTable")
{
	//////////////////////////////////////////////////////////////////////
...
//========================================================================================================================
std::string ARTDAQDataLoggerTable::getFHICLFilename(
    const ConfigurationTree& dataLoggerNode)
{
	__COUT__ << "ARTDAQ DataLogger UID: " << dataLoggerNode.getValue() << std::endl;
	std::string filename = ARTDAQ_FCL_PATH + ARTDAQ_FILE_PREAMBLE + "-";
	std::string uid      = dataLoggerNode.getValue();
                                        ...
                                        const XDAQContextTable*  contextConfig)
{
	   ...
	   2097152}
	      ...
	      metrics: {
	        aggFile: {
	          ...
	          level: 3
	          fileName: "/tmp/datalogger/dl_%UID%_metrics.log"
	          uniquify: true
	        }
	        # ganglia: {
	...
	std::string filename = getFHICLFilename(dataLoggerNode);
	...
	if(out.fail())
	{
		__SS__ << "Failed to open ARTDAQ DataLogger fcl file: " << filename << std::endl;
		__SS_THROW__;
	}
	...
	OUT << "###########################################################" << __E__;
	OUT << "#" << __E__;
	OUT << "# artdaq DataLogger fcl configuration file produced by otsdaq." << __E__;
	OUT << "# 	Creation timestamp: " << StringMacros::getTimestampString() << __E__;
	OUT << "# 	Original filename: " << filename << __E__;
	OUT << "#	otsdaq-ARTDAQ DataLogger UID: " << dataLoggerNode.getValue() << __E__;
	OUT << "#" << __E__;
	OUT << "###########################################################" << __E__;
	OUT << "\n\n";
	...
	//--------------------------------------
	// handle services
	auto services = dataLoggerNode.getNode("servicesLink");
	...
	//--------------------------------------
	// handle daq
	auto daq = dataLoggerNode.getNode("daqLink");
	if(!daq.isDisconnected())
	{
		...
		auto parametersLink = daq.getNode("daqDataLoggerParametersLink");
		if(!parametersLink.isDisconnected())
		{
			...
			if(requiredParameterKeys.size())
			{
					__SS__ << "DataLogger required parameters missing: "
					       << StringMacros::setToString(requiredParameterKeys) << __E__;
					__SS_THROW__;
			}
		}
		OUT << "\n";  // end daq datalogger parameters

		OUT << "sources: {\n";

		PUSHTAB;
		auto sourcesGroup = daq.getNode("daqDataLoggerSourcesLink");
		if(!sourcesGroup.isDisconnected())
		{
				...
				for(auto& source : sources)
				{
					...
					catch(const std::runtime_error& e)
					{
						__MCOUT_WARN__("Are the DAQ sources valid? "
						               ...
						               << "DataLogger DAQ source context '"
						               << sourceContextUID << "' for UID '"
						               << dataLoggerNode.getValue() << "': " << e.what()
						               << __E__);
						continue;
					}
			...
			catch(const std::runtime_error& e)
			{
				__SS__ << "Are the DAQ sources valid? Error occurred looking for "
				          "DataLogger DAQ sources for UID '"
				       << dataLoggerNode.getValue() << "': " << e.what() << std::endl;
		...
		PUSHTAB;
		// other destinations
		auto destinationsGroup = daq.getNode("daqDataLoggerDestinationsLink");
		if(!destinationsGroup.isDisconnected())
		{
				...
				for(auto& destination : destinations)
				{
					...
					catch(const std::runtime_error& e)
					{
						__MCOUT_WARN__(
						    ...
						    << "DataLogger DAQ source context '" << destinationContextUID
						    << "' for UID '" << dataLoggerNode.getValue()
						    << "': " << e.what() << __E__);
						continue;
					}
					std::string host =
			...
			catch(const std::runtime_error& e)
			{
				__SS__ << "Are the DAQ destinations valid? Error occurred looking for "
				          "DataLogger DAQ destinations for UID '"
				       << dataLoggerNode.getValue() << "': " << e.what() << std::endl;
				__COUT_ERR__ << ss.str() << std::endl;
				__SS_THROW__;
			}
	...
	//--------------------------------------
	// handle source
	auto source = dataLoggerNode.getNode("sourceLink");
	...
	//--------------------------------------
	// handle outputs
	auto outputs = dataLoggerNode.getNode("outputsLink");
	...
	//--------------------------------------
	// handle physics
	auto physics = dataLoggerNode.getNode("physicsLink");
	...
	{
		__SS__ << "The field "
		          "ARTDAQGlobalTableForProcessNameLink/processNameForDataLoggers could "
		          "not be accessed. "
		       << "Make sure the link is valid from datalogger '"
		       << dataLoggerNode.getValue() << ".'" << __E__;
		__SS_THROW__;
	}
