...
void getValue(T& value, unsigned int row, unsigned int col) const
{
    ...
    {
            std::cout << __COUT_HDR_FL__ << "\tUnrecognized View data type: " << columnsInfo_[col].getDataType() <<
std::endl; assert(0);
    }
}
...
std::endl; assert(0);
  ...
  /*    template<class T>
      ...
      assert(col < columnsInfo_.size() && row < getNumberOfRows());
              ...
              {
                      std::cout << __COUT_HDR_FL__ << "\tUnrecognized View data type: " <<
     columnsInfo_[col].getDataType() << std::endl; assert(0);
              }
      }*/
