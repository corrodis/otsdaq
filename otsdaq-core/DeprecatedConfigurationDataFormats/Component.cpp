...
Component::Component(std::string name, std::string typeName) : componentName_(name), typeName_(typeName) {}
...
//==============================================================================
void Component::addRegister(std::string name, std::string baseAddress, int size, std::string access,
                            int globalSequencePosition, int globalValue) {
  Register tempRegister_(name);
...
void Component::printInfo(void) {
  ...
  std::cout
      << __COUT_HDR_FL__
      << "REGISTER \t\t BASE ADDRESS \t\t SIZE \t\t ACCESS \t\t||\t INIT \t CONFIG \t START \t STOP \t PAUSE \t RESUME "
      << std::endl;

  for (std::list<Register>::iterator aRegister = registers_.begin(); aRegister != registers_.end(); aRegister++) {
    std::cout << __COUT_HDR_FL__ << aRegister->getName() << "\t\t" << aRegister->getBaseAddress() << "\t\t"
              << std::to_string(aRegister->getSize()) << "\t\t" << aRegister->getAccess() << "\t\t||\t"
              << printPair(aRegister->getInitialize()) << "/t" << printPair(aRegister->getStart()) << "\t"
              << printPair(aRegister->getHalt()) << "\t" << printPair(aRegister->getPause()) << "\t"
