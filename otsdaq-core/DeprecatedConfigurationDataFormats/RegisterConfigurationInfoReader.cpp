...
void RegisterConfigurationInfoReader::initPlatform(void)
{
	...
	{
		__MOUT_ERROR__ << "XML toolkit initialization error: "
...
//==============================================================================
xercesc::DOMNode* RegisterConfigurationInfoReader::getNode(XMLCh*            tagName,
                                                           ...
                                                           unsigned int itemNumber)
{
	...
	if(!nodeList)
	{
		...
		std::cout << __COUT_HDR_FL__
		          << (std::string("Can't find ") + XML_TO_CHAR(tagName) + " tag!")
		          << std::endl;
	}
	//    std::cout << __COUT_HDR_FL__<< "Name: "  <<
	//    XML_TO_CHAR(nodeList->item(itemNumber)->getNodeName()) << std::endl; if(
	//    nodeList->item(itemNumber)->getFirstChild() != 0 )
	//        std::cout << __COUT_HDR_FL__<< "Value: " <<
...
//==============================================================================
xercesc::DOMElement* RegisterConfigurationInfoReader::getElement(XMLCh* tagName,
                                                                 xercesc::DOMNode* parent,
                                                                 unsigned int itemNumber)
{
...
//==============================================================================
xercesc::DOMElement* RegisterConfigurationInfoReader::getElement(
    XMLCh* tagName, xercesc::DOMElement* parent, unsigned int itemNumber)
{
	return dynamic_cast<xercesc::DOMElement*>(getNode(tagName, parent, itemNumber));
}
...
{
	/*
	std::string configurationDataDir = std::string(getenv("CONFIGURATION_DATA_PATH")) +
	"/" + configuration.getTypeName() + "RegisterConfiguration/";
	        //"/ConfigurationInfo/";
	std::string configFile = configurationDataDir + "/0/" + configuration.getTypeName() +
	"RegisterConfiguration" + ".xml"; struct stat fileStatus;
	...
	try
	{
	    ...
	    //<CONFIGURATION>
	    xercesc::DOMElement* configurationElement = getElement(registerNameTag_,
	elementRoot, 0); if( configuration.getConfigurationName() !=
	XML_TO_CHAR(configurationElement->getAttribute(configurationNameAttributeTag_)) )
	    {

	        std::cout << __COUT_HDR_FL__ << "In " << configFile << " the configuration
	name " <<
	XML_TO_CHAR(configurationElement->getAttribute(configurationNameAttributeTag_))
	        << " doesn't match the the class configuration name " <<
	configuration.getConfigurationName() << "     "; assert(0);
	    }
	    //<DATA>
	    xercesc::DOMNodeList* viewNodeList =
	configurationElement->getElementsByTagName(dataTag_); bool storageTypeFound = false;
	    for( XMLSize_t view = 0; view < viewNodeList->getLength(); view++ )
	    {
	  if( !viewNodeList->item(view)->getNodeType() ||
	viewNodeList->item(view)->getNodeType() != xercesc::DOMNode::ELEMENT_NODE )//true is
	not 0 && is element continue;
	  ...
	  xercesc::DOMElement * typeNameElement 	 	= dynamic_cast< xercesc::DOMElement*
	>(viewElement->getElementsByTagName(typeNameTag_		)->item(0));
	        xercesc::DOMElement * registerNameElement 	= dynamic_cast<
	xercesc::DOMElement* >(viewElement->getElementsByTagName(registerNameTag_
	)->item(0)); xercesc::DOMElement * baseAddressElement 	= dynamic_cast<
	xercesc::DOMElement* >(viewElement->getElementsByTagName(baseAddressTag_
	)->item(0)); xercesc::DOMElement * sizeElement		 	= dynamic_cast<
	xercesc::DOMElement* >(viewElement->getElementsByTagName(sizeTag_ )->item(0));
	        xercesc::DOMElement * accessElement		 	= dynamic_cast<
	xercesc::DOMElement* >(viewElement->getElementsByTagName(accessTag_ )->item(0));
	    ...
	    if( !storageTypeFound )
	    {
	        ...
	        << getenv("CONFIGURATION_TYPE") << ") doesn't match with any of the types
	defined in " << configFile << "     "; assert(0);
	    }
