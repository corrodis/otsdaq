...
void RegisterBase::init(void) {
  ...
  for (auto it : *mockupRegisterView_.getRegistersSequencerInfoPointer()) {
    ...
    // the insert function will have no effect if it already finds the key (component name) in theComponentList_;
    ...
    std::find_if(regs->begin(), regs->end(), [it](auto r) { return r.getName() == it.getRegisterName(); })
    ...
    for (auto registerFromConfiguration : *aComponent.second.getRegistersPointer()) {
      ...
      for (auto aRegister : *mockupRegisterView_.getRegistersInfoPointer()) {
        ...
        if (aRegister.getRegisterName() == registerFromConfiguration.getName()
          ...
          registerFromConfiguration.fillRegisterInfo(aRegister.getBaseAddress(), aRegister.getSize(),
                                                     aRegister.getAccess());
        }
      }
    }
  ...
  /*
  for(std::vector<ViewRegisterInfo>::iterator it = mockupRegisterView_.getRegistersInfoPointer().begin(); it !=
...
//==============================================================================
RegisterView* RegisterBase::getMockupRegisterViewPointer() { return &mockupRegisterView_; }
//==============================================================================
