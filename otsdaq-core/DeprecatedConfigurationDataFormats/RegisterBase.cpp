...
RegisterBase::RegisterBase(std::string configurationName, std::string componentType)
    : ConfigurationBase(configurationName), componentTypeName_(componentType)
{
...
void RegisterBase::init(void)
{
	...
	for(auto it : *mockupRegisterView_.getRegistersSequencerInfoPointer())
	{
		...
		// the insert function will have no effect if it already finds the key (component
		// name) in theComponentList_; otherwise, let's add it
		theComponentList_.insert(
		    std::make_pair(it.getComponentName(), Component(it.getComponentName())));
		// theComponentList_ now atleast contains the component, now lets set the state
		// that we are looking at
		auto regs =
		    theComponentList_.find(it.getComponentName())->second.getRegistersPointer();
		std::find_if(regs->begin(),
		             regs->end(),
		             [it](auto r) { return r.getName() == it.getRegisterName(); })
		    ->setState(it.getState(), it.getValueSequencePair());
	}
		...
		for(auto registerFromConfiguration : *aComponent.second.getRegistersPointer())
		{
			...
			for(auto aRegister : *mockupRegisterView_.getRegistersInfoPointer())
			{
				...
				if(aRegister.getRegisterName() == registerFromConfiguration.getName()
				   //&& aRegister.getComponentName() ==
				   // registerFromConfiguration.getComponentName()
				)
				{
					registerFromConfiguration.fillRegisterInfo(aRegister.getBaseAddress(),
					                                           aRegister.getSize(),
					                                           aRegister.getAccess());
				}
			}
	...
	// create new instance of "Component" and add to "theComponentList_; set Name
	// according to XML  tempComponent = new Component(FROM_XML);
	...
	/*
	    for(std::vector<ViewRegisterInfo>::iterator it =
	   mockupRegisterView_.getRegistersInfoPointer().begin(); it !=
	   mockupRegisterView_.getRegistersInfoPointer().end(); ++it){

	        if(it->getTypeName == componentNameType_)
	        {
	            //Check the Component's "type_name"
	            //Accordingly create instances of "Register" based on Configuration XML
	   and add to Component theComponent_.addRegister(it->getregisterName(),
	   it->getBaseAddress, it->getSize(), it->getAccess);
...
//==============================================================================
const RegisterView& RegisterBase::getRegisterView() const { return *activeRegisterView_; }

//==============================================================================
RegisterView* RegisterBase::getRegisterViewPointer() { return activeRegisterView_; }
...
//==============================================================================
std::string RegisterBase::getTypeName() { return componentTypeName_; }
