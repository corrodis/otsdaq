...
ConfigurationView &ConfigurationView::operator=(const ConfigurationView src) {
  __SS__ << "Invalid use of operator=... Should not directly copy a ConfigurationView. Please use "
            "ConfigurationView::copy(sourceView,author,comment)";
  __SS_THROW__;
}

//==============================================================================
ConfigurationView &ConfigurationView::copy(const ConfigurationView &src, ConfigurationVersion destinationVersion,
...
unsigned int ConfigurationView::copyRows(const std::string &author, const ConfigurationView &src,
  ...
  if (src.getNumberOfColumns() != getNumberOfColumns()) {
    __SS__ << "Error! Number of Columns of source view must match destination view."
           << "Dimension of source is [" << src.getNumberOfColumns() << "] and of destination is ["
           << getNumberOfColumns() << "]." << __E__;
    __SS_THROW__;
  }
  ...
  for (unsigned int r = 0; r < srcRowsToCopy; ++r) {
    if (r + srcOffsetRow >= srcRows) break;  // end when no more source rows to copy (past bounds)
    ...
    if (retRow == (unsigned int)-1) retRow = destOffsetRow;  // save row of first copied entry

    // copy data
    for (unsigned int col = 0; col < getNumberOfColumns(); ++col)
      if (generateUniqueDataColumns && columnsInfo_[col].getType() == ViewColumnInfo::TYPE_UNIQUE_DATA)
...
void ConfigurationView::init(void) {
  try {
    ...
    for (auto &colInfo : columnsInfo_) {
      ...
      if (colName == "COMMENT_DESCRIPTION") colName = "COMMENT";
      capsColName = "";
    ...
    if (colNameSet.size() != columnsInfo_.size()) {
      __SS__ << "Configuration Error:\t"
             ...
             << " columns and the unique name count is " << colNameSet.size() << __E__;
      __SS_THROW__;
    }
    ...
    if ((colPos = findColByType(ViewColumnInfo::TYPE_COMMENT)) != INVALID) {
      ...
      {
        __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_COMMENT << " data type in column "
               << columnsInfo_[colPos].getName() << " is repeated. This is not allowed." << __E__;
        __SS_THROW__;
      }

      if (colPos != getNumberOfColumns() - 3) {
        __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_COMMENT
               << " data type column must be 3rd to last (in column " << getNumberOfColumns() - 3 << ")." << __E__;
        __SS_THROW__;
      }
    } else {
      __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_COMMENT << " data type column "
    ...
    if ((colPos = findColByType(ViewColumnInfo::TYPE_AUTHOR)) != INVALID) {
      ...
      {
        __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_AUTHOR << " data type in column "
               << columnsInfo_[colPos].getName() << " is repeated. This is not allowed." << __E__;
        __SS_THROW__;
      }
      ...
      if (colPos != getNumberOfColumns() - 2) {
        __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_AUTHOR
               << " data type column must be 2nd to last (in column " << getNumberOfColumns() - 2 << ")." << __E__;
        __SS_THROW__;
      }
    } else {
      __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_AUTHOR << " data type column "
             << " is missing. This is not allowed." << __E__;
      __SS_THROW__;
    }
    ...
    if ((colPos = findColByType(ViewColumnInfo::TYPE_TIMESTAMP)) != INVALID) {
      ...
      {
        __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_TIMESTAMP << " data type in column "
               << columnsInfo_[colPos].getName() << " is repeated. This is not allowed." << __E__;
        __SS_THROW__;
      }
      ...
      if (colPos != getNumberOfColumns() - 1) {
        __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_TIMESTAMP
               << " data type column must be last (in column " << getNumberOfColumns() - 1 << ")." << __E__;
        __COUT_ERR__ << "\n" << ss.str();
        __SS_THROW__;
      }
    } else {
      __SS__ << "Configuration Error:\t" << ViewColumnInfo::TYPE_TIMESTAMP << " data type column "
             << " is missing. This is not allowed." << __E__;
      __SS_THROW__;
    }
    ...
    for (unsigned int row = 0; row < getNumberOfRows(); ++row) {
      if (uidSet.find(theDataView_[row][colUID_]) != uidSet.end()) {
        __SS__ << ("Entries in UID are not unique. Specifically at row=" + std::to_string(row) +
    ...
    while ((colPos = findColByType(ViewColumnInfo::TYPE_UNIQUE_DATA, colPos + 1)) != INVALID) {
      ...
      if (uDataSet.size() != getNumberOfRows()) {
        __SS__ << "Entries in  Unique Data column " << columnsInfo_[colPos].getName() << " are not unique!"
               << "There are " << getNumberOfRows() << " records and the unique data count is " << uDataSet.size()
               << __E__;
        __SS_THROW__;
      }
    }
      ...
      for (unsigned int groupIdColPos = 0; groupIdColPos < columnsInfo_.size(); ++groupIdColPos)
        if (columnsInfo_[groupIdColPos].isGroupID()) {
          std::map<std::string /*group name*/,
                   std::pair<unsigned int /*memberCount*/, std::set<std::string /*unique data*/> > >
              uGroupDataSets;
          ...
          for (unsigned int row = 0; row < getNumberOfRows(); ++row) {
            ...
            for (const auto &groupId : groupIds) {
              ...
              if (uGroupDataSets[groupId].second.find(theDataView_[row][colPos]) !=
                ...
                __SS__ << "Entries in Unique Group Data column " << columnsInfo_[colPos].getName()
                       << " are not unique for group ID '" << groupId
                       << ".' Specifically at row=" << std::to_string(row) << " value=" << theDataView_[row][colPos]
                       << __E__;
                __SS_THROW__;
              }
          ...
          for (const auto &groupPair : uGroupDataSets)
            if (uGroupDataSets[groupPair.first].second.size() != uGroupDataSets[groupPair.first].first) {
              __SS__ << "Entries in  Unique Data column " << columnsInfo_[colPos].getName()
                     ...
                     << " records and the unique data count is " << uGroupDataSets[groupPair.first].second.size()
                     << __E__;
              __SS_THROW__;
            }
        }
    ...
    for (unsigned int col = 0; col < getNumberOfColumns(); ++col) {
      if (columnsInfo_[col].getType() == ViewColumnInfo::TYPE_FIXED_CHOICE_DATA) {
        ...
        if (theDataChoices.size() && theDataChoices[0] == "arbitraryBool=1") continue;  // arbitrary values allowed

        bool found;
        for (unsigned int row = 0; row < getNumberOfRows(); ++row) {
          found = false;
          // check against default value first
          if (theDataView_[row][col] == rowDefaults[col]) continue;  // default is always ok
        ...
        for (unsigned int row = 0; row < getNumberOfRows(); ++row) {
          if (theDataView_[row][col] == "1" || theDataView_[row][col] == "on" || theDataView_[row][col] == "On" ||
              theDataView_[row][col] == "ON")
            theDataView_[row][col] = ViewColumnInfo::TYPE_VALUE_ON;
          else if (theDataView_[row][col] == "0" || theDataView_[row][col] == "off" ||
                   theDataView_[row][col] == "Off" || theDataView_[row][col] == "OFF")
            theDataView_[row][col] = ViewColumnInfo::TYPE_VALUE_OFF;
          else {
            __SS__ << "Configuration Error:\t" << theDataView_[row][col] << " in column " << columnsInfo_[col].getName()
                   << " is not a valid Type (On/Off) std::string. Possible values are 1, on, On, ON, 0, off, Off, OFF."
                   << __E__;
            __SS_THROW__;
          }
        }
        ...
        for (unsigned int row = 0; row < getNumberOfRows(); ++row) {
          ...
          else if (theDataView_[row][col] == "0" || theDataView_[row][col] == "false" ||
                   theDataView_[row][col] == "False" || theDataView_[row][col] == "FALSE")
            theDataView_[row][col] = ViewColumnInfo::TYPE_VALUE_FALSE;
          else {
            __SS__ << "Configuration Error:\t" << theDataView_[row][col] << " in column " << columnsInfo_[col].getName()
                   << " is not a valid Type (True/False) std::string. Possible values are 1, true, True, TRUE, 0, "
                      "false, False, FALSE."
                   << __E__;
            __SS_THROW__;
          }
        ...
        for (unsigned int row = 0; row < getNumberOfRows(); ++row) {
          if (theDataView_[row][col] == "1" || theDataView_[row][col] == "yes" || theDataView_[row][col] == "Yes" ||
              theDataView_[row][col] == "YES")
            theDataView_[row][col] = ViewColumnInfo::TYPE_VALUE_YES;
          else if (theDataView_[row][col] == "0" || theDataView_[row][col] == "no" || theDataView_[row][col] == "No" ||
                   theDataView_[row][col] == "NO")
            theDataView_[row][col] = ViewColumnInfo::TYPE_VALUE_NO;
          else {
            __SS__ << "Configuration Error:\t" << theDataView_[row][col] << " in column " << columnsInfo_[col].getName()
                   << " is not a valid Type (Yes/No) std::string. Possible values are 1, yes, Yes, YES, 0, no, No, NO."
                   << __E__;
            __SS_THROW__;
          }
        }
      ...
      } else if (columnsInfo_[col].isChildLink())  // Child Link type
      {
        ...
        for (unsigned int row = 0; row < getNumberOfRows(); ++row)
          if (theDataView_[row][col] == "NoLink" || theDataView_[row][col] == "No_Link" ||
              theDataView_[row][col] == "NOLINK" || theDataView_[row][col] == "NO_LINK" ||
              theDataView_[row][col] == "Nolink" || theDataView_[row][col] == "nolink" ||
        ...
        if (columnsInfo_[col].getDataType() != ViewColumnInfo::DATATYPE_STRING) {
          __SS__ << "Configuration Error:\t"
                 << "Column " << col << " with name " << columnsInfo_[col].getName()
                 << " is a Child Link column and has an illegal data type of '" << columnsInfo_[col].getDataType()
                 << "'. The data type for Child Link columns must be " << ViewColumnInfo::DATATYPE_STRING << __E__;
          __SS_THROW__;
        }
        ...
        for (unsigned int row = 0; row < getNumberOfRows(); ++row)
          if (theDataView_[row][col] == "") theDataView_[row][col] = rowDefaults[col];
    ...
    if (groupIdIndexes.size() != groupIdIndexesCount) {
      __SS__ << ("GroupId Labels are not unique!") << "There are " << groupIdIndexesCount
             << " GroupId Labels and the unique count is " << groupIdIndexes.size() << __E__;
      __SS_THROW__;
    }
    if (childLinkIndexes.size() != childLinkIndexesCount) {
      __SS__ << ("Child Link Labels are not unique!") << "There are " << childLinkIndexesCount
             << " Child Link Labels and the unique count is " << childLinkIndexes.size() << __E__;
      __SS_THROW__;
    }
    if (childLinkIdLabels.size() != childLinkIdLabelsCount) {
      __SS__ << ("Child Link ID Labels are not unique!") << "There are " << childLinkIdLabelsCount
             << " Child Link ID Labels and the unique count is " << childLinkIdLabels.size() << __E__;
      __SS_THROW__;
    }

  } catch (...) {
    __COUT__ << "Error occured in ConfigurationView::init() for version=" << version_ << __E__;
    throw;
  }
}  // end init()
...
void ConfigurationView::getValue(std::string &value, unsigned int row, unsigned int col,
...
std::string ConfigurationView::validateValueForColumn(const std::string &value, unsigned int col,
                                                      bool doConvertEnvironmentVariables) const {
  ...
  if (columnsInfo_[col].getDataType() == ViewColumnInfo::DATATYPE_STRING)
    ...
    __SS__ << "\tUnrecognized column data type: " << columnsInfo_[col].getDataType() << " in configuration "
           << tableName_ << " at column=" << columnsInfo_[col].getName() << " for getValue with type '"
           << StringMacros::demangleTypeName(typeid(std::string).name()) << "'" << __E__;
    __SS_THROW__;
  }
...
void ConfigurationView::setValue(const std::string &value, unsigned int row, unsigned int col) {
  ...
  else  // dont allow ViewColumnInfo::DATATYPE_TIME to be set as string.. force use as time_t to standardize string
        // result
  {
    __SS__ << "\tUnrecognized column data type: " << columnsInfo_[col].getDataType() << " in configuration "
           << tableName_ << " at column=" << columnsInfo_[col].getName() << " for setValue with type '"
...
const unsigned int ConfigurationView::getOrInitColStatus(void) {
  ...
  if (colStatus_ == INVALID) {
    __SS__ << "\tMissing " << ViewColumnInfo::COL_NAME_STATUS << " Column in table named '" << tableName_ << "'"
           << __E__;
...
const unsigned int ConfigurationView::getOrInitColPriority(void) {
  ...
  if (colPriority_ == INVALID) {
    __SS__ << "\tMissing " << ViewColumnInfo::COL_NAME_PRIORITY << " Column in table named '" << tableName_ << "'"
           << __E__;
    ss << "Column Types: " << __E__;
    for (unsigned int col = 0; col < columnsInfo_.size(); ++col)
      ss << columnsInfo_[col].getType() << "() " << columnsInfo_[col].getName() << __E__;
...
void ConfigurationView::addRowToGroup(const unsigned int &row, const unsigned int &col,
...
bool ConfigurationView::removeRowFromGroup(const unsigned int &row, const unsigned int &col,
                                           const std::string &groupNeedle, bool deleteRowIfNoGroupLeft) {
...
//
//	Group entry can include | to place a record in multiple groups
bool ConfigurationView::isEntryInGroup(const unsigned int &r, const std::string &childLinkIndex,
...
bool ConfigurationView::isEntryInGroupCol(const unsigned int &r, const unsigned int &c, const std::string &groupNeedle,
  ...
  if (i != j)  // last group check (for case when no ' ' or '|')
  {
    ...
    //__COUT__ << "Group found to compare: " <<
    //		theDataView_[r][c].substr(i,j-i) << __E__;
    if (groupNeedle == theDataView_[r][c].substr(i, j - i)) return true;
  }
...
std::set<std::string> ConfigurationView::getSetOfGroupIDs(const std::string &childLinkIndex, unsigned int r) const {
  return getSetOfGroupIDs(getColLinkGroupID(childLinkIndex), r);
}
std::set<std::string> ConfigurationView::getSetOfGroupIDs(const unsigned int &c, unsigned int r) const {
  ...
  if (r != (unsigned int)-1) {
    ...
    for (r = 0; r < getNumberOfRows(); ++r) {
      ...
      //						&& i == j)
      //					++i;
      //				else if((theDataView_[r][c][j] == ' ' || //trailing white space or | indicates
      //group 						theDataView_[r][c][j] == '|')
      //						&& i != j) // assume end of group name
      //				{
      //					//__COUT__ << "Group found: " <<
      //					//		theDataView_[r][c].substr(i,j-i) << __E__;
...
unsigned int ConfigurationView::findRow(unsigned int col, const std::string &value, unsigned int offsetRow) const {
  for (unsigned int row = offsetRow; row < theDataView_.size(); ++row) {
    if (theDataView_[row][col] == value) return row;
  }

  __SS__ << "\tIn view: " << tableName_ << ", Can't find value=" << value << " in column named "
         << columnsInfo_[col].getName() << " with type=" << columnsInfo_[col].getType() << __E__;
...
//==============================================================================
unsigned int ConfigurationView::findRowInGroup(unsigned int col, const std::string &value, const std::string &groupId,
                                               const std::string &childLinkIndex, unsigned int offsetRow) const {
  unsigned int groupIdCol = getColLinkGroupID(childLinkIndex);
  for (unsigned int row = offsetRow; row < theDataView_.size(); ++row) {
    if (theDataView_[row][col] == value && isEntryInGroupCol(row, groupIdCol, groupId)) return row;
  }

  __SS__ << "\tIn view: " << tableName_ << ", Can't find in group the value=" << value << " in column named '"
         << columnsInfo_[col].getName() << "' with type=" << columnsInfo_[col].getType() << " and GroupID: '" << groupId
         << "' in column '" << groupIdCol << "' with GroupID child link index '" << childLinkIndex << "'" << __E__;
...
unsigned int ConfigurationView::findCol(const std::string &wildCardName) const {
  for (unsigned int col = 0; col < columnsInfo_.size(); ++col)
    if (StringMacros::wildCardMatch(wildCardName /*needle*/, columnsInfo_[col].getName() /*haystack*/)) return col;

  __SS__ << "\tIn view: " << tableName_ << ", Can't find column named '" << wildCardName << "'" << __E__;
  ss << "Existing columns:\n";
  for (unsigned int col = 0; col < columnsInfo_.size(); ++col) ss << "\t" << columnsInfo_[col].getName() << "\n";
...
unsigned int ConfigurationView::findColByType(const std::string &type, int startingCol) const {
  for (unsigned int col = startingCol; col < columnsInfo_.size(); ++col)
    if (columnsInfo_[col].getType() == type) return col;
...
// Getters
//==============================================================================
const std::string &ConfigurationView::getUniqueStorageIdentifier(void) const { return uniqueStorageIdentifier_; }

//==============================================================================
const std::string &ConfigurationView::getTableName(void) const { return tableName_; }

//==============================================================================
const ConfigurationVersion &ConfigurationView::getVersion(void) const { return version_; }

//==============================================================================
const std::string &ConfigurationView::getComment(void) const { return comment_; }

//==============================================================================
const std::string &ConfigurationView::getAuthor(void) const { return author_; }

//==============================================================================
const time_t &ConfigurationView::getCreationTime(void) const { return creationTime_; }

//==============================================================================
const time_t &ConfigurationView::getLastAccessTime(void) const { return lastAccessTime_; }

//==============================================================================
const bool &ConfigurationView::getLooseColumnMatching(void) const { return fillWithLooseColumnMatching_; }
...
const unsigned int &ConfigurationView::getSourceColumnMismatch(void) const { return sourceColumnMismatchCount_; }

//==============================================================================
// getSourceColumnMissing
//	The source information is only valid after modifying the table with ::fillFromJSON
const unsigned int &ConfigurationView::getSourceColumnMissing(void) const { return sourceColumnMissingCount_; }

//==============================================================================
// getSourceColumnNames
//	The source information is only valid after modifying the table with ::fillFromJSON
const std::set<std::string> &ConfigurationView::getSourceColumnNames(void) const { return sourceColumnNames_; }

//==============================================================================
std::set<std::string> ConfigurationView::getColumnNames(void) const {
  std::set<std::string> retSet;
  for (auto &colInfo : columnsInfo_) retSet.emplace(colInfo.getName());
  return retSet;
}
...
std::set<std::string> ConfigurationView::getColumnStorageNames(void) const {
  std::set<std::string> retSet;
  for (auto &colInfo : columnsInfo_) retSet.emplace(colInfo.getStorageName());
  return retSet;
}
...
//==============================================================================
unsigned int ConfigurationView::getNumberOfRows(void) const { return theDataView_.size(); }

//==============================================================================
unsigned int ConfigurationView::getNumberOfColumns(void) const { return columnsInfo_.size(); }

//==============================================================================
const ConfigurationView::DataView &ConfigurationView::getDataView(void) const { return theDataView_; }
...
//==============================================================================
const std::vector<ViewColumnInfo> &ConfigurationView::getColumnsInfo(void) const { return columnsInfo_; }

//==============================================================================
std::vector<ViewColumnInfo> *ConfigurationView::getColumnsInfoP(void) { return &columnsInfo_; }
//==============================================================================
...
//==============================================================================
void ConfigurationView::setTableName(const std::string &name) { tableName_ = name; }

//==============================================================================
void ConfigurationView::setComment(const std::string &comment) { comment_ = comment; }
...
//==============================================================================
void ConfigurationView::setAuthor(const std::string &author) { author_ = author; }

//==============================================================================
void ConfigurationView::setCreationTime(time_t t) { creationTime_ = t; }

//==============================================================================
void ConfigurationView::setLastAccessTime(time_t t) { lastAccessTime_ = t; }

//==============================================================================
void ConfigurationView::setLooseColumnMatching(bool setValue) { fillWithLooseColumnMatching_ = setValue; }
...
void ConfigurationView::print(std::ostream &out) const {
  ...
  for (int i = 0; i < (int)columnsInfo_.size(); ++i)
    out << i << ":" << columnsInfo_[i].getName() << ":" << columnsInfo_[i].getStorageName() << ":"
      ...
      if (columnsInfo_[c].getType() == ViewColumnInfo::TYPE_FIXED_CHOICE_DATA) {
        ...
        if (val == columnsInfo_[c].getDefaultValue())
          ...
          for (int i = 0; i < (int)choices.size(); ++i)
            if (val == choices[i]) choiceIndex = i + 1;
        }
...
void ConfigurationView::printJSON(std::ostream &out) const {
  ...
  for (int c = 0; c < (int)getNumberOfColumns(); ++c) {
    ...
    if (c + 1 < (int)getNumberOfColumns()) out << ",";
    out << "\n";
  }
  out << "},\n";  // close COL_TYPES
  ...
  for (int r = 0; r < (int)getNumberOfRows(); ++r) {
    ...
    out << "\t}";
    if (r + 1 < (int)getNumberOfRows()) out << ",";
    out << "\n";
  }
  out << "]\n";  // close DATA_SET
...
std::string restoreJSONStringEntities(const std::string &str) {
  ...
  if (i == sz - 1) retStr << str[sz - 1];  // output last character (which can't escape anything)
...
int ConfigurationView::fillFromJSON(const std::string &json) {
  ...
  for (; i < json.size(); ++i) {
    switch (json[i]) {
      ...
      case '\r':
        if (inQuotes) break;  // skip if in quote
        if (startNumber != (unsigned int)-1 && endNumber == (unsigned int)-1) endNumber = i;
        startNumber = i;
        break;
      default:;
    }
    ...
    if (0)  // for debugging
    {
      ...
      std::cout << "ExtKey=";
      for (unsigned int k = 0; k < jsonPath.size(); ++k) std::cout << jsonPath[k] << "/";
      std::cout << " - ";
    ...
    if (newValue) {
      ...
      for (unsigned int k = 0; k < keys.size(); ++k)
        if ((currDepth == 1 && keys[k] == currKey) || (currDepth > 1 && keys[k] == jsonPath[1])) matchedKey = k;
            ...
            if (currDepth == 2)  // second level depth
            {
              ...
              for (; ccnt < noc; ++ccnt) {
                ...
                if (fillWithLooseColumnMatching_) {
                  ...
                  theDataView_[row][col] =
                      currVal;  // THERE IS NO CHECK FOR WHAT IS READ FROM THE DATABASE. IT SHOULD BE ALREADY CONSISTENT
                  break;
                } else {
                  ...
                  for (keyIsMatchIndex = 0, keyIsMatchStorageIndex = 0, keyIsMatchCommentIndex = 0;
                    ...
                    if (currKey[keyIsMatchIndex] == '_') continue;  // skip to next character
...
//	{
//		__SS__ << "Invalid row (" << (int)r << ") col (" << (int)c << ") requested!" << __E__;
//		__SS_THROW__;
//	}
//
...
int ConfigurationView::fillFromCSV(const std::string &data, const int &dataOffset,
  ...
  while (k != (int)(std::string::npos)) {
    ...
    while (j < k && j != (int)(std::string::npos)) {
      ...
      if (setURIEncodedValue(data.substr(i, j - i), r, c)) rowWasModified = true;
...
bool ConfigurationView::setURIEncodedValue(const std::string &value, const unsigned int &r, const unsigned int &c,
  ...
  if (columnsInfo_[c].getDataType() == ViewColumnInfo::DATATYPE_NUMBER) {
    ...
    if (!StringMacros::isNumber(convertedString)) {
      __SS__ << "\tIn configuration " << tableName_ << " at column=" << columnsInfo_[c].getName() << " the value set ("
             << convertedString << ")"
...
unsigned int ConfigurationView::addRow(const std::string &author, bool incrementUniqueData, std::string baseNameAutoUID,
                                       unsigned int rowToAdd) {
  // default to last row
  if (rowToAdd == (unsigned int)-1) rowToAdd = getNumberOfRows();
  ...
  for (unsigned int r = getNumberOfRows() - 2; r >= rowToAdd; --r) {
    ...
    for (unsigned int col = 0; col < getNumberOfColumns(); ++col) theDataView_[r + 1][col] = theDataView_[r][col];
  }
  ...
  for (unsigned int col = 0; col < getNumberOfColumns(); ++col) {
    ...
    if (incrementUniqueData &&
        (col == getColUID() ||
         (getNumberOfRows() > 1 && columnsInfo_[col].getType() == ViewColumnInfo::TYPE_UNIQUE_DATA))) {
...
void ConfigurationView::deleteRow(int r) {
  if (r >= (int)getNumberOfRows()) {
    // out of bounds
    __SS__ << "Row " << (int)r << " is out of bounds (Row Count = " << getNumberOfRows() << ") and can not be deleted."
           << __E__;
    __SS_THROW__;
  }
...
const bool ConfigurationView::getChildLink(
    const unsigned int &c, bool &isGroup,
  ...
  if ((isGroup = columnsInfo_[c].isChildLinkGroupID()) || columnsInfo_[c].isChildLinkUID()) {
    ...
    __SS__ << "\tIn view: " << tableName_ << ", Can't find complete child link for column name "
           << columnsInfo_[c].getName() << __E__;
    __SS_THROW__;
  }

  if (!columnsInfo_[c].isChildLink()) return false;  // cant be unique link
  ...
  // if here then invalid table!
  __SS__ << "\tIn view: " << tableName_ << ", Can't find complete child link id for column name "
         << columnsInfo_[c].getName() << __E__;
  __SS_THROW__;
}
