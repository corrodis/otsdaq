...
//==============================================================================
ConfigurationInfoReader::ConfigurationInfoReader(bool allowIllegalColumns) : allowIllegalColumns_(allowIllegalColumns) {
  initPlatform();
...
//==============================================================================
void ConfigurationInfoReader::setAllowColumnErrors(bool setValue) { allowIllegalColumns_ = setValue; }
//==============================================================================
const bool& ConfigurationInfoReader::getAllowColumnErrors(void) { return allowIllegalColumns_; }
...
bool ConfigurationInfoReader::checkViewType(std::string type) {
  ...
  for (unsigned int i = 0; i < types.size(); i++) {
    if (types[i] == systemType) return true;
  }
  ...
  if (systemType != allowedNames[0] && systemType != allowedNames[1] && systemType != allowedNames[2]) {
    __COUT__ << "The type defined in CONFIGURATION_TYPE (" << systemType
             << ") doesn't match with any of the allowed types: File,Database or DatabaseTest" << std::endl;
  ...
  for (unsigned int i = 0; i < types.size(); i++) {
    if (types[i] != allowedNames[0] && types[i] != allowedNames[1] && types[i] != allowedNames[2]) {
      __COUT__ << "The type defined in the info file (" << types[i]
               << ") doesn't match with any of the allowed types: " << allowedNames[0] << ", " << allowedNames[1]
               << " or " << allowedNames[2] << std::endl;
...
//==============================================================================
xercesc::DOMNode* ConfigurationInfoReader::getNode(XMLCh* tagName, xercesc::DOMElement* parent,
...
//==============================================================================
xercesc::DOMElement* ConfigurationInfoReader::getElement(XMLCh* tagName, xercesc::DOMNode* parent,
                                                         unsigned int itemNumber) {
  return dynamic_cast<xercesc::DOMElement*>(getNode(tagName, parent, itemNumber));
}

//==============================================================================
xercesc::DOMElement* ConfigurationInfoReader::getElement(XMLCh* tagName, xercesc::DOMElement* parent,
                                                         unsigned int itemNumber) {
  return dynamic_cast<xercesc::DOMElement*>(getNode(tagName, parent, itemNumber));
}
...
std::string ConfigurationInfoReader::read(ConfigurationBase& configuration) {
  ...
  //	if (std::equal(tmp_test_config_prefix.begin(), tmp_test_config_prefix.end(), tmp_config_name.begin())) return
  //accumulatedExceptions;  KEEP End debugging... for Gennadiy...

  // These environment variables are required
  if (getenv("CONFIGURATION_TYPE") == NULL)
    __COUT__ << "Missing env variable: CONFIGURATION_TYPE. It must be set!" << std::endl;
  if (getenv("CONFIGURATION_DATA_PATH") == NULL)
    __COUT__ << "Missing env variable: CONFIGURATION_DATA_PATH. It must be set!" << std::endl;
  if (getenv("CONFIGURATION_INFO_PATH") == NULL)
    __COUT__ << "Missing env variable: CONFIGURATION_INFO_PATH. It must be set!" << std::endl;
  ...
  try {
    ...
    if (configuration.getConfigurationName() !=
        XML_TO_CHAR(configurationElement->getAttribute(configurationNameAttributeTag_))) {
      __SS__ << "In " << configFile << " the configuration name "
             << XML_TO_CHAR(configurationElement->getAttribute(configurationNameAttributeTag_))
    ...
    if (viewNodeList->getLength() != 1) {
      __SS__ << "In " << configFile << " the configuration name "
             << XML_TO_CHAR(configurationElement->getAttribute(configurationNameAttributeTag_))
             << " there must only be one view. There were " << viewNodeList->getLength() << " found." << std::endl;
    ...
    for (XMLSize_t view = 0; view < viewNodeList->getLength(); view++) {
      ...
      if (!checkViewType(viewType)) continue;
      ...
      for (XMLSize_t column = 0; column < columnNodeList->getLength(); column++) {
        ...
        configuration.getMockupViewP()->getColumnsInfoP()->push_back(ViewColumnInfo(
        ...
        if (capturedException != "") accumulatedExceptions += std::string("\n\nColumn Error:") + capturedException;
      ...
      std::string configurationDescription = XML_TO_CHAR(viewElement->getAttribute(viewDescriptionAttributeTag_));

      configuration.setConfigurationDescription(StringMacros::decodeURIComponent(configurationDescription));
      //__COUT__ << "configurationDescription = " << configurationDescription << std::endl;

      //</VIEW>
    }
    if (!storageTypeFound) {
      __COUT__ << "The type defined in CONFIGURATION_TYPE (" << getenv("CONFIGURATION_TYPE")
...
std::string ConfigurationInfoReader::read(ConfigurationBase* configuration) { return read(*configuration); }
