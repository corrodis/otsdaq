...
class ConfigurationView {
 public:
  ...
  ConfigurationView& copy(const ConfigurationView& src, ConfigurationVersion destinationVersion,
                          const std::string& author);
  unsigned int copyRows(const std::string& author, const ConfigurationView& src, unsigned int srcOffsetRow = 0,
                        unsigned int srcRowsToCopy = (unsigned int)-1, unsigned int destOffsetRow = (unsigned int)-1,
  ...
  template <class T>  // in included .icc source
  unsigned int findRowInGroup(unsigned int col, const T& value, const std::string& groupId,
                              const std::string& childLinkIndex, unsigned int offsetRow = 0) const;
  unsigned int findRowInGroup(unsigned int col, const std::string& value, const std::string& groupId,
                              const std::string& childLinkIndex, unsigned int offsetRow = 0) const;
 ...
 private:
  bool isEntryInGroupCol(const unsigned int& row, const unsigned int& groupCol, const std::string& groupNeedle,
 ...
 public:
  ...
  const bool getChildLink(const unsigned int& col, bool& isGroup,
  ...
  void addRowToGroup(const unsigned int& row, const unsigned int& col,
                     const std::string& groupID);  //, const std::string& colDefault);
  bool removeRowFromGroup(const unsigned int& row, const unsigned int& col, const std::string& groupID,
  ...
  void getValue(std::string& value, unsigned int row, unsigned int col,
  ...
  // special version of getValue for string type
  //	Note: necessary because types of std::basic_string<char> cause compiler problems if no string specific function
  std::string validateValueForColumn(const std::string& value, unsigned int col,
                                     bool convertEnvironmentVariables = true) const;

  std::string getValueAsString(unsigned int row, unsigned int col, bool convertEnvironmentVariables = true) const;
  std::string getEscapedValueAsString(unsigned int row, unsigned int col,
                                      bool convertEnvironmentVariables = true) const;
  ...
  unsigned int addRow(const std::string& author = "", bool incrementUniqueData = false,
                      std::string baseNameAutoUID = "",
                      unsigned int rowToAdd = (unsigned int)-1);  // returns index of added row, default is last row
  void deleteRow(int r);

  // Lore did not like this.. wants special access through separate Supervisor for "Database Management" int
  // addColumn(std::string name, std::string viewName, std::string viewType); //returns index of added column, always is
  // last column unless
  ...
  int fillFromCSV(const std::string& data, const int& dataOffset = 0,
 ...
 private:
  ...
  ConfigurationView& operator=(const ConfigurationView src);  // operator= is purposely undefined and private (DO NOT
                                                              // USE IT!) - should use ConfigurationView::copy()

  std::string uniqueStorageIdentifier_;  // starts empty "", used to implement re-writable views ("temporary views") in
                                         // artdaq db
