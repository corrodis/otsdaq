    ...
    , bitMapInfoP_(0)
{
	...
	if (
	    ...
	    !isGroupID())
	{
		__SS__ << "The type for column " << name_ << " is " << type_ << ", while the only accepted types are: " << TYPE_DATA << " " << TYPE_UNIQUE_DATA << " " << TYPE_UNIQUE_GROUP_DATA << " " << TYPE_MULTILINE_DATA << " " << TYPE_FIXED_CHOICE_DATA << " " << TYPE_UID << " " << TYPE_ON_OFF << " " << TYPE_TRUE_FALSE << " " << TYPE_YES_NO << " " << TYPE_START_CHILD_LINK << "-* " << TYPE_START_CHILD_LINK_UID << "-* " << TYPE_START_CHILD_LINK_GROUP_ID << "-* " << TYPE_START_GROUP_ID << "-* " << std::endl;
	...
	{
		__SS__ << "The data type for column " << name_ << " is " << dataType_ << ", while the only accepted types are: " << DATATYPE_NUMBER << " " << DATATYPE_STRING << " " << DATATYPE_TIME << std::endl;
		if (capturedExceptionString)
			*capturedExceptionString += ss.str();
		else
			__SS_THROW__;
...
void ViewColumnInfo::extractBitMapInfo()
{
	...
	if (type_ == TYPE_BITMAP_DATA)
	{
		...
		{
			__SS__ << "The Bit-Map data parameters for column " << name_ << " should be size 16, but is size " << dataChoices_.size() << ". Bit-Map parameters should be rows, cols, cellBitSize, and min, mid, max color." << std::endl;
			__SS_THROW__;
		}
