...
ViewColumnInfo::ViewColumnInfo(const std::string& type, const std::string& name, const std::string& storageName,
                               const std::string& dataType, const std::string& dataChoicesCSV,
                               std::string* capturedExceptionString)
    : type_(type), name_(name), storageName_(storageName), dataType_(dataType), bitMapInfoP_(0) {
  // verify type
  if ((type_ != TYPE_UID) && (type_ != TYPE_DATA) && (type_ != TYPE_UNIQUE_DATA) && (type_ != TYPE_UNIQUE_GROUP_DATA) &&
      (type_ != TYPE_MULTILINE_DATA) && (type_ != TYPE_FIXED_CHOICE_DATA) && (type_ != TYPE_BITMAP_DATA) &&
      (type_ != TYPE_ON_OFF) && (type_ != TYPE_TRUE_FALSE) && (type_ != TYPE_YES_NO) && (type_ != TYPE_COMMENT) &&
      (type_ != TYPE_AUTHOR) && (type_ != TYPE_TIMESTAMP) && !isChildLink() && !isChildLinkUID() &&
      !isChildLinkGroupID() && !isGroupID()) {
    __SS__ << "The type for column " << name_ << " is " << type_ << ", while the only accepted types are: " << TYPE_DATA
           << " " << TYPE_UNIQUE_DATA << " " << TYPE_UNIQUE_GROUP_DATA << " " << TYPE_MULTILINE_DATA << " "
           << TYPE_FIXED_CHOICE_DATA << " " << TYPE_UID << " " << TYPE_ON_OFF << " " << TYPE_TRUE_FALSE << " "
           << TYPE_YES_NO << " " << TYPE_START_CHILD_LINK << "-* " << TYPE_START_CHILD_LINK_UID << "-* "
  ...
  for (unsigned int i = 0; i < type_.size(); ++i)
    if (!((type_[i] >= 'A' && type_[i] <= 'Z') || (type_[i] >= 'a' && type_[i] <= 'z') ||
          (type_[i] >= '0' && type_[i] <= '9') || (type_[i] == '-' || type_[i] <= '_' || type_[i] <= '.'))) {
  ...
  if ((dataType_ != DATATYPE_NUMBER) && (dataType_ != DATATYPE_STRING) && (dataType_ != DATATYPE_TIME)) {
    __SS__ << "The data type for column " << name_ << " is " << dataType_
           << ", while the only accepted types are: " << DATATYPE_NUMBER << " " << DATATYPE_STRING << " "
  ...
  for (unsigned int i = 0; i < dataType_.size(); ++i)
    if (!((dataType_[i] >= 'A' && dataType_[i] <= 'Z') || (dataType_[i] >= 'a' && dataType_[i] <= 'z') ||
          (dataType_[i] >= '0' && dataType_[i] <= '9') || (dataType_[i] == '-' || dataType_[i] <= '_'))) {
      __SS__ << "The data type for column " << name_ << " is '" << dataType_
             << "'. Data types must contain only letters, numbers,"
  ...
  for (unsigned int i = 0; i < name_.size(); ++i)
    if (!((name_[i] >= 'A' && name_[i] <= 'Z') || (name_[i] >= 'a' && name_[i] <= 'z') ||
          (name_[i] >= '0' && name_[i] <= '9') || (name_[i] == '-' || name_[i] <= '_'))) {
  ...
  //__COUT__ << "dataChoicesCSV " << dataChoicesCSV << std::endl;
  {
    ...
    std::string s;
    while (getline(f, s, ',')) dataChoices_.push_back(StringMacros::decodeURIComponent(s));
...
void ViewColumnInfo::extractBitMapInfo() {
  ...
  if (type_ == TYPE_BITMAP_DATA) {
    ...
    if (dataChoices_.size() < 16) {
      __SS__ << "The Bit-Map data parameters for column " << name_ << " should be size 16, but is size "
             << dataChoices_.size()
             << ". Bit-Map parameters should be rows, cols, cellBitSize, and min, mid, max color." << std::endl;
      __SS_THROW__;
    }
...
//==============================================================================
const std::string& ViewColumnInfo::getType(void) const { return type_; }

//==============================================================================
const std::string& ViewColumnInfo::getDefaultValue(void) const {
  if (getDataType() == ViewColumnInfo::DATATYPE_STRING) {
    if (getType() == ViewColumnInfo::TYPE_ON_OFF || getType() == ViewColumnInfo::TYPE_TRUE_FALSE ||
...
//==============================================================================
// isNumberDataType
const bool ViewColumnInfo::isNumberDataType(void) const { return (dataType_ == DATATYPE_NUMBER); }  // end isBoolType()

//==============================================================================
const std::string& ViewColumnInfo::getName(void) const { return name_; }

//==============================================================================
const std::string& ViewColumnInfo::getStorageName(void) const { return storageName_; }

//==============================================================================
const std::string& ViewColumnInfo::getDataType(void) const { return dataType_; }

//==============================================================================
const std::vector<std::string>& ViewColumnInfo::getDataChoices(void) const { return dataChoices_; }
...
const bool ViewColumnInfo::isChildLink(void) const {
  return (type_.find(TYPE_START_CHILD_LINK) == 0 && type_.length() > TYPE_START_CHILD_LINK.length() &&
...
const bool ViewColumnInfo::isGroupID(void) const {
  return (type_.find(TYPE_START_GROUP_ID) == 0 && type_.length() > TYPE_START_GROUP_ID.length() &&
          type_[TYPE_START_GROUP_ID.length()] == '-');
}

//==============================================================================
// isUID
const bool ViewColumnInfo::isUID(void) const { return (type_ == TYPE_UID); }
