...
//==============================================================================
IterateConfiguration::IterateConfiguration(void) : ConfigurationBase(IterateConfiguration::ITERATE_TABLE) {}
...
//==============================================================================
std::vector<IterateConfiguration::Command> IterateConfiguration::getPlanCommands(ConfigurationManager* configManager,
  ...
  auto commandChildren = planNode.getNode(IterateConfiguration::iterateTableCols_.PlanLink_).getChildren();

  for (auto& commandChild : commandChildren) {
    __COUT__ << "Command \t" << commandChild.first << std::endl;

    __COUT__ << "\t\tStatus \t" << commandChild.second.getNode(IterateConfiguration::planTableCols_.Status_)
             << std::endl;

    __COUT__ << "\t\tType \t" << commandChild.second.getNode(IterateConfiguration::planTableCols_.CommandType_)
             << std::endl;

    if (!commandChild.second.getNode(IterateConfiguration::planTableCols_.Status_).getValue<bool>())
      continue;  // skip disabled commands

    commands.push_back(IterateConfiguration::Command());
    commands.back().type_ =
        commandChild.second.getNode(IterateConfiguration::planTableCols_.CommandType_).getValue<std::string>();

    if (commandChild.second.getNode(IterateConfiguration::planTableCols_.CommandLink_).isDisconnected())
      continue;  // skip if no command parameters

    auto commandSpecificFields =
        commandChild.second.getNode(IterateConfiguration::planTableCols_.CommandLink_).getChildren();
    ...
    for (unsigned int i = 0; i < commandSpecificFields.size() - 3; ++i)  // ignore last three columns
    {
      ...
      __COUT__ << "\t\tParameter \t" << commandSpecificFields[i].first << " = \t" << commandSpecificFields[i].second
               << std::endl;

      if (commandSpecificFields[i].first == IterateConfiguration::commandTargetCols_.TargetsLink_) {
        ...
        for (auto& target : targets) {
          ...
          ConfigurationTree targetNode = target.second.getNode(IterateConfiguration::targetCols_.TargetLink_);
          ...
          __COUT__ << "\t\t = \t"
                   << "Table:" << targetNode.getConfigurationName() << " UID:" << targetNode.getUIDAsString()
                   << std::endl;
        ...
        for (auto& dimensionalLoop : dimensionalLoops) {
          ...
          numberOfIterations =
              dimensionalLoop.second.getNode(IterateConfiguration::macroDimLoopCols_.NumberOfIterations_)
                  .getValue<unsigned long>();

          __COUTV__(numberOfIterations);

          if (numberOfIterations == 0) {
            __SS__ << "Illegal number of iterations value of '" << numberOfIterations
          ...
          if (!firstDimension) argStr += ";";
          firstDimension = false;
          argStr += std::to_string(numberOfIterations);

          auto paramLinkNode = dimensionalLoop.second.getNode(IterateConfiguration::macroDimLoopCols_.ParamLink_);
          ...
          for (auto& macroParam : macroParams) {
            ...
            argStr += ",";
            argStr += macroParam.second.getNode(IterateConfiguration::macroParamCols_.Name_).getValue<std::string>();
            argStr += ":";
            argStr += macroParam.second.getNode(IterateConfiguration::macroParamCols_.Value_).getValue<std::string>();
            argStr += ":";
            argStr += macroParam.second.getNode(IterateConfiguration::macroParamCols_.Step_).getValue<std::string>();
        ...
        commands.back().params_.emplace(std::pair<std::string /*param name*/, std::string /*param value*/>(
            IterateConfiguration::commandExecuteMacroParams_.MacroArgumentString_, argStr));
