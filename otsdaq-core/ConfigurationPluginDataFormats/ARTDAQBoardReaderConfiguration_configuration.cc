...
void ARTDAQBoardReaderConfiguration::init(ConfigurationManager *configManager) {
  ...
  std::vector<const XDAQContextConfiguration::XDAQContext *> readerContexts = contextConfig->getBoardReaderContexts();
  ...
  for (auto &readerContext : readerContexts) {
    try {
      ConfigurationTree readerConfigNode = contextConfig->getSupervisorConfigNode(
          configManager, readerContext->contextUID_, readerContext->applications_[0].applicationUID_);

      __COUT__ << "Path for this reader config is " << readerContext->contextUID_ << "/"
               << readerContext->applications_[0].applicationUID_ << "/" << readerConfigNode.getValueAsString()
               << std::endl;
      ...
      __COUT_ERR__
          << "This board reader will likely not get instantiated properly! Proceeding anyway with fcl generation."
          << std::endl;

      // proceed anyway, because it was really annoying to not be able to activate the configuration group when the
      // context is being developed also.
      //__SS_THROW__;
    }
  ...
  for (auto &child : childrenMap) {
    ...
    if (thisContext == nullptr) {
      ...
      if (child.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) {
        outputFHICL(configManager, child.second, contextConfig->getARTDAQAppRank(thisContext->contextUID_),
                    contextConfig->getContextAddress(thisContext->contextUID_),
                    contextConfig->getARTDAQDataPort(configManager, thisContext->contextUID_), contextConfig);
      }
    }
  }
}
...
void ARTDAQBoardReaderConfiguration::outputFHICL(ConfigurationManager *configManager,
                                                 const ConfigurationTree &boardReaderNode, unsigned int selfRank,
                                                 std::string selfHost, unsigned int selfPort,
  ...
  /*
          the file will look something like this:
            ...
            daq: {
                  ...
                  request_window_offset: 0 # Request message contains tzero. Window will be from tzero - offset to tzero
     + width request_window_width: 0 stale_request_timeout: "0xFFFFFFFF" # How long to wait before discarding request
     messages that are outside the available data request_windows_are_unique: true # If request windows are unique,
     avoids a copy operation, but the same data point cannot be used for two requests. If this is not anticipated, leave
     set to "true"

                  separate_data_thread: false # MUST be true for triggers to be applied! If triggering is not desired,
     but a separate readout thread is, set this to true, triggers_enabled to false and trigger_mode to ignored.
                  separate_monitoring_thread: false # Whether a thread should be started which periodically calls
     checkHWStatus_, a user-defined function which should be used to check hardware status registers and report to
     MetricMan. poll_hardware_status: false # Whether checkHWStatus_ will be called, either through the thread or at the
     start of getNext hardware_poll_interval_us: 1000000 # If hardware monitoring thread is enabled, how often should it
     call checkHWStatus_
