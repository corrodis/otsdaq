...
#define XDAQ_RUN_FILE \
  std::string(getenv("XDAQ_CONFIGURATION_DATA_PATH")) + "/" + std::string(getenv("XDAQ_CONFIGURATION_XML")) + ".xml"
...
//"StartXDAQ_gen.sh" #define ARTDAQ_MPI_SCRIPT		std::string(getenv("XDAQ_CONFIGURATION_DATA_PATH")) + "/"+
//"StartMPI_gen.sh"

const std::string XDAQContextConfiguration::DEPRECATED_SUPERVISOR_CLASS =
...
const std::set<std::string> XDAQContextConfiguration::FETypeClassNames_ = {
    "ots::FESupervisor", "ots::FEDataManagerSupervisor", "ots::ARTDAQFEDataManagerSupervisor"};
const std::set<std::string> XDAQContextConfiguration::DMTypeClassNames_ = {
    "ots::DataManagerSupervisor", "ots::FEDataManagerSupervisor", "ots::ARTDAQFEDataManagerSupervisor"};
const std::set<std::string> XDAQContextConfiguration::LogbookTypeClassNames_ = {"ots::LogbookSupervisor"};
...
const std::set<std::string> XDAQContextConfiguration::ConfigurationGUITypeClassNames_ = {
...
//========================================================================================================================
XDAQContextConfiguration::XDAQContextConfiguration(void) : ConfigurationBase("XDAQContextConfiguration") {
  //////////////////////////////////////////////////////////////////////
  ...
  //xsi:noNamespaceSchemaLocation="ConfigurationInfo.xsd"> 	   <CONFIGURATION Name="XDAQContextConfiguration">
  //	     <VIEW Name="XDAQ_CONTEXT_CONFIGURATION" Type="File,Database,DatabaseTest">
  //           <COLUMN Type="UID" 	           Name="ContextUID" 	                 StorageName="CONTEXT_UID"
  //           DataType="VARCHAR2"/> <COLUMN Type="ChildLink-0" 	   Name="LinkToApplicationTable"
  //           StorageName="LINK_TO_APPLICATION_CONFIGURATION" DataType="VARCHAR2"/> <COLUMN Type="ChildLinkGroupID-0"
  //           Name="ApplicationGroupID" 	         StorageName="APPLICATION_GROUP_ID" DataType="VARCHAR2"/>
  //           <COLUMN Type="OnOff" 	           Name="Status" 	                     StorageName="STATUS"
  //           DataType="VARCHAR2"/> <COLUMN Type="Data" 	           Name="Id"
  //           StorageName="ID" 		                         DataType="VARCHAR2"/> <COLUMN Type="Data"
  //           Name="Address" 	                     StorageName="ADDRESS" DataType="VARCHAR2"/> <COLUMN
  //           Type="Data" 	           Name="Port"                         	 StorageName="PORT"
  //           DataType="VARCHAR2"/> <COLUMN Type="Data" 	           Name="ARTDAQDataPort"
  //           StorageName="ARTDAQ_DATA_PORT"  	             DataType="VARCHAR2"/> <COLUMN Type="Comment"
  //           Name="CommentDescription" 	         StorageName="COMMENT_DESCRIPTION" DataType="VARCHAR2"/>
  //           <COLUMN Type="Author" 	           Name="Author" 	                     StorageName="AUTHOR"
  //           DataType="VARCHAR2"/> <COLUMN Type="Timestamp" 	       Name="RecordInsertionTime"
...
unsigned int XDAQContextConfiguration::getARTDAQAppRank(const std::string &contextUID) const {
  ...
  if (artdaqBoardReaders_.size() == 0 && artdaqEventBuilders_.size() == 0 && artdaqAggregators_.size() == 0) {
    __COUT_WARN__ << "Assuming since there are 0 active ARTDAQ context UID(s), we can ignore rank failure."
                  << std::endl;
    return -1;
  }
  ...
  auto localGetRank = [](const XDAQContext &context) -> unsigned int {
    if (context.applications_.size() != 1) {
      __SS__ << "Invalid number of ARTDAQ applications in context '" << context.contextUID_
  ...
  for (auto &i : artdaqBoardReaders_)
    if (contexts_[i].contextUID_ == contextUID) return localGetRank(contexts_[i]);
  ...
  __SS__ << "ARTDAQ rank could not be found for context UID '" << contextUID << ".'" << std::endl;
...
unsigned int XDAQContextConfiguration::getARTDAQDataPort(const ConfigurationManager *configManager,
  ...
  for (auto &context : contexts_) {
    if (context.contextUID_ == contextUID) {
      if (context.applications_.size() != 1) {
        __SS__ << "Invalid number of ARTDAQ applications in context '" << contextUID << ".' Must be 1. Currently is "
      ...
      if (context.applications_[0].class_ == "ots::ARTDAQDataManagerSupervisor") {
        auto processors =
            getSupervisorConfigNode(configManager, context.contextUID_, context.applications_[0].applicationUID_)
        ...
        for (auto &processor : processors) {
          processorType = processor.second.getNode("ProcessorPluginName").getValue<std::string>();
          __COUTV__(processorType);

          if (processorType == "ARTDAQConsumer" || processorType == "ARTDAQProducer")
            return processor.second.getNode("LinkToProcessorTable")
                .getNode(XDAQContextConfiguration::ARTDAQ_OFFSET_PORT)
      ...
      // else
      return getSupervisorConfigNode(configManager, context.contextUID_, context.applications_[0].applicationUID_)
          .getNode(XDAQContextConfiguration::ARTDAQ_OFFSET_PORT)
          .getValue<unsigned int>();
    }
  }
...
ConfigurationTree XDAQContextConfiguration::getApplicationNode(const ConfigurationManager *configManager,
  ...
  return configManager->__SELF_NODE__.getNode(contextUID + "/" + colContext_.colLinkToApplicationTable_ + "/" + appUID);
}
...
void XDAQContextConfiguration::extractContexts(ConfigurationManager *configManager) {
  ...
  //	Note: this is important because there are maps in MacroMaker and SupervisorDescriptorInfoBase that rely on
  //localId() as key
  ...
  for (auto &child : children) {
    ...
    for (auto appChild : appChildren) {
      ...
      contexts_.back().applications_.back().sourceConfig_ =
          appChild.second.getConfigurationName() + "_v" + appChild.second.getConfigurationVersion().toString() + " @ " +
      ...
      if ((contexts_.back().applications_.back().id_ == 200 &&
        ...
        __SS__ << "XDAQ Application ID of 200 is reserved for the Gateway Supervisor "
               ...
               << " appName=" << contexts_.back().applications_.back().applicationUID_ << std::endl;
        __SS_THROW__;
      }
      ...
      if (contexts_.back().applications_.back().class_ == XDAQContextConfiguration::DEPRECATED_SUPERVISOR_CLASS) {
        contexts_.back().applications_.back().class_ = XDAQContextConfiguration::GATEWAY_SUPERVISOR_CLASS;
        __COUT__ << "Fixing deprecated Supervisor class from " << XDAQContextConfiguration::DEPRECATED_SUPERVISOR_CLASS
      ...
      if (contexts_.back().applications_.back().module_.find("libSupervisor.so") != std::string::npos) {
        __COUT__ << "Fixing deprecated Supervisor class from " << contexts_.back().applications_.back().module_
                 << " to ";
      ...
      try {
        ...
        appChild.second.getNode(colApplication_.colStopPriority_)
            .getValue(contexts_.back().applications_.back().stateMachineCommandPriority_["Stop"]);
      } catch (...) {
    ...
    if (isARTDAQContext(contexts_.back().contextUID_)) {
      ...
      if (contexts_.back().applications_.size() != 1) {
        __SS__ << "ARTDAQ Context '" << contexts_.back().contextUID_ << "' must have one Application! "
      ...
      if (contexts_.back().applications_[0].class_ ==  // if board reader
        ...
        __SS__ << "ARTDAQ Context must be have Application of an allowed class type:\n "
               ...
               << "\tots::DataLoggerApp\n"
               << "\tots::DispatcherApp\n"
               << "\nClass found was " << contexts_.back().applications_[0].class_ << std::endl;
        __SS_THROW__;
      }
    }
  }
...
//========================================================================================================================
// void XDAQContextConfiguration::outputXDAQScript(std::ostream &out)
//{
/*
...
void XDAQContextConfiguration::outputXDAQXML(std::ostream &out) {
  ...
  for (XDAQContext &context : contexts_) {
    ...
    sprintf(tmp, "\t<!-- ContextUID='%s' sourceConfig='%s' -->", context.contextUID_.c_str(),
    ...
    for (XDAQApplication &app : context.applications_) {
      ...
      out << "\t\t\t<properties xmlns=\"urn:xdaq-application:" << app.class_.substr(foundColon)
      ...
      for (XDAQApplicationProperty &appProperty : app.properties_) {
        ...
        if (!appProperty.status_) out << "\t\t\t\t<!--\n";

        sprintf(tmp, "\t\t\t\t<%s xsi:type=\"%s\">%s</%s>\n", appProperty.name_.c_str(), appProperty.type_.c_str(),
                appProperty.value_.c_str(), appProperty.name_.c_str());
        out << tmp;
      ...
      if (context.status_ && !app.status_) out << "\t\t-->\n";
      out << "\n";
...
std::string XDAQContextConfiguration::getContextUID(const std::string &url) const {
  for (auto context : contexts_) {
    ...
    if (url == context.address_ + ":" + std::to_string(context.port_)) return context.contextUID_;
  }
  return "";
}
...
DEFINE_OTS_CONFIGURATION(XDAQContextConfiguration)
