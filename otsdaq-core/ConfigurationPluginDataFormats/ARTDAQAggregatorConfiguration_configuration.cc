...
void ARTDAQAggregatorConfiguration::init(ConfigurationManager *configManager) {
  ...
  std::vector<const XDAQContextConfiguration::XDAQContext *> aggContexts = contextConfig->getAggregatorContexts();
  ...
  for (auto &aggContext : aggContexts) {
    ConfigurationTree aggConfigNode = contextConfig->getSupervisorConfigNode(
        configManager, aggContext->contextUID_, aggContext->applications_[0].applicationUID_);

    __COUT__ << "Path for this aggregator config is " << aggContext->contextUID_ << "/"
             << aggContext->applications_[0].applicationUID_ << "/" << aggConfigNode.getValueAsString() << std::endl;

    outputFHICL(configManager, aggConfigNode, contextConfig->getARTDAQAppRank(aggContext->contextUID_),
                contextConfig->getContextAddress(aggContext->contextUID_),
                contextConfig->getARTDAQDataPort(configManager, aggContext->contextUID_), contextConfig);
  }
}
...
void ARTDAQAggregatorConfiguration::outputFHICL(ConfigurationManager *configManager,
                                                const ConfigurationTree &aggregatorNode, unsigned int selfRank,
      ...
      try {
        ...
        for (auto &destination : destinations) {
          ...
          OUT << destination.second.getNode("destinationKey").getValue() << ": {"
              ...
              << " source_rank: " << selfRank << " destination_rank: " << destinationRank
