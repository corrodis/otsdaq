...
//==============================================================================
DesktopIconConfiguration::DesktopIconConfiguration(void) : ConfigurationBase("DesktopIconConfiguration") {
...
void DesktopIconConfiguration::init(ConfigurationManager *configManager) {
  ...
  for (auto &child : childrenMap) {
    ...
    if (icon->folderPath_ == ViewColumnInfo::DATATYPE_STRING_DEFAULT)
    ...
    if (!child.second.getNode(COL_APP_LINK).isDisconnected()) {
      ...
      if (icon->windowContentURL_[icon->windowContentURL_.size() - 1] != '=') icon->windowContentURL_ += "?urn=";
    ...
    if (!child.second.getNode(COL_PARAMETER_LINK).isDisconnected()) {
      ...
      for (const auto param : paramGroupMap) {
        if (notFirst)
          ...
          notFirst = true;
        icon->windowContentURL_ +=
            ConfigurationManager::encodeURIComponent(param.second.getNode(COL_PARAMETER_KEY).getValue<std::string>()) +
            "=" +
...
std::string DesktopIconConfiguration::removeCommas(const std::string &str, bool andHexReplace, bool andHTMLReplace) {
  std::string retStr = "";
