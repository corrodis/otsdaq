...
//========================================================================================================================
ARTDAQBuilderConfiguration::ARTDAQBuilderConfiguration(void) : ConfigurationBase("ARTDAQBuilderConfiguration") {
  //////////////////////////////////////////////////////////////////////
...
void ARTDAQBuilderConfiguration::init(ConfigurationManager *configManager) {
  ...
  std::vector<const XDAQContextConfiguration::XDAQContext *> builderContexts = contextConfig->getEventBuilderContexts();
  ...
  for (auto &builderContext : builderContexts) {
    ConfigurationTree builderAppNode = contextConfig->getApplicationNode(
        configManager, builderContext->contextUID_, builderContext->applications_[0].applicationUID_);
    ...
    __COUT__ << "Path for this aggregator config is " << builderContext->contextUID_ << "/"
             << builderContext->applications_[0].applicationUID_ << "/" << builderConfigNode.getValueAsString()
             << std::endl;

    outputFHICL(configManager, builderConfigNode, contextConfig->getARTDAQAppRank(builderContext->contextUID_),
                contextConfig->getContextAddress(builderContext->contextUID_),
                contextConfig->getARTDAQDataPort(configManager, builderContext->contextUID_), contextConfig);
  }
}
...
//========================================================================================================================
void ARTDAQBuilderConfiguration::outputFHICL(ConfigurationManager *configManager, const ConfigurationTree &builderNode,
      ...
      try {
        ...
        for (auto &destination : destinations) {
          ...
          OUT << destination.second.getNode("destinationKey").getValue() << ": {"
              << " transferPluginType: " << destination.second.getNode("transferPluginType").getValue()
              << " destination_rank: " << destinationRank << " max_fragment_size_words: "
              << destination.second.getNode("ARTDAQGlobalConfigurationLink/maxFragmentSizeWords")
        ...
        __SS__ << "Are the Net Monitor Transport Service destinations valid? Error occurred looking for Event Builder "
                  "transport service destinations for UID '"
  ...
  if (!daq.isDisconnected()) {
    ...
    if (!parametersLink.isDisconnected()) {
      ...
      for (auto &parameter : parameters) {
        if (!parameter.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) PUSHCOMMENT;

        OUT << parameter.second.getNode("daqParameterKey").getValue() << ": "
            << parameter.second.getNode("daqParameterValue").getValue() << "\n";

        if (!parameter.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) POPCOMMENT;
      }
    }
      ...
      try {
        ...
        for (auto &source : sources) {
          ...
          OUT << source.second.getNode("sourceKey").getValue() << ": {"
              << " transferPluginType: " << source.second.getNode("transferPluginType").getValue()
    ...
    if (!metricsGroup.isDisconnected()) {
      ...
      for (auto &metric : metrics) {
        if (!metric.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) PUSHCOMMENT;
        ...
        if (!metricParametersGroup.isDisconnected()) {
          ...
          for (auto &metricParameter : metricParameters) {
            if (!metricParameter.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) PUSHCOMMENT;
            ...
            if (!metricParameter.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) POPCOMMENT;
          }
        }
        POPTAB;
        OUT << "}\n\n";  // end metric

        if (!metric.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) POPCOMMENT;
      }
    }
    POPTAB;
    OUT << "}\n\n";  // end metrics
  ...
  if (!outputs.isDisconnected()) {
    ...
    for (auto &outputPlugin : outputPlugins) {
      if (!outputPlugin.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) PUSHCOMMENT;

      OUT << outputPlugin.second.getNode("outputKey").getValue() << ": {\n";
      PUSHTAB;
      OUT << "module_type: " << outputPlugin.second.getNode("outputModuleType").getValue() << "\n";
      ...
      if (!pluginParameterLink.isDisconnected()) {
        ...
        for (auto &pluginParameter : pluginParameters) {
          if (!pluginParameter.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) PUSHCOMMENT;
          ...
          if (!pluginParameter.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) POPCOMMENT;
        }
      }
      POPTAB;
      OUT << "}\n\n";  // end output module

      if (!outputPlugin.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) POPCOMMENT;
    }
  ...
  if (!physics.isDisconnected()) {
    ...
    if (!analyzers.isDisconnected()) {
      ...
      for (auto &module : modules) {
        if (!module.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) PUSHCOMMENT;

        OUT << module.second.getNode("analyzerKey").getValue() << ": {\n";
        PUSHTAB;
        OUT << "module_type: " << module.second.getNode("analyzerModuleType").getValue() << "\n";
        ...
        if (!moduleParameterLink.isDisconnected()) {
          ...
          for (auto &moduleParameter : moduleParameters) {
            if (!moduleParameter.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) PUSHCOMMENT;
            ...
            if (!moduleParameter.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) POPCOMMENT;
          }
        }
        POPTAB;
        OUT << "}\n\n";  // end analyzer module

        if (!module.second.getNode(ViewColumnInfo::COL_NAME_STATUS).getValue<bool>()) POPCOMMENT;
      }
      POPTAB;
      OUT << "}\n\n";  // end analyzer
    }
    ...
    if (!producers.isDisconnected()) {
      ...
      for (auto &module : modules) {
        ...
        PUSHTAB;
        OUT << "module_type: " << module.second.getNode("producerModuleType").getValue() << "\n";
    ...
    if (!filters.isDisconnected()) {
      ...
      for (auto &module : modules) {
        ...
        PUSHTAB;
        OUT << "module_type: " << module.second.getNode("filterModuleType").getValue() << "\n";
  ...
  //--------------------------------------
  // handle process_name
  OUT << "process_name: " << builderNode.getNode("ARTDAQGlobalConfigurationForProcessNameLink/processNameForBuilders")
      << "\n";
