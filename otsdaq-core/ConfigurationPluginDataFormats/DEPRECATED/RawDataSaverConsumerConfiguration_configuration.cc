#include "otsdaq-core/ConfigurationPluginDataFormats/RawDataSaverConsumerConfiguration.h"
...
RawDataSaverConsumerConfiguration::RawDataSaverConsumerConfiguration(void)
  ...
  //Name="RawDataSaverConsumerConfiguration"> 	    <VIEW Name="RAW_DATA_SAVER_CONSUMER_CONFIGURATION"
  //Type="File,Database,DatabaseTest"> 	      <COLUMN Name="ProcessorID"   StorageName="PROCESSOR_ID"
  //DataType="VARCHAR2"/> 	      <COLUMN Name="FilePath"      StorageName="FILE_PATH"       DataType="VARCHAR2"/>
...
std::vector<std::string> RawDataSaverConsumerConfiguration::getProcessorIDList(void) const {
  ...
  for (auto& it : processorIDToRowMap_) list.push_back(it.first);
  return list;
}
