...
ModuleToFEConfiguration::ModuleToFEConfiguration(void)
    : TableBase("ModuleToFEConfiguration")
{
	//////////////////////////////////////////////////////////////////////
...
std::string       enumValue1;
	...
	{
	    TableBase::activeTableView_->getValue(enumValue1,row,Enum1);
	    TableBase::activeTableView_->getValue(enumValue2,row,Enum2);
	}
	 */
}

//==============================================================================
std::list<std::string> ModuleToFEConfiguration::getFEWModulesList(
    unsigned int FEWNumber) const
{
	...
	for(unsigned int row = 0; row < TableBase::activeTableView_->getNumberOfRows(); row++)
	{
		TableBase::activeTableView_->getValue(tmpFEW, row, FEWName);
		if(tmpFEW == FEWNumber)
		{
			TableBase::activeTableView_->getValue(moduleName, row, ModuleName);
...
//==============================================================================
std::list<std::string> ModuleToFEConfiguration::getFERModulesList(
    unsigned int FERNumber) const
{
	std::string            moduleName;
	unsigned int           tmpFER;
	std::list<std::string> list;
	for(unsigned int row = 0; row < TableBase::activeTableView_->getNumberOfRows(); row++)
	{
		TableBase::activeTableView_->getValue(tmpFER, row, FERName);
		if(tmpFER == FERNumber)
		{
			TableBase::activeTableView_->getValue(moduleName, row, ModuleName);
			list.push_back(moduleName);
