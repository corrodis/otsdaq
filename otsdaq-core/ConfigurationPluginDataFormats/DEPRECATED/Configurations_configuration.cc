...
//==============================================================================
Configurations::Configurations(void) : ConfigurationBase("Configurations") {
  //////////////////////////////////////////////////////////////////////
...
ConfigurationVersion Configurations::getConditionVersion(const ConfigurationGroupKey &ConfigurationGroupKey,
  ...
  std::cout << __COUT_HDR_FL__
            << "*******************************************************************************************************"
               "*********************"
            << std::endl;
  std::cout << __COUT_HDR_FL__ << "\tCan't find KOC " << koc << " with ConfigurationGroupKey " << ConfigurationGroupKey
...
int Configurations::setConditionVersionForView(ConfigurationView *cfgView, ConfigurationGroupKey ConfigurationGroupKey,
  ...
  for (row = 0; row < cfgView->getNumberOfRows(); row++) {
    ...
    if (ConfigurationGroupKey == tmpConfigurationGroupKey) {
      ...
      if (tmpKOC == koc) {
        ...
        std::cout << __COUT_HDR_FL__ << "Found ConfigKey(" << ConfigurationGroupKey << ") and KOCAlias(" << koc
                  << ") pair."
                  << " Current version is: " << tmpOldKOCVersion << " New version is: " << newKOCVersion << std::endl;
        if (newKOCVersion == tmpOldKOCVersion) return 1;  // no change necessary
...
void Configurations::getListOfKocsForView(ConfigurationView *cfgView, std::set<std::string> &kocs,
  ...
  for (unsigned int row = 0; row < cfgView->getNumberOfRows(); row++) {
    ...
    if (ConfigurationGroupKey == ConfigurationGroupKey::INVALID || ConfigurationGroupKey == tmpConfigurationGroupKey) {
