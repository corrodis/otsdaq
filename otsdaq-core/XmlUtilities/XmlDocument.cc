...
#include "otsdaq-core/XmlUtilities/XmlDocument.h"
...
xercesc::DOMElement* XmlDocument::addTextElementToParent(std::string childName, std::string childText,
  ...
  {
    __COUT_ERR__ << "Error caught attempting to create a text node for this text: " << childText
...
//	returns pointer to element that is added
xercesc::DOMElement* XmlDocument::addTextElementToParent(std::string childName, std::string childText,
                                                         std::string parentName, unsigned int parentIndex) {
  ...
  if (parentIndex >= nodeList->getLength()) {
    __COUT__ << "WARNING: Illegal parent index attempted in tags with name: " << parentName
...
// 	On Success, The child index of the added element with respect to the parent is returned and can be used to add
//		children to the new element
//	On Failure, return -1
/*
unsigned int XmlDocument::addElementToParent(std::string field, std::string value, xercesc::DOMElement *parentEl, bool
verbose)
{
        ...
        {
                //display parent info
                //__COUT__ << "Parent Name: "  << XML_TO_CHAR(parentEl->getNodeName()) << " Field: " << field << "
Value: " << value << std::endl; if( parentEl->getFirstChild() != NULL && parentEl->getFirstChild()->getNodeType() ==
DOMNode::TEXT_NODE)
...
XML_TO_CHAR(parentEl->getFirstChild()->getNodeValue()) << std::endl;
    ...
    if( parentEl->getFirstChild() != NULL && parentEl->getFirstChild()->getNodeType() == DOMNode::TEXT_NODE)
        return nodeList->getLength() - 2; //return child index among parent's children, not counting first child text
node return nodeList->getLength() - 1; //return child index among parent's children
}
*/
//==============================================================================
// XmlDocument::addDataElement
//	Add field/value element to XML doc at parent which is returned from getElementsByTagName(parentName), entry
//number parentNameIndex
// 	On Success, The child index of the added element with respect to the parent is returned and can be used to add
//		children to the new element
//	On Failure, return -1
/*
unsigned int XmlDocument::addDataElement ( std::string field, std::string value, std::string parentName, unsigned int
parentNameIndex)
{
        DOMNodeList *nodeList = theDocument_->getElementsByTagName(CONVERT_TO_XML(parentName));

        if(parentNameIndex >= nodeList->getLength()) {
                __COUT__ << "illegal parent index attempted in tags with name: " << parentName << ", index: " <<
parentNameIndex << std::endl; return -1; //illegal index attempted
        }
...
//==============================================================================
// XmlDocument::addDataElement
//	Add field/value element to XML doc at parentIndexArray (with depth of parent indicated by parentIndexArraySize)
//	If parentIndexArray = NULL, element is added with <DATA> parent
//		otherwise, parentIndexArray indicates the parent within the node list for <DATA> where
//		the element will be added
// 	On Success, The child index of the added element with respect to the parent is returned and can be used to add
//		children to the new element
//	On Failure, return -1
/*
unsigned int XmlDocument::addDataElement ( std::string field, std::string value, unsigned int *parentIndexArray,
unsigned int parentIndexArraySize)
{
    ...
    //__COUT__ << "field: " << field << ", value: " << value << ", parent: " << parentIndexArraySize << std::endl;
        ...
        if(parentIndexArray) //if there passed an array find parent relative to data element
        {
                ...
                for(unsigned int i=0;i<parentIndexArraySize;++i)
                {
                        ...
                        {
                                if(((DOMElement*)(nodeList->item(tmpi)))->getNodeType() == DOMNode::TEXT_NODE) continue;
//skip text nodes
...
DOMNode::TEXT_NODE) ++tmpi;

                        if(tmpi >= nodeList->getLength()) {
                                __COUT__ << "illegal child index attempted in nested parents: " << parentIndexArray[i]
<< ", depth: " << i << ", tmpi: " << tmpi << std::endl; return -1; //illegal child index attempted in nested parents
                        }
...
unsigned int XmlDocument::addXmlData (XmlDocument *xmldoc)
{
        ...
        int retIndex = dataElement->getChildNodes()->getLength(); //will be index of first appended data element

        //add all first level child elements of data and recurse on them
        DOMNodeList *nodeList = xmldoc->dataElement->getChildNodes();	 //get all children	within data
        for(unsigned int i = 0; i<nodeList->getLength();++i)
        {
                if(nodeList->item(i)->getNodeType() == DOMNode::TEXT_NODE) //ignore text node children
                        continue;
...
std::string field, value = "";
        ...
        if( currEl->getFirstChild() != NULL && currEl->getFirstChild()->getNodeType() == DOMNode::TEXT_NODE) //if has a
text node first, insert as value attribute value = escapeString(XML_TO_CHAR(currEl->getFirstChild()->getNodeValue()));
...
void XmlDocument::recursiveOutputXmlDocument(xercesc::DOMElement* currEl, std::ostringstream* out, bool dispStdOut,
                                             std::string tabStr) {
  // open field tag
...
/*
std::string XmlDocument::getDataElement (const std::string field, const unsigned int occurance)
{
...
/*
std::string XmlDocument::recursiveFindElement (DOMElement *currEl, const std::string field, const unsigned int
occurance, unsigned int &count)
{
    if (XML_TO_CHAR(currEl->getNodeName()) == field && occurance == count++) //found, done!!
    {
        if( currEl->getFirstChild() != NULL && currEl->getFirstChild()->getNodeType() == DOMNode::TEXT_NODE) //if has a
text node first, return as value attribute return escapeString(XML_TO_CHAR(currEl->getFirstChild()->getNodeValue()));
        else
    ...
    //look through children recursively
        ...
        for(unsigned int i = 0; i<nodeList->getLength();++i)
                if(nodeList->item(i)->getNodeType() != DOMNode::TEXT_NODE) //ignore text node children
        {
            retStr = recursiveFindElement ((DOMElement*)(nodeList->item(i)),field,occurance,count);
...
//  recursively searches and returns the value for field found occurance number of times
/*
void XmlDocument::recursiveFindAllElements (DOMElement *currEl, const std::string field,std::vector<std::string>
*retVec)
{
    if (XML_TO_CHAR(currEl->getNodeName()) == field &&
            currEl->getFirstChild() != NULL && currEl->getFirstChild()->getNodeType() == DOMNode::TEXT_NODE) //if has a
text node first, return as value attribute retVec->push_back(XML_TO_CHAR(currEl->getFirstChild()->getNodeValue()));
    ...
    //look through children recursively
        ...
        for(unsigned int i = 0; i<nodeList->getLength();++i)
                if(nodeList->item(i)->getNodeType() != DOMNode::TEXT_NODE) //ignore text node children
                        recursiveFindAllElements ((DOMElement*)(nodeList->item(i)),field,retVec);
}
*/
//==============================================================================
...
//	and (if !allowWhiteSpace) remove white space (so that read from file white space artifact removed)
//
//	convert &amp; = &
  ...
  for (unsigned int i = 0; i < inString.length(); i++)
    if (inString[i] != ' ') {
      ...
      if (inString[i] == '\r' || inString[i] == '\n' ||  // remove new line chars
          ...
          (inString[i] > char(126) &&
           inString[i] <
               char(161)))  // this is aggravated by the bug in MFextensions (though Eric says he fixed on 8/24/2016)
...
void XmlDocument::saveXmlDocument(std::string filePath) {
  ...
  // Return the first registered theImplementation_ that has the desired features. In this case, we are after a DOM
  // theImplementation_ that has the LS feature... or Load/Save.  DOMImplementation *theImplementation_ =
...
bool XmlDocument::loadXmlDocument(std::string filePath) {
  ...
  try {
    ...
    theDocument_ =
        parser->adoptDocument();  // instead of getDocument() so parser will not free theDocument_ when released

    // Get the top-level element: Name is "root". No attributes for "root"
    rootElement_ = theDocument_->getDocumentElement();
    if (!rootElement_) throw(std::runtime_error("empty XML theDocument_"));
...
//change text value to escaped version escapeString(XML_TO_CHAR(((DOMElement*)(nodeList->item(i)))->getNodeValue()))));
                else
