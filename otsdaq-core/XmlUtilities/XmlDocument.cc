...
//	On Failure, return -1
/*
unsigned int XmlDocument::addElementToParent(std::string field, std::string value, xercesc::DOMElement *parentEl, bool verbose)
{	
    ...
    if( parentEl->getFirstChild() != NULL && parentEl->getFirstChild()->getNodeType() == DOMNode::TEXT_NODE)
    	return nodeList->getLength() - 2; //return child index among parent's children, not counting first child text node
	return nodeList->getLength() - 1; //return child index among parent's children
}
*/
//==============================================================================
//XmlDocument::addDataElement
//	Add field/value element to XML doc at parent which is returned from getElementsByTagName(parentName), entry number parentNameIndex
// 	On Success, The child index of the added element with respect to the parent is returned and can be used to add
//		children to the new element
//	On Failure, return -1
/*
unsigned int XmlDocument::addDataElement ( std::string field, std::string value, std::string parentName, unsigned int parentNameIndex)
{
...
unsigned int XmlDocument::addDataElement ( std::string field, std::string value, unsigned int *parentIndexArray, unsigned int parentIndexArraySize)
{
	...
	{		
		...
		for(unsigned int i=0;i<parentIndexArraySize;++i)
		{
			...
			{
				if(((DOMElement*)(nodeList->item(tmpi)))->getNodeType() == DOMNode::TEXT_NODE) continue; //skip text nodes
...
std::string field, value = "";
	...
	//XMLString::release( &tmpField );
 	...
 	if( currEl->getFirstChild() != NULL && currEl->getFirstChild()->getNodeType() == DOMNode::TEXT_NODE) //if has a text node first, insert as value attribute
		value = escapeString(XML_TO_CHAR(currEl->getFirstChild()->getNodeValue()));
...
/*
std::string XmlDocument::recursiveFindElement (DOMElement *currEl, const std::string field, const unsigned int occurance, unsigned int &count)
{
    if (XML_TO_CHAR(currEl->getNodeName()) == field && occurance == count++) //found, done!!
    {
        if( currEl->getFirstChild() != NULL && currEl->getFirstChild()->getNodeType() == DOMNode::TEXT_NODE) //if has a text node first, return as value attribute
...
//  recursively searches and returns the value for field found occurance number of times
/*
void XmlDocument::recursiveFindAllElements (DOMElement *currEl, const std::string field,std::vector<std::string> *retVec)
{
    if (XML_TO_CHAR(currEl->getNodeName()) == field &&
            currEl->getFirstChild() != NULL && currEl->getFirstChild()->getNodeType() == DOMNode::TEXT_NODE) //if has a text node first, return as value attribute
...
void XmlDocument::recursiveFixTextFields(DOMElement *currEl)
{
	...
	for(unsigned int i = 0; i<nodeList->getLength();++i)
		if(nodeList->item(i)->getNodeType() == DOMNode::TEXT_NODE) //fix text nodes
			((DOMElement*)(nodeList->item(i)))->setTextContent(CONVERT_TO_XML(			//change text value to escaped version
