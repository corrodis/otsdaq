...
void HttpXmlDocument::recursiveOutputXmlDocument(xercesc::DOMElement *currEl, std::ostringstream *out, bool dispStdOut,
                                                 std::string tabStr, bool allowWhiteSpace) {
  // open field tag
...
std::string HttpXmlDocument::recursiveFindElementValue(xercesc::DOMElement *currEl, const std::string &field,
  ...
  if (XML_TO_CHAR(currEl->getNodeName()) == field && occurance == count++)  // found, done!!
  {
    if (currEl->getFirstChild() != NULL &&
        currEl->getFirstChild()->getNodeType() ==
            xercesc::DOMNode::TEXT_NODE)  // if has a text node first, return as value attribute
...
// HttpXmlDocument::recursiveFindElement
//  recursively searches and returns the value for field found occurance number of times
void HttpXmlDocument::recursiveFindAllElements(xercesc::DOMElement *currEl, const std::string &field,
...
void HttpXmlDocument::recursiveAddElementToParent(xercesc::DOMElement *child, xercesc::DOMElement *parent, bool html) {
          ...
          xercesc::DOMNode::TEXT_NODE)  // if has a text node first, insert as value attribute
  {
    ...
    if (html) childText = escapeString(childText);
  }
...
bool HttpXmlDocument::loadXmlDocument(const std::string &filePath) {
  ...
  try {
    ...
    if (nodeList->getLength())  // may not always be header element
      headerElement_ =
          (xercesc::DOMElement *)(theDocument_->getElementsByTagName(CONVERT_TO_XML(headerTagName_))->item(0));
    else
      headerElement_ = 0;

    dataElement_ = (xercesc::DOMElement *)(theDocument_->getElementsByTagName(CONVERT_TO_XML(dataTagName_))
...
//	On Failure, return -1
/*
xercesc::DOMElement* HttpXmlDocument::addDataElement ( std::string field, std::string value, unsigned int
*parentIndexArray, unsigned int parentIndexArraySize)
{

    //std::cout << __COUT_HDR_FL__ << "field: " << field << ", value: " << value << ", parent: " << parentIndexArraySize
<< std::endl;
    ...
    if(parentIndexArray) //if there passed an array find parent relative to data element
    {
        ...
        for(unsigned int i=0;i<parentIndexArraySize;++i)
        {
            ...
            {
                if(((xercesc::DOMElement*)(nodeList->item(tmpi)))->getNodeType() == xercesc::DOMNode::TEXT_NODE)
...
xercesc::DOMNode::TEXT_NODE) ++tmpi;

            if(tmpi >= nodeList->getLength())
            {
                std::cout << __COUT_HDR_FL__ << "illegal child index attempted in nested parents: " <<
parentIndexArray[i] << ", depth: " << i << ", tmpi: " << tmpi << std::endl; return 0; //illegal child index attempted in
nested parents
            }
