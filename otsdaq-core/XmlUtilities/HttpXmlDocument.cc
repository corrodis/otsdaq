...
HttpXmlDocument::HttpXmlDocument(std::string cookieCode, std::string displayName)
    : XmlDocument("ROOT"), headerElement_(0), dataElement_(0), headerTagName_("HEADER"), dataTagName_("DATA"), cookieCodeTagName_("CookieCode"), displayNameTagName_("DisplayName")
{
...
HttpXmlDocument::HttpXmlDocument(const HttpXmlDocument &doc)
    : XmlDocument(doc), headerElement_(0), dataElement_(0), headerTagName_(doc.headerTagName_), dataTagName_(doc.dataTagName_), cookieCodeTagName_(doc.cookieCodeTagName_), displayNameTagName_(doc.displayNameTagName_)
{
	//std::cout << __COUT_HDR_FL__ << "in" << std::endl;
	*this = doc;
	//std::cout << __COUT_HDR_FL__ << "out" << std::endl;
...
void HttpXmlDocument::setHeader(std::string cookieCode, std::string displayName)
{
	...
	{
		std::stringstream ss;
		ss << __COUT_HDR_FL__ << "Can NOT set header to doc with a header! Only allowed for docs without header element.";
		__SS_THROW__;
	}
...
{
	if (!parent) parent = dataElement_;  //default to data element
                                                 ...
                                                 bool dispStdOut, std::string tabStr, bool allowWhiteSpace)
{
	...
	{
		if (dispStdOut) std::cout << " value='" << escapeString(XML_TO_CHAR(currEl->getFirstChild()->getNodeValue()), allowWhiteSpace) << "'";
		if (out) *out << " value='" << escapeString(XML_TO_CHAR(currEl->getFirstChild()->getNodeValue()), allowWhiteSpace) << "'";
	}
                                                       ...
                                                       const unsigned int occurance, unsigned int &count)
{
	...
	{
		if (currEl->getFirstChild() != NULL && currEl->getFirstChild()->getNodeType() == xercesc::DOMNode::TEXT_NODE)  //if has a text node first, return as value attribute
...
void HttpXmlDocument::recursiveFindAllElements(xercesc::DOMElement *currEl, const std::string &field, std::vector<std::string> *retVec)
{
	if (XML_TO_CHAR(currEl->getNodeName()) == field &&
	    currEl->getFirstChild() != NULL && currEl->getFirstChild()->getNodeType() == xercesc::DOMNode::TEXT_NODE)  //if has a text node first, return as value attribute
                                                           ...
                                                           const unsigned int occurance, unsigned int &count)
{
	...
	for (unsigned int i = 0; i < nodeList->getLength(); ++i)
		if (nodeList->item(i)->getNodeType() != xercesc::DOMNode::TEXT_NODE)  //ignore text node children
		{
			...
			if (retEl) return retEl;  //found among children already, done
				                      //else continue search within children recursively
		}
...
void HttpXmlDocument::recursiveAddElementToParent(xercesc::DOMElement *child, xercesc::DOMElement *parent, bool html)
{
	...
	if (child->getFirstChild() != NULL && child->getFirstChild()->getNodeType() == xercesc::DOMNode::TEXT_NODE)  //if has a text node first, insert as value attribute
	{
		childText = XML_TO_CHAR(child->getFirstChild()->getNodeValue());
		if (html)
			childText = escapeString(childText);
	}
	//std::cout << __COUT_HDR_FL__<< "childName " << childName <<  " childText " << childText << std::endl;
...
std::string	HttpXmlDocument::escapeString(string inString)
{
    ...
    for(unsigned int i=0;i<inString.length();++i)
        if(inString[i] != ' ')
        {
            ...
            {
                inString.insert(i,(inString[i] == '\'')?"&apos":"&quot"); //insert \ before quotes
...
void HttpXmlDocument::recursiveFixTextFields(xercesc::DOMElement *currEl)
{
	...
	for (unsigned int i = 0; i < nodeList->getLength(); ++i)
		if (nodeList->item(i)->getNodeType() == xercesc::DOMNode::TEXT_NODE)              //fix text nodes
			((xercesc::DOMElement *)(nodeList->item(i)))->setTextContent(CONVERT_TO_XML(  //change text value to escaped version
...
//	On Failure, return -1
/*
xercesc::DOMElement* HttpXmlDocument::addDataElement ( std::string field, std::string value, unsigned int *parentIndexArray, unsigned int parentIndexArraySize)
{
 
    //std::cout << __COUT_HDR_FL__ << "field: " << field << ", value: " << value << ", parent: " << parentIndexArraySize << std::endl;
    ...
    if(parentIndexArray) //if there passed an array find parent relative to data element
    {
        ...
        for(unsigned int i=0;i<parentIndexArraySize;++i)
        {
            ...
            {
                if(((xercesc::DOMElement*)(nodeList->item(tmpi)))->getNodeType() == xercesc::DOMNode::TEXT_NODE) continue; //skip text nodes
            ...
            //if(parentEl->getFirstChild() != NULL && parentEl->getFirstChild()->getNodeType() == xercesc::DOMNode::TEXT_NODE) ++tmpi;
 
            if(tmpi >= nodeList->getLength())
            {
                std::cout << __COUT_HDR_FL__ << "illegal child index attempted in nested parents: " << parentIndexArray[i] << ", depth: " << i << ", tmpi: " << tmpi << std::endl;
