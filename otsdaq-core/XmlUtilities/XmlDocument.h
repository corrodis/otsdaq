...
class XmlDocument
{
  public:
	...
	void recursiveRemoveChild (xercesc::DOMElement* childEl, xercesc::DOMElement* parentEl);  //remove entire element and sub tree, recursively
  ...
  protected:
	...
	//	unsigned int 	addElementToParent				(std::string field, std::string value, xercesc::DOMElement *parentEl, bool verbose=false);
	//	void			recursiveAddElementToParent 	(xercesc::DOMElement *currEl, xercesc::DOMElement *parentEl);
	//    std::string     recursiveFindElement            (xercesc::DOMElement *currEl, const std::string field, const unsigned int occurance, unsigned int &count);
