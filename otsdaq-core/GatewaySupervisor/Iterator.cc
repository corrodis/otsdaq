...
void Iterator::IteratorWorkLoop(Iterator* iterator) try {
  ...
  while (1) {
    ...
    if (theIteratorStruct.doPauseAction_) {
      ...
      while (!iterator->checkCommand(&theIteratorStruct)) __COUT__ << "Waiting to pause..." << __E__;
      ...
      while (!iterator->checkCommand(&theIteratorStruct)) __COUT__ << "Waiting to halt..." << __E__;
      ...
      //			//lockout the messages array for the remainder of the scope
      //			//this guarantees the reading thread can safely access the messages
      //			if(iterator->theSupervisor_->VERBOSE_MUTEX) __COUT__ << "Waiting for iterator access" <<
      //__E__; 			std::lock_guard<std::mutex> lock(iterator->accessMutex_);
      //			if(iterator->theSupervisor_->VERBOSE_MUTEX) __COUT__ << "Have iterator access" << __E__;
      //
      //			iterator->activePlanIsRunning_ = false;
        ...
        theIteratorStruct.activePlan_ != "")  // important, because after errors, still "running" until halt
    {
      if (theIteratorStruct.commandIndex_ == (unsigned int)-1) {
        ...
        theIteratorStruct.commands_ =
            itConfig->getPlanCommands(theIteratorStruct.cfgMgr_, theIteratorStruct.activePlan_);
        ...
        for (auto& command : theIteratorStruct.commands_) {
          ...
          __COUT__ << "table " << IterateConfiguration::commandToTableMap_.at(command.type_) << __E__;
      ...
      if (!theIteratorStruct.commandBusy_) {
        ...
        } else if (theIteratorStruct.commandIndex_ == theIteratorStruct.commands_.size())  // Done!
        {
          ...
          try {
            theIteratorStruct.cfgMgr_->activateConfigurationGroup(theIteratorStruct.originalConfigGroup_,
          ...
          //					if(iterator->theSupervisor_->VERBOSE_MUTEX) __COUT__ << "Waiting for iterator access" <<
          //__E__; 					std::lock_guard<std::mutex> lock(iterator->accessMutex_);
          //					if(iterator->theSupervisor_->VERBOSE_MUTEX) __COUT__ << "Have iterator access" <<
          //__E__;
          //
          //					//similar to halt
          //					iterator->activePlanIsRunning_ = false;
...
void Iterator::startCommand(IteratorWorkLoopStruct* iteratorStruct) try {
  ...
  if (type == IterateConfiguration::COMMAND_BEGIN_LABEL) {
    ...
    return startCommandConfigureAlias(iteratorStruct,
                                      iteratorStruct->commands_[iteratorStruct->commandIndex_]
                                          .params_[IterateConfiguration::commandConfigureAliasParams_.SystemAlias_]);
  ...
  __COUT__ << "Activating original group..." << __E__;
  try {
    iteratorStruct->cfgMgr_->activateConfigurationGroup(iteratorStruct->originalConfigGroup_,
...
void Iterator::startCommandChooseFSM(IteratorWorkLoopStruct* iteratorStruct, const std::string& fsmName) {
  ...
  if (!configLinkNode.isDisconnected()) {
    try  // for backwards compatibility
    {
      ...
      __COUT_INFO__ << "No state machine Run alias. Ignoring and assuming alias of '" << iteratorStruct->fsmRunAlias_
                    << ".'" << __E__;
  ...
  iteratorStruct->fsmNextRunNumber_ =
      iteratorStruct->theIterator_->theSupervisor_->getNextRunNumber(iteratorStruct->fsmName_);
...
//(GatewaySupervisor* theSupervisor, const std::string& fsmName)
{
  ...
  if (currentState == "Initialized" || currentState == "Halted") {
    ...
    errorStr = theSupervisor->attemptStateMachineTransition(0, 0, "Abort", fsmName,
                                                            WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
                                                            WebUsers::DEFAULT_ITERATOR_USERNAME, fsmCommandParameters);
  else
    errorStr = theSupervisor->attemptStateMachineTransition(0, 0, "Halt", fsmName,
                                                            WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
                                                            WebUsers::DEFAULT_ITERATOR_USERNAME, fsmCommandParameters);
  ...
  if (iteratorStruct) {
    __COUT__ << "Halted plan '" << iteratorStruct->activePlan_ << "' at command index " << iteratorStruct->commandIndex_
             << ". " << __E__;
    __MOUT__ << "Halted plan '" << iteratorStruct->activePlan_ << "' at command index " << iteratorStruct->commandIndex_
             << ". " << __E__;
...
void Iterator::startCommandRepeatLabel(IteratorWorkLoopStruct* iteratorStruct) {
  ...
  if (numOfRepetitions <= 0) {
    ...
    iteratorStruct->commands_[iteratorStruct->commandIndex_]
        .params_[IterateConfiguration::commandRepeatLabelParams_.NumberOfRepetitions_] = repStr;  // re-store as string
  ...
  for (i = iteratorStruct->commandIndex_; i > 0; --i)  // assume 0 is always the fallback option
    if (iteratorStruct->commands_[i].type_ == IterateConfiguration::COMMAND_BEGIN_LABEL &&
        iteratorStruct->commands_[iteratorStruct->commandIndex_]
            ...
            iteratorStruct->commands_[i].params_[IterateConfiguration::commandBeginLabelParams_.Label_])
      break;
...
void Iterator::startCommandRun(IteratorWorkLoopStruct* iteratorStruct) {
  ...
  if (currentState == "Configured")
    errorStr = iteratorStruct->theIterator_->theSupervisor_->attemptStateMachineTransition(
        0, 0, "Start", iteratorStruct->fsmName_, WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
        WebUsers::DEFAULT_ITERATOR_USERNAME, iteratorStruct->fsmCommandParameters_);
  else
  ...
  // else successfully launched
  __COUT__ << "FSM in transition = " << iteratorStruct->theIterator_->theSupervisor_->theStateMachine_.isInTransition()
           << __E__;
...
void Iterator::startCommandConfigureGroup(IteratorWorkLoopStruct* iteratorStruct) {
  ...
  std::string group = iteratorStruct->commands_[iteratorStruct->commandIndex_]
                          .params_[IterateConfiguration::commandConfigureGroupParams_.GroupName_];
  ConfigurationGroupKey key =
      ConfigurationGroupKey(iteratorStruct->commands_[iteratorStruct->commandIndex_]
...
void Iterator::startCommandConfigureAlias(IteratorWorkLoopStruct* iteratorStruct, const std::string& systemAlias) {
  ...
  if (currentState == "Initial")
    errorStr = iteratorStruct->theIterator_->theSupervisor_->attemptStateMachineTransition(
        0, 0, "Initialize", iteratorStruct->fsmName_, WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
        WebUsers::DEFAULT_ITERATOR_USERNAME, iteratorStruct->fsmCommandParameters_);
  else if (currentState == "Halted")
    errorStr = iteratorStruct->theIterator_->theSupervisor_->attemptStateMachineTransition(
        0, 0, "Configure", iteratorStruct->fsmName_, WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
        WebUsers::DEFAULT_ITERATOR_USERNAME, iteratorStruct->fsmCommandParameters_);
  else if (currentState == "Configured")
    errorStr = iteratorStruct->theIterator_->theSupervisor_->attemptStateMachineTransition(
        0, 0, "Halt", iteratorStruct->fsmName_, WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
        WebUsers::DEFAULT_ITERATOR_USERNAME, iteratorStruct->fsmCommandParameters_);
  else
...
void Iterator::startCommandMacro(IteratorWorkLoopStruct* iteratorStruct, bool isFrontEndMacro) {
  ...
  const std::string& macroName = iteratorStruct->commands_[iteratorStruct->commandIndex_]
                                     .params_[IterateConfiguration::commandExecuteMacroParams_.MacroName_];
  const std::string& enableSavingOutput =
      iteratorStruct->commands_[iteratorStruct->commandIndex_]
          .params_[IterateConfiguration::commandExecuteMacroParams_.EnableSavingOutput_];
  const std::string& outputFilePath = iteratorStruct->commands_[iteratorStruct->commandIndex_]
                                          .params_[IterateConfiguration::commandExecuteMacroParams_.OutputFilePath_];
  const std::string& outputFileRadix = iteratorStruct->commands_[iteratorStruct->commandIndex_]
                                           .params_[IterateConfiguration::commandExecuteMacroParams_.OutputFileRadix_];
  const std::string& inputArgs = iteratorStruct->commands_[iteratorStruct->commandIndex_]
                                     .params_[IterateConfiguration::commandExecuteMacroParams_.MacroArgumentString_];
  ...
  for (const auto& target : iteratorStruct->commands_[iteratorStruct->commandIndex_].targets_) {
    ...
    xoap::MessageReference replyMessage =
        iteratorStruct->theIterator_->theSupervisor_->SOAPMessenger::sendWithSOAPReply(
    ...
    if (response != type + "Done" || error != "") {
      // error occurred!
      __SS__ << "Error transmitting request to target interface '" << target.UID_ << "' from '"
             << WebUsers::DEFAULT_ITERATOR_USERNAME << ".' Response '" << response << "' with error: " << error
             << __E__;
      __SS_THROW__;
    }
  }  // end target loop
...
void Iterator::startCommandModifyActive(IteratorWorkLoopStruct* iteratorStruct) {
  ...
  if ("True" == iteratorStruct->commands_[iteratorStruct->commandIndex_]
                    .params_[IterateConfiguration::commandModifyActiveParams_.DoTrackGroupChanges_])
    doTrackGroupChanges = true;

  const std::string& startValueStr = iteratorStruct->commands_[iteratorStruct->commandIndex_]
                                         .params_[IterateConfiguration::commandModifyActiveParams_.FieldStartValue_];
  const std::string& stepSizeStr =
      iteratorStruct->commands_[iteratorStruct->commandIndex_]
          .params_[IterateConfiguration::commandModifyActiveParams_.FieldIterationStepSize_];
  ...
  if (startValueStr.size() &&
      (startValueStr[startValueStr.size() - 1] == 'f' || startValueStr.find('.') != std::string::npos)) {
    // handle as double
...
bool Iterator::checkCommandRun(IteratorWorkLoopStruct* iteratorStruct) {
  ...
  // all RunControlStateMachine access commands should be mutually exclusive with GatewaySupervisor main thread state
  // machine accesses  should be mutually exclusive with GatewaySupervisor main thread state machine accesses  lockout the
  // messages array for the remainder of the scope  this guarantees the reading thread can safely access the messages
  if (iteratorStruct->theIterator_->theSupervisor_->VERBOSE_MUTEX) __COUT__ << "Waiting for FSM access" << __E__;
  std::lock_guard<std::mutex> lock(iteratorStruct->theIterator_->theSupervisor_->stateMachineAccessMutex_);
  if (iteratorStruct->theIterator_->theSupervisor_->VERBOSE_MUTEX) __COUT__ << "Have FSM access" << __E__;

  if (iteratorStruct->theIterator_->theSupervisor_->theStateMachine_.isInTransition()) return false;
  ...
  if (iteratorStruct->doPauseAction_) {
    ...
    if (currentState == "Paused") {
      ...
      errorStr = iteratorStruct->theIterator_->theSupervisor_->attemptStateMachineTransition(
          0, 0, "Pause", iteratorStruct->fsmName_, WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
          WebUsers::DEFAULT_ITERATOR_USERNAME, iteratorStruct->fsmCommandParameters_);
    else if (currentState == "Configured") {
    ...
    if (currentState == "Halted") {
      ...
      return true;
               ...
               currentState == "Paused")
      errorStr = iteratorStruct->theIterator_->theSupervisor_->attemptStateMachineTransition(
          0, 0, "Abort", iteratorStruct->fsmName_, WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
          WebUsers::DEFAULT_ITERATOR_USERNAME, iteratorStruct->fsmCommandParameters_);
    ...
    if (currentState == "Paused")  // launch transition to running
      errorStr = iteratorStruct->theIterator_->theSupervisor_->attemptStateMachineTransition(
          0, 0, "Resume", iteratorStruct->fsmName_, WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
          WebUsers::DEFAULT_ITERATOR_USERNAME, iteratorStruct->fsmCommandParameters_);
  ...
  } else  // else in Running state! Check for end of run
  {
    ...
    if (waitOnRunningThreads) {
      ...
      if (allFrontEndsAreDone) {
        ...
        errorStr = iteratorStruct->theIterator_->theSupervisor_->attemptStateMachineTransition(
            0, 0, "Stop", iteratorStruct->fsmName_, WebUsers::DEFAULT_ITERATOR_USERNAME /*fsmWindowName*/,
            WebUsers::DEFAULT_ITERATOR_USERNAME, iteratorStruct->fsmCommandParameters_);
    ...
    if (remainingDurationInSeconds > 1) {
      ...
      iteratorStruct->commands_[iteratorStruct->commandIndex_]
          .params_[IterateConfiguration::commandRunParams_.DurationInSeconds_] = str;  // re-store as string
...
void Iterator::pauseIterationPlan(HttpXmlDocument& xmldoc) {
  ...
  if (workloopRunning_ && activePlanIsRunning_ && !commandPause_) {
    ...
    __COUT__ << "Invalid pause command attempted. " << workloopRunning_ << " " << activePlanIsRunning_ << " "
             << commandPause_ << " " << activePlanName_ << __E__;
  }
}
...
void Iterator::haltIterationPlan(HttpXmlDocument& xmldoc) {
  ...
  } else  // no thread, so halt (and reset Error') without command to thread
  {
    ...
    //		{
    //			__SS__ << "Invalid halt command attempted. Can only halt when there is an active iteration plan." <<
    //__E__;
    //			__MOUT_ERR__ << ss.str();
    //
    //			xmldoc.addTextElementToData("error_message", ss.str());
...
void Iterator::getIterationPlanStatus(HttpXmlDocument& xmldoc) {
  xmldoc.addTextElementToData("current_state", theSupervisor_->theStateMachine_.isInTransition()
