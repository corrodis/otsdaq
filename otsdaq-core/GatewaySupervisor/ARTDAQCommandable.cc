...
ots::ARTDAQCommandable::ARTDAQCommandable(GatewaySupervisor* super)
    : artdaq::Commandable(), theSupervisor_(super), theCommander_(nullptr) {}
...
bool ots::ARTDAQCommandable::do_initialize(fhicl::ParameterSet const& ps, uint64_t, uint64_t) {
  ...
  auto ret = theSupervisor_->attemptStateMachineTransition(
      nullptr, nullptr, "Initialize", theSupervisor_->activeStateMachineName_,
      theSupervisor_->activeStateMachineWindowName_, "ARTDAQCommandable", parameters);
  if (ret == "")
    ret = theSupervisor_->attemptStateMachineTransition(
        nullptr, nullptr, "Configure", theSupervisor_->activeStateMachineName_,
        theSupervisor_->activeStateMachineWindowName_, "ARTDAQCommandable", parameters);
  report_string_ = ret;
  return ret == "";
}
...
bool ots::ARTDAQCommandable::do_start(art::RunID, uint64_t, uint64_t) {
  std::vector<std::string> parameters;
  auto ret = theSupervisor_->attemptStateMachineTransition(
      nullptr, nullptr, "Start", theSupervisor_->activeStateMachineName_, theSupervisor_->activeStateMachineWindowName_,
...
bool ots::ARTDAQCommandable::do_stop(uint64_t, uint64_t) {
  std::vector<std::string> parameters;
  auto ret = theSupervisor_->attemptStateMachineTransition(
      nullptr, nullptr, "Stop", theSupervisor_->activeStateMachineName_, theSupervisor_->activeStateMachineWindowName_,
      "ARTDAQCommandable", parameters);
  report_string_ = ret;
  return ret == "";
}
...
bool ots::ARTDAQCommandable::do_pause(uint64_t, uint64_t) {
  std::vector<std::string> parameters;
  auto ret = theSupervisor_->attemptStateMachineTransition(
      nullptr, nullptr, "Pause", theSupervisor_->activeStateMachineName_, theSupervisor_->activeStateMachineWindowName_,
      "ARTDAQCommandable", parameters);
  report_string_ = ret;
  return ret == "";
}
...
bool ots::ARTDAQCommandable::do_resume(uint64_t, uint64_t) {
  std::vector<std::string> parameters;
  auto ret = theSupervisor_->attemptStateMachineTransition(
      nullptr, nullptr, "Resume", theSupervisor_->activeStateMachineName_,
      theSupervisor_->activeStateMachineWindowName_, "ARTDAQCommandable", parameters);
  report_string_ = ret;
  return ret == "";
}
...
bool ots::ARTDAQCommandable::do_shutdown(uint64_t) {
  std::vector<std::string> parameters;
  auto ret = theSupervisor_->attemptStateMachineTransition(
      nullptr, nullptr, "Halt", theSupervisor_->activeStateMachineName_, theSupervisor_->activeStateMachineWindowName_,
      "ARTDAQCommandable", parameters);
  if (ret == "")
    ret = theSupervisor_->attemptStateMachineTransition(
        nullptr, nullptr, "Shutdown", theSupervisor_->activeStateMachineName_,
        theSupervisor_->activeStateMachineWindowName_, "ARTDAQCommandable", parameters);
  report_string_ = ret;
  return ret == "";
}
...
bool ots::ARTDAQCommandable::do_meta_command(std::string const& command, std::string const& args) {
  ...
  auto ret = theSupervisor_->attemptStateMachineTransition(
      nullptr, nullptr, command, theSupervisor_->activeStateMachineName_, theSupervisor_->activeStateMachineWindowName_,
      "ARTDAQCommandable", parameters);
  report_string_ = ret;
  return ret == "";
}
