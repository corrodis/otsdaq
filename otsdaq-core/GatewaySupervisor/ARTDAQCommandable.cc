...
ots::ARTDAQCommandable::~ARTDAQCommandable()
{
	if(theCommander_ != nullptr)
	{
		...
		if(commanderThread_.joinable())
			commanderThread_.join();
	}
}
...
void ots::ARTDAQCommandable::init(int commanderId, std::string commanderType)
{
	...
	commanderThread_ = boost::thread(
	    attrs, boost::bind(&artdaq::CommanderInterface::run_server, theCommander_.get()));
}
                                           ...
                                           uint64_t)
{
	...
	auto ret = theSupervisor_->attemptStateMachineTransition(
	    ...
	    nullptr,
	    "Initialize",
	    theSupervisor_->activeStateMachineName_,
	    theSupervisor_->activeStateMachineWindowName_,
	    "ARTDAQCommandable",
	    parameters);
	if(ret == "")
...
bool ots::ARTDAQCommandable::do_meta_command(std::string const& command,
