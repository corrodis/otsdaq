...
#define CORE_TABLE_INFO_FILENAME                                                               \
  ((getenv("SERVICE_DATA_PATH") == NULL) ? (std::string(getenv("USER_DATA")) + "/ServiceData") \
...
GatewaySupervisor::GatewaySupervisor(xdaq::ApplicationStub* s)
  ...
  // xoap::bind(this, &GatewaySupervisor::supervisorGetUserInfo,        		"SupervisorGetUserInfo",
  // XDAQ_NS_URI);
  xoap::bind(this, &GatewaySupervisor::supervisorSystemLogbookEntry, "SupervisorSystemLogbookEntry", XDAQ_NS_URI);
  xoap::bind(this, &GatewaySupervisor::supervisorLastConfigGroupRequest, "SupervisorLastConfigGroupRequest",
             XDAQ_NS_URI);
...
//========================================================================================================================
//	TODO: Lore needs to detect program quit through killall or ctrl+c so that Logbook entry is made when ots is
//halted
...
void GatewaySupervisor::init(void) {
  ...
  try {
    auto artdaqStateChangeEnabled = CorePropertySupervisorBase::getSupervisorConfigurationNode()
                                        .getNode("EnableARTDAQCommanderPlugin")
                                        .getValue<bool>();
    if (artdaqStateChangeEnabled) {
      auto artdaqStateChangePort =
          CorePropertySupervisorBase::getSupervisorConfigurationNode().getNode("ARTDAQCommanderID").getValue<int>();
      auto artdaqStateChangePluginType = CorePropertySupervisorBase::getSupervisorConfigurationNode()
  ...
  {
    const std::set<std::string>& contextMemberNames =
        CorePropertySupervisorBase::theConfigurationManager_->getContextMemberNames();
    const std::set<std::string>& backboneMemberNames =
        CorePropertySupervisorBase::theConfigurationManager_->getBackboneMemberNames();
    const std::set<std::string>& iterateMemberNames =
        CorePropertySupervisorBase::theConfigurationManager_->getIterateMemberNames();
    ...
    if (fp)  // check for all core table names in file, and force their presence
    {
      ...
      if (fp) {
        for (const auto& name : contextMemberNames) fprintf(fp, "%s\n", name.c_str());
        for (const auto& name : backboneMemberNames) fprintf(fp, "%s\n", name.c_str());
        for (const auto& name : iterateMemberNames) fprintf(fp, "%s\n", name.c_str());
        fclose(fp);
      } else {
...
void GatewaySupervisor::StateChangerWorkLoop(GatewaySupervisor* theSupervisor) {
  ...
  try {
    ...
    __SS__ << "FATAL Console error. Could not initialize socket at ip '" << ipAddressForStateChangesOverUDP
           ...
           << ". Perhaps it is already in use? Exiting State Changer SOAPUtilities::receive loop." << __E__;
    __COUT__ << ss.str();
    __SS_THROW__;
    return;
  }
    ...
    if (sock.receive(buffer, 0 /*timeoutSeconds*/, 1 /*timeoutUSeconds*/, false /*verbose*/) != -1) {
      ...
      if (nCommas == 0) {
        __SS__ << "Unrecognized State Machine command :-" << buffer
               << "-. Format is FiniteStateMachineName,Command,Parameter(s). Where Parameter(s) is/are optional."
               << __E__;
...
void GatewaySupervisor::makeSystemLogbookEntry(std::string entryText) {
  ...
  for (auto& logbookInfo : logbookInfoMap) {
    xoap::MessageReference retMsg =
        SOAPMessenger::sendWithSOAPReply(logbookInfo.second.getDescriptor(), "MakeSystemLogbookEntry", parameters);
    ...
    __COUT__ << "Returned Status: " << retParameters.getValue("Status")
...

{
  ...
  if (!supervisorGuiHasBeenLoaded_ &&
      (supervisorGuiHasBeenLoaded_ = true))  // make system logbook entry that ots has been started
...
void GatewaySupervisor::stateMachineXgiHandler(xgi::Input* in, xgi::Output* out) {
  // for simplicity assume all commands should be mutually exclusive with iterator thread state machine accesses (really
  // should just be careful with RunControlStateMachine access)
  if (VERBOSE_MUTEX) __COUT__ << "Waiting for FSM access" << __E__;
  std::lock_guard<std::mutex> lock(stateMachineAccessMutex_);
  if (VERBOSE_MUTEX) __COUT__ << "Have FSM access" << __E__;
  ...
  if (!theWebUsers_.xmlRequestOnGateway(cgiIn, out, &xmlOut, userInfo)) return;  // access failed
  ...
  std::vector<std::string> parameters;
  if (command == "Configure") parameters.push_back(CgiDataUtilities::postData(cgiIn, "ConfigurationAlias"));
  attemptStateMachineTransition(&xmlOut, out, command, fsmName, fsmWindowName, userInfo.username_, parameters);
}

//========================================================================================================================
std::string GatewaySupervisor::attemptStateMachineTransition(HttpXmlDocument* xmldoc, std::ostringstream* out,
                                                             const std::string& command, const std::string& fsmName,
  ...
  if (command == "Configure") {
    ...
    std::string fn =
        FSM_LAST_GROUP_ALIAS_PATH + FSM_LAST_GROUP_ALIAS_FILE_START + username + "." + FSM_USERS_PREFERENCES_FILETYPE;
...
//	SOAPParameters parameters;
//	/*
//	 //FIXME I don't think that there should be this if statement and I guess it has been copied from the
//stateMachineXgiHandler method 	 if (command == "Configure")
//
//{
//	 parameters.addParameter("RUN_KEY", CgiDataUtilities::postData(cgi, "ConfigurationAlias"));
//	 __COUT__ << "Configure --> Name: RUN_KEY Value: " << parameters.getValue("RUN_KEY") << __E__;
//	 }
//	 else if (command == "Start")
//
//{
...
//========================================================================================================================
xoap::MessageReference GatewaySupervisor::stateMachineResultXoapHandler(xoap::MessageReference message)
...
bool GatewaySupervisor::stateMachineThread(toolbox::task::WorkLoop* workLoop) {
  ...
  std::string reply =
      send(allSupervisorInfo_.getGatewayDescriptor(), stateMachineWorkLoopManager_.getMessage(workLoop));
  ...
  if (reply == "Fault") {
    __SS__ << "Failure to send Workloop transition command '" << command << "!' An error response '" << reply
           << "' was received." << __E__;
    __COUT_ERR__ << ss.str();
    __MOUT_ERR__ << ss.str();
  }
  return false;  // execute once and automatically remove the workloop so in WorkLoopManager the try
                 // workLoop->remove(job_) could be commented out
...

{
  ...
  // The way the workloop manager now works is safe since it cancel the thread only when the infoRequestResultHandler is
  // called  and that method can be called ONLY when I am already out of here!
...

{
  ...
  //	//If TRUE, cookie code is good, and refreshed code is in cookieCode, also pointers optionally for uint8_t
  //userPermissions, uint64_t uid
...

{
  ...
  // diagService_->reportError("GatewaySupervisor::stateInitial: workloop active:
  // "+stringF(calibWorkloop_->isActive())+", workloop type: "+calibWorkloop_->getType(),DIAGINFO);
}
   ...
   catch(xcept::Exception &e)
{
   ...
   diagService_->reportError("GatewaySupervisor::statePaused: workloop active: "+stringF(calibWorkloop_->isActive())+",
workloop type: "+calibWorkloop_->getType(),DIAGINFO);
   */
}
   ...
   catch(xcept::Exception &e)
{
   ...
   diagService_->reportError("GatewaySupervisor::stateRunning: workloop active: "+stringF(calibWorkloop_->isActive())+",
workloop type: "+calibWorkloop_->getType(),DIAGINFO);
   */
}
...

{
  ...
  /*
   ...
   diagService_->reportError("GatewaySupervisor::stateHalted: workloop active: "+stringF(calibWorkloop_->isActive())+",
workloop type: "+calibWorkloop_->getType(),DIAGINFO);
...
"+stringF(calibWorkloop_->isActive())+", workloop type: "+calibWorkloop_->getType(),DIAGDEBUG);
   */
}
...
void GatewaySupervisor::enteringError(toolbox::Event::Reference e) {
  ...
  if (RunControlStateMachine::asyncFailureReceived_) {
    ...
    ss << "\nFailure performing transition from " << failedEvent.getFromState() << "-"
       << theStateMachine_.getStateName(failedEvent.getFromState()) << " to " << failedEvent.getToState() << "-"
...
void GatewaySupervisor::transitionConfiguring(toolbox::Event::Reference e) {
  ...
  std::string systemAlias =
      SOAPUtilities::translate(theStateMachine_.getCurrentMessage()).getParameters().getValue("ConfigurationAlias");
  ...
  try {
    theConfigurationGroup_ =
        CorePropertySupervisorBase::theConfigurationManager_->getConfigurationGroupFromAlias(systemAlias);
  } catch (...) {
  ...
  __COUT__ << "Configuration group name: " << theConfigurationGroup_.first << " key: " << theConfigurationGroup_.second
           << __E__;

  // make logbook entry
  {
    std::stringstream ss;
    ss << "Configuring '" << systemAlias << "' which translates to " << theConfigurationGroup_.first << " ("
  ...
  // load and activate
  try {
    CorePropertySupervisorBase::theConfigurationManager_->loadConfigurationGroup(theConfigurationGroup_.first,
                                                                                 theConfigurationGroup_.second, true);
    ...
    __SS__ << "\nTransition to Configuring interrupted! System Alias " << systemAlias << " was translated to "
  ...
  // check if configuration dump is enabled on configure transition
  {
    ConfigurationTree configLinkNode =
        CorePropertySupervisorBase::theConfigurationManager_->getSupervisorConfigurationNode(supervisorContextUID_,
      ...
      try  // errors in dump are not tolerated
      {
        ...
        try  // for backwards compatibility
        {
          ConfigurationTree fsmLinkNode =
              configLinkNode.getNode("LinkToStateMachineTable").getNode(activeStateMachineName_);
  ...
  // update Macro Maker front end list
  {
    ...
    std::string reply =
        SOAPMessenger::send(CorePropertySupervisorBase::allSupervisorInfo_.getAllMacroMakerTypeSupervisorInfo()
                                .begin()
...

{
  ...
  // important to give time for StartOTS script to recognize command and for apps to instantiate things (before user
  // does Initialize)
  for (int i = 0; i < 10; ++i) {
    sleep(1);
    RunControlStateMachine::theProgressBar_.step();
    ...
    if (!configLinkNode.isDisconnected()) {
      try  // errors in dump are not tolerated
      {
        ...
        if (dumpConfiguration) {
          // dump configuration
          CorePropertySupervisorBase::theConfigurationManager_->dumpActiveConfiguration(
              dumpFilePath + "/" + dumpFileRadix + "_Run" + runNumber + "_" + std::to_string(time(0)) + ".dump",
              dumpFormat);
        }
...
bool GatewaySupervisor::handleBroadcastMessageTarget(const SupervisorInfo& appInfo, xoap::MessageReference message,
                                                     const std::string& command, const unsigned int& iteration,
                                                     std::string& reply, unsigned int threadIndex) {
  ...
  while (!subIterationsDone)  // start subIteration handling loop
  {
    ...
    if (iteration == 0 && subIteration == 0) {
      for (unsigned int j = 0; j < 4; ++j)
        __COUT__ << "Broadcast thread " << threadIndex << "\t"
                 << "Sending message to Supervisor " << appInfo.getName() << " [LID=" << appInfo.getId()
                 << "]: " << command << __E__;
    } else  // else this not the first time through the supervisors
    {
      if (subIteration == 0) {
        for (unsigned int j = 0; j < 4; ++j)
          __COUT__ << "Broadcast thread " << threadIndex << "\t"
                   << "Sending message to Supervisor " << appInfo.getName() << " [LID=" << appInfo.getId()
                   << "]: " << command << " (iteration: " << iteration << ")" << __E__;
      } else {
        for (unsigned int j = 0; j < 4; ++j)
          __COUT__ << "Broadcast thread " << threadIndex << "\t"
                   << "Sending message to Supervisor " << appInfo.getName() << " [LID=" << appInfo.getId()
                   << "]: " << command << " (iteration: " << iteration << ", sub-iteration: " << subIteration << ")"
                   << __E__;
      }
    }
    ...
    } catch (const xdaq::exception::Exception& e)  // due to xoap send failure
    {
      ...
      __SS__ << "Error! Gateway Supervisor can NOT " << command << " Supervisor instance = '" << appInfo.getName()
             << "' [LID=" << appInfo.getId() << "] in Context '" << appInfo.getContextName()
             << "' [URL=" << appInfo.getURL() << "].\n\n"
             << "Xoap message failure. Did the target Supervisor crash? Try re-initializing or restarting otsdaq."
             << __E__;
      __COUT_ERR__ << ss.str();
      __MOUT_ERR__ << ss.str();
    ...
    if ((reply != command + "Done") && (reply != command + "Response") && (reply != command + "Iterate") &&
      ...
      try {
        xoap::MessageReference errorMessage = sendWithSOAPReply(
            appInfo.getDescriptor(), SOAPUtilities::makeSOAPMessageReference("StateMachineErrorMessageRequest"));
        SOAPParameters parameters;
        ...
        if (error == "") {
          ...
          err << "Unknown error from Supervisor instance = '" << appInfo.getName() << "' [LID=" << appInfo.getId()
              << "] in Context '" << appInfo.getContextName() << "' [URL=" << appInfo.getURL()
              << "]. If the problem persists or is repeatable, please notify admins.\n\n";
          error = err.str();
        }

        __SS__ << "Received error from Supervisor instance = '" << appInfo.getName() << "' [LID=" << appInfo.getId()
               << "] in Context '" << appInfo.getContextName() << "' [URL=" << appInfo.getURL()
        ...
        if (command == "Error") return true;  // do not throw exception and exit loop if informing all apps about error
      ...
      } catch (const xdaq::exception::Exception& e)  // due to xoap send failure
      {
        ...
        __SS__ << "Error! Gateway Supervisor failed to read error message from Supervisor instance = '"
               << appInfo.getName() << "' [LID=" << appInfo.getId() << "] in Context '" << appInfo.getContextName()
               << "' [URL=" << appInfo.getURL() << "].\n\n"
               << "Xoap message failure. Did the target Supervisor crash? Try re-initializing or restarting otsdaq."
               << __E__;
        __COUT_ERR__ << ss.str();
      ...
      __COUT__ << "Broadcast thread " << threadIndex << "\t"
               << "Supervisor instance = '" << appInfo.getName() << "' [LID=" << appInfo.getId() << "] in Context '"
               << appInfo.getContextName() << "' [URL=" << appInfo.getURL() << "] flagged for another iteration to "
               << command << "... (iteration: " << iteration << ")" << __E__;
      ...
      __COUT__ << "Broadcast thread " << threadIndex << "\t"
               << "Supervisor instance = '" << appInfo.getName() << "' [LID=" << appInfo.getId() << "] in Context '"
               << appInfo.getContextName() << "' [URL=" << appInfo.getURL() << "] flagged for another sub-iteration to "
               << command << "... (iteration: " << iteration << ", sub-iteration: " << subIteration << ")" << __E__;

    } else  // else success response
    {
      __COUT__ << "Broadcast thread " << threadIndex << "\t"
               << "Supervisor instance = '" << appInfo.getName() << "' [LID=" << appInfo.getId() << "] in Context '"
               << appInfo.getContextName() << "' [URL=" << appInfo.getURL() << "] was " << command << "'d correctly!"
               << __E__;
    }
...
void GatewaySupervisor::broadcastMessageThread(GatewaySupervisor* supervisorPtr,
  ...
  while (!threadStruct->exitThread_) {
    ...
    if (threadStruct->workToDo_) {
      ...
      try {
        if (supervisorPtr->handleBroadcastMessageTarget(threadStruct->getAppInfo(), threadStruct->getMessage(),
                                                        threadStruct->getCommand(), threadStruct->getIteration(),
                                                        threadStruct->getReply(), threadStruct->threadIndex_))
...
void GatewaySupervisor::broadcastMessage(xoap::MessageReference message) {
  ...
  try {
    ...
    __SS__ << "Error getting supervisor priority. Was there a change in the context?"
           << " Remember, if the context was changed, it is safest to relaunch StartOTS.sh. " << e.what() << __E__;
    XCEPT_RAISE(toolbox::fsm::exception::Exception, ss.str());
  }

  RunControlStateMachine::theProgressBar_.step();

  // std::vector<std::vector<uint8_t/*bool*/>> supervisorIterationsDone; //Note: can not use bool because std::vector
  // does not allow access by reference of type bool
  ...
  for (unsigned int i = 0; i < numberOfThreads; ++i) {
    ...
    std::thread(
        [](GatewaySupervisor* supervisorPtr, GatewaySupervisor::BroadcastThreadStruct* threadStruct) {
  ...
  try {
    ...
    do  // while !iterationsDone
    {
      ...
      if (iteration) __COUT__ << "Starting iteration: " << iteration << __E__;
            ...
            do {
              for (unsigned int k = 0; k < numberOfThreads; ++k) {
                if (!broadcastThreadStructs[k].workToDo_) {
                  ...
                  broadcastThreadStructs[k].setMessage(appInfo, message, command, iteration,
      ...
      if (iteration || !broadcastIterationsDone_) __COUT__ << "Completed iteration: " << iteration << __E__;
      ++iteration;
    ...
    for (unsigned int i = 0; i < numberOfThreads; ++i) broadcastThreadStructs[i].exitThread_ = true;
...
void GatewaySupervisor::wait(int milliseconds, std::string who) const {
  for (int s = 1; s <= milliseconds; s++) {
    ...
    if (s % 100 == 0) __COUT__ << s << " msecs " << who << __E__;
  }
}
...
void GatewaySupervisor::loginRequest(xgi::Input* in, xgi::Output* out) {
  ...
  if (Command == "sessionId") {
    ...
    //	Whenever client submits user name and password it is jumbled by sessionId when sent to server and sent along
    //with UUID. Server uses sessionId to unjumble.
    //
    ...
    *out << sid;
    ...
    uint64_t uid = theWebUsers_.attemptActiveSession(
        uuid, jumbledUser, jumbledPw, newAccountCode,
    ...
    if (uid != theWebUsers_.NOT_FOUND_IN_DATABASE) {
      uint64_t asCnt =
          theWebUsers_.getActiveSessionCountForUser(uid) - 1;  // subtract 1 to remove just started session from count
      char asStr[20];
    ...
    uint64_t uid = theWebUsers_.attemptActiveSessionWithCert(
        uuid, jumbledEmail, cookieCode, username,
        cgi.getEnvironment().getRemoteAddr());  // after call jumbledUser holds displayName on success
    ...
    if (theWebUsers_.cookieCodeLogout(cookieCode, logoutOthers == "1", &uid, cgi.getEnvironment().getRemoteAddr()) !=
...
void GatewaySupervisor::tooltipRequest(xgi::Input* in, xgi::Output* out) {
  ...
  //**** start LOGIN GATEWAY CODE ***//
  // If TRUE, cookie code is good, and refreshed code is in cookieCode, also pointers optionally for uint8_t
  // userPermissions, uint64_t uid  Else, error message is returned in cookieCode  Notes: cookie code not refreshed if
  // RequestType = getSystemMessages
  std::string cookieCode = CgiDataUtilities::postData(cgi, "CookieCode");
  uint64_t uid;

  if (!theWebUsers_.cookieCodeIsActiveForRequest(cookieCode, 0 /*userPermissions*/, &uid, "0" /*dummy ip*/,
                                                 false /*refresh*/)) {
    *out << cookieCode;
    return;
  }
  ...
  if (Command == "check") {
    WebUsers::tooltipCheckForUsername(
        theWebUsers_.getUsersUsername(uid), &xmldoc, CgiDataUtilities::getData(cgi, "srcFile"),
        CgiDataUtilities::getData(cgi, "srcFunc"), CgiDataUtilities::getData(cgi, "srcId"));
...
void GatewaySupervisor::setSupervisorPropertyDefaults() {
  CorePropertySupervisorBase::setSupervisorProperty(
      ...
      std::string() + "*=1 | gatewayLaunchOTS=-1 | gatewayLaunchWiz=-1 | codeEditor=-1");
}
...
void GatewaySupervisor::request(xgi::Input* in, xgi::Output* out) {
  ...
  try {
    if (requestType == "getSettings") {
      ...
      std::map<std::string /*alias*/, std::pair<std::string /*group name*/, ConfigurationGroupKey>> aliasMap =
          CorePropertySupervisorBase::theConfigurationManager_->getActiveGroupAliases();

      // get stateMachineAliasFilter if possible
      ConfigurationTree configLinkNode =
          CorePropertySupervisorBase::theConfigurationManager_->getSupervisorConfigurationNode(
      ...
      //	* can be used as wild card.
      {
        ...
        for (auto& aliasMapPair : aliasMap) {
          ...
          if (filterArr.size() == 1) {
            if (filterArr[0] != "" && filterArr[0] != "*" && aliasMapPair.first != filterArr[0]) filterMatch = false;
          } else {
            i = -1;
          ...
          xmlOut.addTextElementToData(
              "config_key",
              ConfigurationGroupKey::getFullGroupString(aliasMapPair.second.first, aliasMapPair.second.second).c_str());
          ...
          try {
            CorePropertySupervisorBase::theConfigurationManager_->loadConfigurationGroup(
                aliasMapPair.second.first, aliasMapPair.second.second, false, 0, 0, 0, &groupComment, &groupAuthor,
                &groupCreationTime, false /*false to not load member map*/);
      ...
      for (auto it : allSupervisorInfo_.getAllFETypeSupervisorInfo()) {
        ...
        xmlOut.addTextElementToParent("fec_urn", std::to_string(it.second.getId()), "fec_list");
      }
    } else if (requestType == "getSystemMessages") {
      xmlOut.addTextElementToData(
          "systemMessages", theSystemMessenger_.getSystemMessage(theWebUsers_.getUsersDisplayName(userInfo.uid_)));
      ...
      try {
        ...
        for (const auto& fsmNode : fsmNodes) xmlOut.addTextElementToData("stateMachineName", fsmNode.first);
      ...
      if (theStateMachine_.isInTransition())
        xmlOut.addTextElementToData("transition_progress",
      ...
      if (!theStateMachine_.isInTransition()) {
        ...
        if (!configLinkNode.isDisconnected()) {
          try  // for backwards compatibility
          {
            ...
            __COUT_INFO__ << "No state machine Run alias. Ignoring and assuming alias of '" << stateMachineRunAlias
                          << ".'" << __E__;
          }
        }
        // else
      ...
      std::vector<DesktopIconConfiguration::DesktopIcon> icons = iconConfiguration->getAllDesktopIcons();

      std::string iconString = "";  // comma-separated icon string, 7 fields:
      //				0 - caption 		= text below icon
      //				1 - altText 		= text icon if no image given
      //				2 - uniqueWin 		= if true, only one window is allowed, else
      //multiple instances of window 				3 - permissions 	= security level needed to see
      //icon 				4 - picfn 			= icon image filename
      ...
      for (const auto& icon : icons) {
        ...
        CorePropertySupervisorBase::extractPermissionsMapFromString(icon.permissionThresholdString_,
        ...
        iconString += "," + std::string("1");  // set permission to 1 so the desktop shows every icon that the server
                                               // allows (i.e., trust server security, ignore client security)
      ...
      codeEditor_.xmlRequest(CgiDataUtilities::getData(cgiIn, "option"), cgiIn, &xmlOut);
    } else {
...
void GatewaySupervisor::launchStartOTSCommand(const std::string& command, ConfigurationManager* cfgMgr) {
  ...
  for (const auto& hostname : hostnames) {
    std::string fn = (std::string(getenv("SERVICE_DATA_PATH")) + "/StartOTS_action_" + hostname + ".cmd");
    FILE* fp = fopen(fn.c_str(), "w");
    if (fp) {
...

{
  ...
  std::string refreshOption = parameters.getValue(
      "RefreshOption");  // give external supervisors option to refresh cookie or not, "1" to refresh
  std::string ipAddress =
      parameters.getValue("IPAddress");  // give external supervisors option to refresh cookie or not, "1" to refresh

  // If TRUE, cookie code is good, and refreshed code is in cookieCode, also pointers optionally for uint8_t
  // userPermissions, uint64_t uid  Else, error message is returned in cookieCode
  ...
  theWebUsers_.cookieCodeIsActiveForRequest(cookieCode, &userGroupPermissionsMap,
                                            &uid /*uid is not given to remote users*/, ipAddress, refreshOption == "1",
                                            &userWithLock, &activeSessionIndex);
...
xoap::MessageReference GatewaySupervisor::lastConfigGroupRequestHandler(const SOAPParameters& parameters) {
  ...
  std::pair<std::string /*group name*/, ConfigurationGroupKey> theGroup = loadGroupNameAndKey(fileName, timeString);
...
unsigned int GatewaySupervisor::getNextRunNumber(const std::string& fsmNameIn) {
  ...
  for (unsigned int i = 0; i < fsmName.size(); ++i)
    if ((fsmName[i] >= 'a' && fsmName[i] <= 'z') || (fsmName[i] >= 'A' && fsmName[i] <= 'Z') ||
...
std::pair<std::string /*group name*/, ConfigurationGroupKey> GatewaySupervisor::loadGroupNameAndKey(
  ...
  if (!groupFile) {
    ...
    return std::pair<std::string /*group name*/, ConfigurationGroupKey>("", ConfigurationGroupKey());
  }
...
void GatewaySupervisor::saveGroupNameAndKey(
    const std::pair<std::string /*group name*/, ConfigurationGroupKey>& theGroup, const std::string& fileName) {
  std::string fullPath = FSM_LAST_GROUP_ALIAS_PATH + "/" + fileName;
...
//   {
//        *out << "<FEWList>" << __E__;
//        for (std::set<xdaq::ApplicationDescriptor*>::iterator pxFESupervisorsIt=pxFESupervisors_.begin();
