...
WizardSupervisor::WizardSupervisor(xdaq::ApplicationStub *s) throw(xdaq::exception::Exception)
  ...
  xoap::bind(this, &WizardSupervisor::supervisorLastConfigGroupRequest, "SupervisorLastConfigGroupRequest",
             XDAQ_NS_URI);
  init();
...
void WizardSupervisor::generateURL() {
  ...
  if (fp) {
    ...
    __COUT_INFO__ << "(Reverting to default wiz security) Sequence length file NOT found: " << SEQUENCE_FILE_NAME
                  << std::endl;
  ...
  __COUT__ << getenv("OTS_CONFIGURATION_WIZARD_SUPERVISOR_SERVER") << ":" << getenv("PORT")
...
//========================================================================================================================
void WizardSupervisor::tooltipRequest(xgi::Input *in, xgi::Output *out) throw(xgi::exception::Exception) {
  cgicc::Cgicc cgi(in);
  ...
  if (Command == "check") {
    WebUsers::tooltipCheckForUsername(
        WebUsers::DEFAULT_ADMIN_USERNAME, &xmldoc, CgiDataUtilities::getData(cgi, "srcFile"),
...
//========================================================================================================================
void WizardSupervisor::toggleSecurityCodeGeneration(xgi::Input *in, xgi::Output *out) throw(xgi::exception::Exception) {
  cgicc::Cgicc cgi(in);
  ...
  if (Command == "TurnGenerationOn") {
    ...
    std::thread([&](WizardSupervisor *ptr, std::string securityCode) { printURL(ptr, securityCode); }, this,
                securityCode_)
        .detach();
...
//	verify cookie
xoap::MessageReference WizardSupervisor::supervisorSequenceCheck(xoap::MessageReference message) throw(
  ...
  if (securityCode_ == submittedSequence)
    permissionMap.emplace(std::pair<std::string /*groupName*/, WebUsers::permissionLevel_t>(
...
xoap::MessageReference WizardSupervisor::supervisorLastConfigGroupRequest(xoap::MessageReference message) throw(
    xoap::exception::Exception) {
  SOAPParameters parameters;
...
//========================================================================================================================
void WizardSupervisor::Default(xgi::Input *in, xgi::Output *out) throw(xgi::exception::Exception) {
  __COUT__ << "Unauthorized Request made, security sequence doesn't match!" << std::endl;
  *out << "Unauthorized Request.";
}

//========================================================================================================================
void WizardSupervisor::verification(xgi::Input *in, xgi::Output *out) throw(xgi::exception::Exception) {
  cgicc::Cgicc cgi(in);
...
//========================================================================================================================
void WizardSupervisor::request(xgi::Input *in, xgi::Output *out) throw(xgi::exception::Exception) {
...
//========================================================================================================================
void WizardSupervisor::requestIcons(xgi::Input *in, xgi::Output *out) throw(xgi::exception::Exception) {
  cgicc::Cgicc cgi(in);
  ...
  *out << "Configure,CFG,0,1,icon-Configure.png,/urn:xdaq-application:lid=280/,/"
      ...
      "RecordWiz_ConfigurationGUI.html?urn=280&recordAlias=Front%2Dend,Config Wizards"
       ...
       << ",Block Diagram,CFG,0,1,icon-Configure.png,/WebPath/html/ConfigurationSubsetBlockDiagram.html?urn=280,Config "
          "Wizards"
       <<
      //",Consumer
...
//========================================================================================================================
void WizardSupervisor::editSecurity(xgi::Input *in, xgi::Output *out) throw(xgi::exception::Exception) {
...
//========================================================================================================================
void WizardSupervisor::UserSettings(xgi::Input *in, xgi::Output *out) throw(xgi::exception::Exception) {
  // if sequence doesn't match up -> return
  cgicc::Cgicc cgi(in);
  std::string submittedSequence = CgiDataUtilities::postData(cgi, "sequence");
  std::string securityFileName = SECURITY_FILE_NAME;
  ...
  if (Command != "") {
    ...
    if (Command == "Import") {
      ...
      //
      //
      //			if(std::filesystem::status_known(tmpDir) ? std::filesystem::exists(tmpDir) :
      //std::filesystem::exists(tmpDir))
...
std::string WizardSupervisor::validateUploadFileType(const std::string fileType) {
  for (unsigned int i = 0; i < allowedFileUploadTypes_.size(); ++i)
    if (allowedFileUploadTypes_[i] == fileType) return matchingFileUploadTypes_[i];  // found and done
...
void WizardSupervisor::savePostPreview(std::string &subject, std::string &text,
...
*/
  ...
  /*
          ...
          for (unsigned int i=0; i<files.size(); ++i)
          {
                  ...
                  if(xmldoc) xmldoc->addTextElementToData(XML_LOGBOOK_ENTRY_FILE,files[i].getDataType()); //return file
     type
