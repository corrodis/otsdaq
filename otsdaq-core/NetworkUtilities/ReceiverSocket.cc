...
//========================================================================================================================
int ReceiverSocket::receive(std::string& buffer, unsigned int timeoutSeconds, unsigned int timeoutUSeconds,
                            bool verbose) {
...
int ReceiverSocket::receive(std::string& buffer, unsigned long& fromIPAddress, unsigned short& fromPort,
  ...
  if (FD_ISSET(socketNumber_, &fileDescriptor_)) {
    buffer.resize(maxSocketSize_);  // NOTE: this is inexpensive according to Lorenzo/documentation in C++11 (only
                                    // increases size once and doesn't decrease size)
    if ((numberOfBytes_ = recvfrom(socketNumber_, &buffer[0], maxSocketSize_, 0, (struct sockaddr*)&fromAddress_,
                                   &addressLength_)) == -1) {
    ...
    //__COUT__ << __PRETTY_FUNCTION__ << "IP: " << std::hex << fromIPAddress << std::dec << " port: " << fromPort <<
    //std::endl;
    ...
    // NOTE: this is inexpensive according to Lorenzo/documentation in C++11 (only increases size once and doesn't
    // decrease size)
...
//========================================================================================================================
int ReceiverSocket::receive(std::vector<uint32_t>& buffer, unsigned int timeoutSeconds, unsigned int timeoutUSeconds,
                            bool verbose) {
  return receive(buffer, dummyIPAddress_, dummyPort_, timeoutSeconds, timeoutUSeconds, verbose);
}
...
//	returns 0 on success, -1 on failure
//	NOTE: must call Socket::initialize before receiving!
int ReceiverSocket::receive(std::vector<uint32_t>& buffer, unsigned long& fromIPAddress, unsigned short& fromPort,
                            unsigned int timeoutSeconds, unsigned int timeoutUSeconds, bool verbose) {
  // lockout other receivers for the remainder of the scope
  std::lock_guard<std::mutex> lock(receiveMutex_);
  ...
  if (FD_ISSET(socketNumber_, &fileDescriptor_)) {
    buffer.resize(maxSocketSize_ / sizeof(uint32_t));  // NOTE: this is inexpensive according to Lorezno/documentation
    ...
    if (verbose)
      __COUT__ << __COUT_HDR_FL__ << "No new messages for " << timeoutSeconds + timeoutUSeconds / 1000. << "s (Total "
               << readCounter_ * (timeoutSeconds + timeoutUSeconds / 1000.)
               << "s). Read request timed out for port: " << ntohs(sin.sin_port) << std::endl;
    return -1;
  }
