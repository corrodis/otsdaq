...
int TransmitterSocket::send(Socket& toSocket, const std::string& buffer, bool verbose) {
  ...
  while (offset < buffer.size() && sts > 0) {
    ...
    {
      __COUT__ << "Sending "
               << " from: " << getIPAddress() << ":" << ntohs(socketAddress_.sin_port)
               << " to: " << toSocket.getIPAddress() << ":" << ntohs(toSocket.getSocketAddress().sin_port)
    ...
    if (delay)
      ...
      delay = true;
    sts = sendto(socketNumber_, buffer.c_str() + offset, thisSize, 0, (struct sockaddr*)&(toSocket.getSocketAddress()),
                 sizeof(sockaddr_in));
    offset += sts;
  }

  if (sts <= 0) {
    __COUT__ << "Error writing buffer for port " << ntohs(socketAddress_.sin_port) << ": " << strerror(errno)
             << std::endl;
    return -1;
  }
  return 0;
...
int TransmitterSocket::send(Socket& toSocket, const std::vector<uint16_t>& buffer, bool verbose) {
  ...
  while (offset < buffer.size() && sts > 0) {
    ...
    sts = sendto(socketNumber_, &buffer[0] + offset, thisSize, 0, (struct sockaddr*)&(toSocket.getSocketAddress()),
                 sizeof(sockaddr_in));
    offset += sts / 2;
  }
