...
Socket::Socket(void) {
  __SS__ << "ERROR: This method should never be called. This is the protected constructor. There is something wrong in "
            "your inheritance scheme!"
         << std::endl;
  __COUT_ERR__ << "\n" << ss.str();
...
//========================================================================================================================
Socket::~Socket(void) {
  __COUT__ << "CLOSING THE SOCKET #" << socketNumber_ << " IP: " << IPAddress_ << " port: " << getPort()
           << " htons: " << socketAddress_.sin_port << std::endl;
  if (socketNumber_ != -1) close(socketNumber_);
}

//========================================================================================================================
void Socket::initialize(unsigned int socketReceiveBufferSize) {
  __COUT__ << "Initializing port " << ntohs(socketAddress_.sin_port) << " htons: " << socketAddress_.sin_port
           << std::endl;
  ...
  if (ntohs(socketAddress_.sin_port) != 0) fromPort = toPort = ntohs(socketAddress_.sin_port);
  ...
  for (int p = fromPort; p <= toPort && !socketInitialized; p++) {
    ...
    if (bind(socketNumber_, res->ai_addr, res->ai_addrlen) == -1) {
      std::cout << __COUT_HDR_FL__ << "Error********Error********Error********Error********Error********Error"
                << std::endl;
      ...
      std::cout << __COUT_HDR_FL__ << ":):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):)"
                << std::endl;
      std::cout << __COUT_HDR_FL__ << "SOCKET ON PORT: " << port.str() << " ON IP: " << IPAddress_ << " INITIALIZED OK!"
                << std::endl;
      std::cout << __COUT_HDR_FL__ << ":):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):)"
                << std::endl;
      std::cout << __COUT_HDR_FL__ << ":):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):):)"
  ...
  __COUT__ << "Setting socket receive buffer size = " << socketReceiveBufferSize << " 0x" << std::hex
           << socketReceiveBufferSize << std::dec << __E__;
  if (setsockopt(socketNumber_, SOL_SOCKET, SO_RCVBUF, (char*)&socketReceiveBufferSize,
